...
class Catalogo_cuentas_model extends CI_model {
		...
		else
		{
			...
			else if ($strNivel !== NULL)//Si existe nivel
			{
				...
				else if($strNivel == 'NIVEL 3')
				{
					...
					//Si el tipo no corresponde a balanza de comprobación, significa que todas las cuentas se mostrarán en el reporte balanza de comprobación
					if($strTipo != 'BALANZA COMPROBACION')
					{
						$this->db->where('CC.tercer_nivel <>', '00');
						$this->db->where('CC.cuarto_nivel', '00000');
					}
    ...
    public function buscar_saldos_cuenta_balanza_comp($dteFechaInicial, $dteFechaFinal, $arrCta, $strNivel = NULL)
	{
		//Variables que se utilizan para agregar restricciones a la búsqueda de datos
		$strRestricciones = '';
		...
		{	
			//Se comenta esta parte para que salga igual al resumen de auxiliares
		...
		//Si el nivel 3 se encuentra seleccionado
		if($strNivel == "NIVEL 3")
		{
			...
			if($strCuartoNivel != "00000")
			{
				...
				{
					$queryCuenta .= " AND CC.cuarto_nivel = '$strCuartoNivel' ";
				}
			}
		}
	 ...
	 /*Método para regresar los saldos de una cuenta contable
     *(se utiliza en el reporte estados financieros)*/
	public function buscar_saldos_ctas_estados_financieros($dteFechaInicial, $dteFechaFinal, $arrCuentas)
	{


		//Variable que se utiliza para agregar los saldos de la cuenta
		$querySaldosCta = "SELECT CC.cuenta_id, ";
		$querySaldosCta .= " IFNULL((SELECT SUM(PD.importe) AS importe
							        FROM  polizas P
							        INNER JOIN  polizas_detalles AS PD ON P.poliza_id = PD.poliza_id
						            WHERE  P.fecha < '$dteFechaInicial'
							        AND    P.estatus ='ACTIVO'
						            AND    PD.cuenta_id = CC.cuenta_id 
							        AND    PD.naturaleza = 'CARGO'),0) AS cargos_saldo_inicial, ";
		...
		$querySaldosCta .= " IFNULL((SELECT SUM(PD.importe) AS importe
							        ...
							        AND    P.estatus ='ACTIVO'
						            AND    PD.cuenta_id = CC.cuenta_id 
							        AND    PD.naturaleza = 'ABONO'),0) AS abonos_saldo_inicial, ";

		$querySaldosCta .= " IFNULL((SELECT SUM(PD.importe) AS importe
							        FROM  polizas P
							        INNER JOIN  polizas_detalles AS PD ON P.poliza_id = PD.poliza_id
						            WHERE  P.fecha >= '$dteFechaInicial'
				   					AND    P.fecha <= '$dteFechaFinal' 
							        AND    P.estatus ='ACTIVO'
						            AND    PD.cuenta_id = CC.cuenta_id 
							        AND    PD.naturaleza = 'CARGO'),0) AS cargos_saldo_actual, ";
		...
		$querySaldosCta .= " IFNULL((SELECT SUM(PD.importe) AS importe
							        ...
							        AND    P.estatus ='ACTIVO'
						            AND    PD.cuenta_id = CC.cuenta_id 
							        AND    PD.naturaleza = 'ABONO'),0) AS abonos_saldo_actual ";
	    $querySaldosCta .= " FROM catalogo_cuentas AS CC ";
	    $querySaldosCta .= " WHERE (";

	    //Separar array para obtener cuentas
		$vecCuentas = explode("|", $arrCuentas);
		$intTemp = 0;

	    //Hacer recorrido para obtener las cuentas del vector
	    while ($intTemp < count($vecCuentas))
	    {
	    	//Separar array para obtener niveles de la cuenta
			$arrCuenta = explode("-", $vecCuentas[$intTemp]);
			$strPrimerNivel = $arrCuenta[0];
			$strSegundoNivel = $arrCuenta[1];
			$strTercerNivel = $arrCuenta[2];
			$strCuartoNivel = $arrCuenta[3];

			
	    	//Si el contador es mayor a cero (concatenar OR a la cadena para obtener datos de otra cuenta)
			if($intTemp > 0)
			{
			...
			$querySaldosCta .=  " CC.primer_nivel = '".$strPrimerNivel."' ";
	...
	public function buscar_saldos_ctas_estados_financierosAct($dteFechaInicial, $dteFechaFinal, $arrCuentas, $strNaturalezaBusq = FALSE)
	{


		//Variable que se utiliza para agregar los saldos de la cuenta
		$querySaldosCta = "SELECT CC.cuenta_id, CC.naturaleza, ";
		$querySaldosCta .= " IFNULL((SELECT SUM(PD.importe) AS importe
							        FROM  polizas P
							        INNER JOIN  polizas_detalles AS PD ON P.poliza_id = PD.poliza_id
						            WHERE  P.fecha < '$dteFechaInicial'
		...
		if($strNaturalezaBusq != NULL)
		{
			$strNaturaleza = $strNaturalezaBusq;
		}
		...
		{
			//Si se cumple la sentencia, tomar la naturaleza de la primer cuenta
			if($intContCta == 0 && $strNaturaleza == '')
			{
				//Asignar naturaleza de la cuenta
				$strNaturaleza  = $arrSdo->naturaleza;
			}
			...
			//Incrementar contador por cada cuenta
			$intContCta++;
		...
		$objCuenta->saldo_anterior = $intSdoAnterior;
		$objCuenta->movimientos = $intMovMes;
		$objCuenta->saldo_final = $intSdoFinal;		
		$objCuenta->naturaleza = $strNaturaleza;		
   ...
   /*Método para regresar el acumulado de los saldos de las cuentas contables
										  	 	   ...
										  	 	   $strCuentaFinal, $strFormulario = NULL)
	{
		...
		$queryCuentas = "SELECT cuenta_id, descripcion, naturaleza, acepta_movimientos, 
							    CONCAT_WS(' ', primer_nivel, segundo_nivel, tercer_nivel, cuarto_nivel) AS cuenta, 
							    descripcion, primer_nivel, segundo_nivel, tercer_nivel, cuarto_nivel
		...
		foreach ($objCuentas->result() as $arrCta)
		{
			...
			$querySaldosCta .= " FROM catalogo_cuentas AS CC ";
			$queryNivel = " WHERE CC.primer_nivel = '$strPrimerNivel' ";

			//Si existe cuenta del segundo nivel
			if($strSegundoNivel != "00")
			{
				$queryNivel .= " AND CC.segundo_nivel = '$strSegundoNivel' ";
			}

			//Si existe cuenta del tercer nivel
			if($strTercerNivel != "00")
			{
				$queryNivel .= " AND CC.tercer_nivel = '$strTercerNivel' ";
			}
			...
			{
				if ($arrCta->acepta_movimientos == "NO"){
					$queryNivel ="WHERE  CC.cuenta_padre_id = ".$arrCta->cuenta_id ;
				}
				else
				{
					$queryNivel .= " AND CC. cuarto_nivel = '$strCuartoNivel' ";
				}
			}

			$querySaldosCta .= $queryNivel;
			...
			$objCuenta->acepta_movimientos = $arrCta->acepta_movimientos;
	...
	/*Método para regresar los cargos (detalles) de la cuenta de un cliente
     *(se utiliza al momento de generar una póliza de aplicación)*/
	public function buscar_cargos_cliente($strCodCli, $dteFecha, $strPrimerNivel = NULL, $strTercerNivel = NULL)
	{
		//Variables que se utilizan para agregar restricciones a la búsqueda de datos
		$strRestricciones = '';

		//Si existe primer nivel de la cuenta
		if($strPrimerNivel !== NULL)
		{
			$strRestricciones .= " AND CC.primer_nivel = '$strPrimerNivel'";
		}

		//Si existe tercer nivel de la cuenta
		if($strTercerNivel !== NULL)
		{
			$strRestricciones .= " AND CC.tercer_nivel = '$strTercerNivel'";
		}


		$strSQL = $this->db->query("SELECT P.poliza_id, P.fecha, CC.cuenta_id, CC.segundo_nivel, CC.tercer_nivel, PD.importe
								    FROM   polizas P INNER JOIN polizas_detalles PD ON PD.poliza_id = P.poliza_id
								    ...
								    AND    PD.naturaleza = 'CARGO'
								    $strRestricciones
								    AND    CC.cuarto_nivel = '$strCodCli'
								    ORDER BY P.fecha, CC.segundo_nivel, CC.tercer_nivel");
	...
	public function buscar_cuentas_balanza()
	{
		$strSQL = $this->db->query("SELECT cuenta_id, cuenta_padre_id, sat_cuenta_id, 
									...
									FROM   catalogo_cuentas 
									WHERE  cuarto_nivel = _latin1'00000'
									ORDER BY  primer_nivel, segundo_nivel, tercer_nivel, cuarto_nivel");
    ...
    public function buscar_saldos_cuenta_balanza($strAnio, $strMes, $strPrimerNivel, $strSegundoNivel, 
    											 $strTercerNivel, $strAceptaMovimientos)
	{
		...
		$querySaldosCta = " SELECT (SELECT SUM(PD.importe) AS importe 
			...
			if (($strTercerNivel != "00") OR ($strAceptaMovimientos == "SI"))
			{
				$querySaldosCta .= " AND CC.tercer_nivel = '$strTercerNivel' ";
			}
	...
	public function filtro($strBusqueda, $intNumRows, $intPos)
	{
        ...
        $this->db->or_like('SCC.codigo', $strBusqueda);
        $this->db->or_like('SCC.descripcion', $strBusqueda);
        $this->db->or_like('CC.tipo_cuenta', $strBusqueda);
		$this->db->from('catalogo_cuentas CC');
		$this->db->join('sat_catalogo_cuentas AS SCC', 'CC.sat_cuenta_id = SCC.sat_cuenta_id', 'inner');
		$this->db->join('catalogo_cuentas AS CCP', 'CC.cuenta_padre_id = CCP.cuenta_id', 'left');
		...
		$this->db->select("CC.cuenta_id, 
						   CONCAT_WS(' ', CC.primer_nivel, CC.segundo_nivel, CC.tercer_nivel, CC.cuarto_nivel) AS cuenta,
						   CC.descripcion, CC.tipo_cuenta, CC.naturaleza, CC.estatus, 
						   CONCAT_WS(' - ', SCC.codigo, SCC.descripcion) AS sat_cuenta,
						   CASE 
							    WHEN   CC.cuenta_padre_id > 0 
							    THEN  CONCAT_WS(' ', CCP.primer_nivel, CCP.segundo_nivel, CCP.tercer_nivel, CCP.cuarto_nivel, '-', CCP.descripcion)

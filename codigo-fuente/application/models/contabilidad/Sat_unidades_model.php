...
class Sat_unidades_model extends CI_model {
	...
	public function guardar(stdClass $objSatUnidad)
	{
		...
		return $this->db->insert('sat_unidades', $arrDatos);
	}
	...
	public function modificar(stdClass $objSatUnidad)
	{
		...
		$this->db->where('unidad_id', $objSatUnidad->intUnidadID);
		$this->db->limit(1);
		//Actualizar los datos del registro
		return $this->db->update('sat_unidades', $arrDatos);
	}

	//Método para modificar el estatus de un registro
	public function set_estatus($intUnidadID, $strEstatus)
	{
		...
		$this->db->where('unidad_id', $intUnidadID);
		$this->db->limit(1);
		//Actualizar los datos del registro
		return $this->db->update('sat_unidades', $arrDatos);
	}
	...
	public function buscar($intUnidadID = NULL, $strCodigo = NULL, $strBusqueda = NULL)
	{
		$this->db->select('unidad_id, codigo, nombre, descripcion, estatus');
		$this->db->from('sat_unidades');
	...
	public function filtro($strBusqueda, $intNumRows, $intPos)
	{
		...
		$this->db->select('unidad_id, codigo, nombre, estatus');
		$this->db->from('sat_unidades');
		$this->db->like('codigo', $strBusqueda);
	    $this->db->or_like('nombre', $strBusqueda);
	    $this->db->or_like('estatus', $strBusqueda);
	...
	{
		$this->db->select(" unidad_id, CONCAT_WS(' - ', codigo, nombre) AS unidad ", FALSE);
        $this->db->from('sat_unidades');

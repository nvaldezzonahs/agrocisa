...
class Refacciones_marcas_model extends CI_model {
	...
	public function guardar(stdClass $objMarca)
	{
		...
		return $this->db->insert('refacciones_marcas', $arrDatos);
	}
	...
	public function modificar(stdClass $objMarca)
	{
		...
		$this->db->where('refacciones_marca_id', $objMarca->intRefaccionesMarcaID);
		$this->db->limit(1);
		//Actualizar los datos del registro
		return $this->db->update('refacciones_marcas', $arrDatos);
	}

    //Método para modificar el estatus de un registro
	public function set_estatus($intRefaccionesMarcaID, $strEstatus)
	{
		...
		$this->db->where('refacciones_marca_id', $intRefaccionesMarcaID);
		$this->db->limit(1);
		//Actualizar los datos del registro
		return $this->db->update('refacciones_marcas', $arrDatos);
	}
	...
	{
		$this->db->select('refacciones_marca_id, descripcion, estatus');
		$this->db->from('refacciones_marcas');
		//Si existe id de la marca de refacciones
		if ($intMaquinariaMarcaID !== NULL)
		{   
			$this->db->where('refacciones_marca_id', $intMaquinariaMarcaID);
			$this->db->limit(1);
	...
	{
		$this->db->select(' refacciones_marca_id,  descripcion ');
        $this->db->from('refacciones_marcas');

...
class Salidas_maquinaria_proveedor_model extends CI_model {

	//Método para guardar un registro nuevo
	public function guardar($strFolio, $objSalidaProveedor)
	{
		...
		$arrDatos = array('sucursal_id' => $this->session->userdata('sucursal_id'),
						  'tipo_movimiento' => mb_strtoupper($objSalidaProveedor->strTipoMovimiento),
						  'folio' => $strFolioConsecutivo, 
						  'fecha' => mb_strtoupper($objSalidaProveedor->strFecha),  
						  'referencia_id'=> $objSalidaProveedor->intReferenciaID,
						  'observaciones' => mb_strtoupper($objSalidaProveedor->strObservaciones),
		...
		$this->guardar_maquinaria_detalles($intMovimientoMaquinariaID, $objSalidaProveedor->arrMaquinarias);
		
		//Hacer un llamado para actualizar el campo salida_id en la tabla maquinaria_inventario
		$this->actualizar_maquinaria_inventario($intMovimientoMaquinariaID, $objSalidaProveedor->arrMaquinarias);
	...
	public function modificar($intMovimientoMaquinariaID, $objSalidaProveedor)
	{
		//Iniciamos la transacción
		$this->db->trans_begin();
		//Asignar datos al array
	...
	{	
	    ...
	    $this->db->where("MM.tipo_movimiento", SALIDA_MAQUINARIA_DEVOLUCION_PROVEEDOR);
	    $this->db->where("MM.sucursal_id", $this->session->userdata('sucursal_id'));
		$this->db->from('movimientos_maquinaria MM');
		$this->db->join('movimientos_maquinaria MM2', 'MM2.movimiento_maquinaria_id = MM.referencia_id AND MM2.tipo_movimiento = 1', 'inner');
		$this->db->join('ordenes_compra_maquinaria OCM', 'OCM.orden_compra_maquinaria_id = MM2.referencia_id', 'inner');
	    $this->db->join('proveedores P', 'P.proveedor_id = OCM.proveedor_id', 'inner');
	...
	//Método para regresar los registros que coincidan con el criterio de búsqueda proporcionado
	public function buscar($intMovimientoMaquinariaID = NULL, $dteFechaInicial = NULL, $dteFechaFinal = NULL, $intProveedorID = NULL, 

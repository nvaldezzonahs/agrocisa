...
class Salidas_maquinaria_ajuste_model extends CI_model {

	//Método para guardar un registro nuevo
	public function guardar($strFolio, $objSalidaAjuste)
	{
		...
		$arrDatos = array('sucursal_id' => $this->session->userdata('sucursal_id'),
						  'tipo_movimiento' => mb_strtoupper($objSalidaAjuste->strTipoMovimiento),
						  'folio' => $strFolioConsecutivo, 
						  'fecha' => mb_strtoupper($objSalidaAjuste->strFecha),
						  'observaciones' => mb_strtoupper($objSalidaAjuste->strObservaciones),
		...
		$this->guardar_maquinaria_detalles($intMovimientoMaquinariaID, $objSalidaAjuste->arrMaquinarias);

		//Hacer un llamado para actualizar el campo salida_id en la tabla maquinaria_inventario
		$this->actualizar_maquinaria_inventario($intMovimientoMaquinariaID, $objSalidaAjuste->arrMaquinarias, 'INACTIVO');
	...
	public function modificar($intMovimientoMaquinariaID, $objSalidaAjuste)
	{
		//Iniciamos la transacción
		$this->db->trans_begin();
		//Asignar datos al array
	...
	public function set_estatus($intMovimientoMaquinariaID, $strEstatus)
	{
		...
		if($strEstatus == 'ACTIVO'){
			$strEstatusInv = 'INACTIVO';
		}
		else{
			$strEstatusInv = 'ACTIVO';
		}
		$this->actualizar_maquinaria_inventario($intMovimientoMaquinariaID, $otdMaquinarias, $strEstatusInv);
	...
	public function actualizar_maquinaria_inventario($intMovimientoMaquinariaID, $arrMaquinarias, $strEstatus)
	{
	...
	public function filtro($dteFechaInicial = NULL, $dteFechaFinal = NULL,  $intNumRows, $intPos)
	{	
		//Seleccionar el número de registros que coincidan con los criterios de búsqueda
		if($dteFechaInicial != '0000-00-00' && $dteFechaFinal != '0000-00-00')
	    {
	   		$this->db->where("DATE(MM.fecha) BETWEEN '$dteFechaInicial' AND '$dteFechaFinal'", NULL, FALSE);
	    }
	    $this->db->from('movimientos_maquinaria AS MM');
		$this->db->join('usuarios AS UC', 'UC.usuario_id = MM.usuario_creacion', 'left');
		$this->db->where('MM.tipo_movimiento', SALIDA_MAQUINARIA_POR_AJUSTE);
	...
	public function buscar_detalles($intMovimientoMaquinariaID)
	{
		$this->db->select('	MMD.movimiento_maquinaria_id, 
							...
							MMD.maquinaria_descripcion_id AS strMaquinariaDescripcionID,
							MMD.codigo, 
							MMD.descripcion_corta, 
							MMD.descripcion, 
						    MMD.serie,
						    MMD.serie AS strSerie, 
						    MMD.motor,
						    MI.numero_pedimento,
						    MI.consignacion ');

...
class Solicitudes_traspasos_refacciones_internas_model extends CI_model {
	...
	public function guardar(stdClass $objSolicitudTraspaso)
	{
		...
		//Tabla solicitudes_traspasos_refacciones_internas
		...
		$this->db->insert('solicitudes_traspasos_refacciones_internas', $arrDatos);
		//Agregar id del nuevo registro al objeto
		$objSolicitudTraspaso->intSolicitudTraspasoRefaccionesInternasID = $this->db->insert_id();
	...
	public function modificar(stdClass $objSolicitudTraspaso)
	{
		...
		$this->db->where('solicitud_traspaso_refacciones_internas_id', $objSolicitudTraspaso->intSolicitudTraspasoRefaccionesInternasID);
		$this->db->limit(1);
		//Actualizar los datos del registro
		$this->db->update('solicitudes_traspasos_refacciones_internas', $arrDatos);

		//Eliminar los detalles guardados
		$this->db->where('solicitud_traspaso_refacciones_internas_id', $objSolicitudTraspaso->intSolicitudTraspasoRefaccionesInternasID);
		$this->db->delete('solicitudes_traspasos_refacciones_internas_detalles');
	...
	public function set_estatus($intSolicitudTraspasoRefaccionesInternasID, $strEstatus)
	{
	...
	public function buscar($intSolicitudTraspasoRefaccionesInternasID = NULL, $dteFechaInicial = NULL, 
						   $dteFechaFinal = NULL, $strEstatus = NULL, $strBusqueda = NULL)
	{
		$this->db->select("STR.solicitud_traspaso_refacciones_internas_id, STR.folio, 
						   DATE_FORMAT(STR.fecha,'%d/%m/%Y') AS fecha, 
						   STR.observaciones, STR.estatus, 
						   UC.usuario AS usuario_creacion,
						   DATE_FORMAT(STR.fecha_creacion,'%d/%m/%Y %r') AS fecha_creacion", FALSE);
		$this->db->from('solicitudes_traspasos_refacciones_internas AS STR');
		$this->db->join('usuarios AS UC', 'STR.usuario_creacion = UC.usuario_id', 'left');
		//Si existe id de la solicitud de traspaso
		if ($intSolicitudTraspasoRefaccionesInternasID !== NULL)
		{   
			$this->db->where('STR.solicitud_traspaso_refacciones_internas_id', $intSolicitudTraspasoRefaccionesInternasID);
			$this->db->limit(1);
		...
		else
		{
			...
			$this->db->where("(STR.folio LIKE '%$strBusqueda%')"); 
		                   ...
		                   $strEstatus,$strBusqueda = NULL, $intNumRows, $intPos)
	{
		...
		$this->db->select("STR.solicitud_traspaso_refacciones_internas_id, STR.folio,
						   DATE_FORMAT(STR.fecha,'%d/%m/%Y') AS fecha, STR.estatus", FALSE);
		$this->db->from('solicitudes_traspasos_refacciones_internas AS STR');
		$this->db->join('sucursales AS ST', 'STR.sucursal_id = ST.sucursal_id', 'inner');
		$this->db->where('STR.sucursal_id',  $this->session->userdata('sucursal_id'));
		//Si existe rango de fechas
	...
	*/
	...
	{
		$this->db->select("STR.solicitud_traspaso_refacciones_internas_id, 
			               STR.folio", FALSE);
        $this->db->from('solicitudes_traspasos_refacciones_internas AS STR');
        ...
        $this->db->where("(STR.folio LIKE '%$strDescripcion%')");  
	...
	/*******************************************************************************************************************
	Funciones de la tabla solicitudes_traspasos_refacciones_internas_detalles
	*********************************************************************************************************************/
	...
	public function guardar_detalles(stdClass $objSolicitudTraspaso)
	{
		...
		//Hacer recorrido para insertar los datos en la tabla solicitudes_traspasos_refacciones_internas_detalles
		for ($intCon = 0; $intCon < sizeof($arrRefaccionID); $intCon++) 
		{
			//Asignar datos al array
			$arrDatos = array('solicitud_traspaso_refacciones_internas_id' => $objSolicitudTraspaso->intSolicitudTraspasoRefaccionesInternasID,
							  'renglon' => ($intCon + 1),
			...
			//Guardar los datos del registro
			$this->db->insert('solicitudes_traspasos_refacciones_internas_detalles', $arrDatos);
		}
	}

	//Método para regresar los detalles de un registro
	public function buscar_detalles($intSolicitudTraspasoRefaccionesInternasID)
	{
		...
		$this->db->where('STRD.solicitud_traspaso_refacciones_internas_id', $intSolicitudTraspasoRefaccionesInternasID);

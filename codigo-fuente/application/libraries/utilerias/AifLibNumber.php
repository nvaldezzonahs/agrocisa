 ...
 * seicientos veintiseis mil cuatrocientos treinta y tres millones
...
class AifLibNumber {
    ...
    * [4] Notacion larga Cifras cerradas
    * [5] Notacion larga para los sufijos..
    * El arreglo 4 y 5 se puede incrementar para soportar numeros superiores
    * @var array
    */
    private static $nStr = array(array('cero', 'un'),
    ...
    //Arreglo se utiliza para asignar los datos de la moneda
    private static $ARR_TIPO_MONEDA = array("MXN" => "pesos_peso_M.N.",
                                            "XXX" => "pesos_peso_M.N.",
                                            "USD" => "dolares_dolar_USD");
    ...
    * @param string $number
     * @param string $strCodigoMoneda
    * @return boolean|string
    */
    public static function toCurrency($number, $strCodigoMoneda = NULL) 
    {   
        //Variable que se utiliza para asignar los datos de la moneda que coincida con el código
        $strDatosMoneda = '';

        //Si mo existe el código de la moneda
        if($strCodigoMoneda === NULL)
        {  
           //Asignar el código de la moneda correspondiente al peso mexicano
           $strCodigoMoneda = 'MXN';
        }

        //Asignar los datos de la moneda que coincida con el código
        $strDatosMoneda = self::$ARR_TIPO_MONEDA[$strCodigoMoneda];
        //Se quita el guion  de la cadena que contiene los datos de la moneda
        list($strTipoMonedaPlural, $strTipoMonedaSingular, $strMonedaNacional) = explode("_", $strDatosMoneda);
        ...
        if (!$number)
            return FALSE;
        if (preg_match('/(llones|llon)$/', $number))
            $number .= ' de '.$strTipoMonedaPlural.' ';
        else
            $number = preg_match('/uno$/', $number) ? (preg_replace('/uno$/',
                            '', $number) . ' un '.$strTipoMonedaSingular) : ($number . $strTipoMonedaPlural.' ');
       // $decimal = round($decimal[0] . $decimal[1] . '.' . substr($decimal, 2));
        return strtoupper($number . $decimal . '/100 '.$strMonedaNacional);
    }

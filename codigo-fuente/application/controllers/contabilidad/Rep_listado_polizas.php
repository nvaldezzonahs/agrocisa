...
class Rep_listado_polizas extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('contabilidad/rep_listado_polizas.php', $arrDatos);
	}
	...
	/*Método para generar un reporte PDF con el listado de pólizas
	...
	public function get_reporte() 
	{
		...
		$strFolioInicial = trim($this->input->post('strFolioInicial'));
		$strFolioFinal = trim($this->input->post('strFolioFinal'));
		...
		$otdResultado = $this->polizas->buscar_polizas($dteFechaInicial, $dteFechaFinal,
								  					   $strFolioInicial, $strFolioFinal, 
								  					   $strTipoPoliza, $strSucursales);
		...
		{
			$pdf->strLinea3 =  utf8_decode('TIPO DE PÓLIZA: '.$strTipoPoliza);
		}
        ...
        $pdf->Output('listado_polizas.pdf','I'); 
	}
	

	/*Método para generar un archivo XLS con el listado de pólizas
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ...
			     ->setCellValue('A8', 'SUCURSAL: '.$strTituloSucursales);

		//Si existe tipo de póliza
		if($strTipoPoliza != NULL)
		{
			$objExcel->setActiveSheetIndex(0)
			         ->setCellValue('A9', 'TIPO DE PÓLIZA: '.$strTipoPoliza);
		}
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'listado_polizas.xls', 'listado de pólizas', $intFila);
	}
}
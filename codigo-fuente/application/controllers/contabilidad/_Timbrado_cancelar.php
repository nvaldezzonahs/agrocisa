...
class Timbrado_cancelar extends MY_Controller {
	...
	{	
		...
		//------------------------------------------------------------------------------------------------------------------------
		$rutaLlavePrivada = "./certificados/".$otdCertificado->folio.".key";
		$rutaCertificado = "./certificados/".$otdCertificado->folio.".cer";

		//ECHO 'EL certificados es '.$otdCertificado->folio;
		...
		//$otdMovimiento[0]->RFC = 'eku9003173c9';
		...
		$objCancelar->folios = Array(0 => $otdMovimiento[0]->uuid);
		$objCancelar->motivo = $strCodigoMotivo;

	
		//Si el código del motivo de cancelación es 01-Comprobante emitido con errores con relación.
		if($strCodigoMotivo == '01')
		{
			$objCancelar->folioSustitucion = $strUuidSustitucion;
		}
		...
		$objRespuesta = $objClienteSOAP->Cancelacion40_1($objCancelar);

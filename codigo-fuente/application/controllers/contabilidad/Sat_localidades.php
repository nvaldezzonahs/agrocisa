...
class Sat_localidades extends MY_Controller {
	...
	function __construct()
	{
		...
		//Cargamos el modelo
		$this->load->model('contabilidad/sat_localidades_model', 'localidades');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('contabilidad/sat_localidades', $arrDatos);
	}
	...
	{ 
		...
		$objSatLocalidad = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista 
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objSatLocalidad->intLocalidadID = $this->input->post('intLocalidadID');
		$objSatLocalidad->intEstadoID = $this->input->post('intEstadoID');
		$objSatLocalidad->intEstadoIDAnterior = $this->input->post('intEstadoIDAnterior');
		$objSatLocalidad->strCodigo = trim($this->input->post('strCodigo'));
		$objSatLocalidad->strCodigoAnterior = trim($this->input->post('strCodigoAnterior'));
		$objSatLocalidad->strDescripcion = trim($this->input->post('strDescripcion'));
		$objSatLocalidad->intUsuarioID = $this->session->userdata('usuario_id');
		
        //Definir las reglas de validación
		//Validar que el código sea único por cada estado
        if (($objSatLocalidad->intLocalidadID == '') OR ($objSatLocalidad->strCodigoAnterior != $objSatLocalidad->strCodigo) OR 
        	($objSatLocalidad->intEstadoIDAnterior != $objSatLocalidad->intEstadoID))
        {

            $this->form_validation->set_rules('strCodigo', 'localidad', 
        									  'required|callback_get_existencia['.$objSatLocalidad->intEstadoID.']');
        }
        else
        {
		...
		else
		{
			...
			if (is_numeric($objSatLocalidad->intLocalidadID))
			{
				$bolResultado = $this->localidades->modificar($objSatLocalidad);
			}
			else
			{ 
				$bolResultado = $this->localidades->guardar($objSatLocalidad);
			}
	...
	//Verifica la existencia del código (localidad) en el estado
	//para evitar duplicación de datos al momento de modificar o guardar un registro.
    public function get_existencia($strCodigo, $intEstadoID) 
    {	
    	//Concatenar datos para realizar la búsqueda
    	$strCriteriosBusq = $intEstadoID.'|'.$strCodigo;
		...
		if($otdResultado)
		{
		    ...
		    $this->form_validation->set_message('get_existencia', 'La  %s ya ha sido registrada en el estado, favor de verificar.');
	...
	public function get_datos()
	{
		...
		{
			//Se recupera cadena concatenada con los criterios de búsqueda: estado_id|codigo
	...
	public function get_combo_box()
	{
		...
		$arrDatos['localidades'] = $this->localidades->get_combo_box();
		//Enviar datos a la vista del formulario
		$this->output->set_content_type('application/json')->set_output(json_encode($arrDatos));
	}
	...
	{	            
		...
		$otdResultado = $this->localidades->buscar(NULL, NULL, $strBusqueda); 
		...
		$pdf->strLinea1 = utf8_decode('LISTADO DE LOCALIDADES');
		//Establece los títulos de la cabecera
		$pdf->arrCabecera = array(utf8_decode('CÓDIGO'), utf8_decode('DESCRIPCIÓN'), 'ESTADO', 'ESTATUS');
		//Establece el ancho de las columnas de cabecera
		$pdf->arrAnchura = array(15, 95, 60, 20);
		...
		{	
			...
			{ 
				...
				$pdf->Row(array($arrCol->codigo, utf8_decode($arrCol->descripcion), 
								utf8_decode($arrCol->estado), $arrCol->estatus), 
								$pdf->arrAlineacion);
		...
		$pdf->Output('sat_localidades.pdf','I'); 
	}
	...
	{	
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'sat_localidades.xls', 'localidades', $intFila);
	}
}
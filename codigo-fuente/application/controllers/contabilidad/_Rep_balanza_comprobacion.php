...
class Rep_balanza_comprobacion extends MY_Controller {
	...
	public function get_reporte() 
	{
		...
		$pdf->arrAnchura = array(20, 52, 40, 40, 40);
		$pdf->arrAnchura2 = array(20, 52, 20, 20, 20, 20, 20, 20);
	...
	//Función que se utiliza para escribir los datos del reporte 
  	...
  	{	
		...
		$strAgregarCta = 'NO';
  		//Variable que se utiliza para asignar el acumulado de los cargos del saldo inicial
  		$intAcumCargosSdoInicial = 0;
  		//Variable que se utiliza para asignar el acumulado de los abonos del saldo inicial
		$intAcumAbonosSdoInicial = 0;
		//Variable que se utiliza para asignar el acumulado de los cargos de movimientos
		$intAcumCargosMov = 0;
		//Variable que se utiliza para asignar el acumulado de los abonos de movimientos
		$intAcumAbonosMov = 0;
		//Variable que se utiliza para asignar el acumulado de los cargos del saldo actual
  		$intAcumCargosSdoActual = 0;
  		//Variable que se utiliza para asignar el acumulado de los abonos del saldo actual
		$intAcumAbonosSdoActual = 0;


		//Variable que se utiliza para asignar cargos del saldo inicial
		$intCargosSdoInicial = 0;
		//Variable que se utiliza para asignar abonos del saldo inicial
		$intAbonosSdoInicial = 0;
		//Variable que se utiliza para asignar cargos del saldo actual
		$intCargosSdoActual = 0;
		//Variable que se utiliza para asignar bonos del saldo actual
		...
		//Calcular saldo inicial para la cuenta con naturaleza deudora
		$intSaldoInicialCtaD = $intCargosSdoInicial - $intAbonosSdoInicial;
		//Calcular saldo inicial para la cuenta con naturaleza acreedora
		$intSaldoInicialCtaA = $intAbonosSdoInicial - $intCargosSdoInicial;

		//Dependiendo del nivel incrementar acumulados
		switch ($strNivel)
		{
			case "NIVEL 1"://Primer nivel
				//Dependiendo de la naturaleza de la cuenta, incrementar acumulado del saldo inicial (anterior)
				if ($arrCta->naturaleza == "DEUDORA")
				{
					$intAcumCargosSdoInicial += $intSaldoInicialCtaD;
				}
				else
				{
					$intAcumAbonosSdoInicial += $intSaldoInicialCtaA;
				}

				//Incrementar acumulados de los movimientos
				$intAcumCargosMov += $intCargosSdoActual;
				$intAcumAbonosMov += $intAbonosSdoActual;
				break;
			case "NIVEL 2"://Segundo nivel
				//Si no existe cuenta padre
				if ($arrCta->cuenta_padre_id == "")
				{
					//Dependiendo de la naturaleza de la cuenta, incrementar acumulado del saldo inicial (anterior)
					if ($arrCta->naturaleza == "DEUDORA")
					{
		...
		$intSaldoActual = ($intCargosSdoInicial + $intCargosSdoActual - 
					 	   $intAbonosSdoInicial - $intAbonosSdoActual);

		//Dependiendo del tipo de reporte, mostrar información en el archivo
		if($strTipoReporte == 'TODAS')//Todas las cuentas
		{
			//Asignar SI para agregar datos de la cuenta
			$strAgregarCta = 'SI';
		}
		else if($strTipoReporte == 'SOLO_MOVIMIENTOS')//Sólo con movimientos en el rango de fechas
		{
			//Si se cumple la sentencia
			if (($intCargosSdoActual != 0) OR ($intAbonosSdoActual != 0))
			{
			    //Asignar SI para agregar datos de la cuenta
				$strAgregarCta = 'SI';
			}
		...
		{	
			...
			else
			{
				...
				if (($intSaldoActual > 0.01) OR ($intSaldoActual < -0.01))
				{
					//Asignar SI para agregar datos de la cuenta
					$strAgregarCta = 'SI';
				}
		...
		if($strAgregarCta == 'SI')
		{
			//Dependiendo del nivel asignar datos de la cuenta
			if (($arrCta->segundo_nivel == "00") && ($arrCta->tercer_nivel == "00")
				 && ($arrCta->cuarto_nivel == "00000"))
			{
				
				$arrAuxiliar['cuenta'] = $arrCta->primer_nivel;
					
			}
			else if ($arrCta->tercer_nivel == "00")
			{
				$arrAuxiliar['cuenta'] = $arrCta->primer_nivel." ".$arrCta->segundo_nivel;
			}
			else
			{
			...
			//Dependiendo de la naturaleza de la cuenta, asignar el saldo inicial (anterior)
			if ($arrCta->naturaleza == "DEUDORA")
			{	

				//Si existe saldo inicial de la cuenta
				if (($intSaldoInicialCtaD > 0.01) OR ($intSaldoInicialCtaD < -0.01))
				{
					//Si el tipo de archivo es PDF convertir cantidad a formato moneda
					$arrAuxiliar['cargos_saldo_inicial'] =  (($strTipoArchivo == 'PDF') ? 
							          			   			 '$'.number_format($intSaldoInicialCtaD, 2, '.', ',') : $intSaldoInicialCtaD);
			    }
				else
				{
					//Si el tipo de archivo es PDF convertir cantidad a formato moneda
				...
				$arrAuxiliar['abonos_saldo_inicial'] = '';

			}
			else
			{

				$arrAuxiliar['cargos_saldo_inicial'] = '';

				//Si existe saldo inicial de la cuenta
				if (($intSaldoInicialCtaA > 0.01) OR ($intSaldoInicialCtaA < -0.01))
				{
					//Si el tipo de archivo es PDF convertir cantidad a formato moneda
					$arrAuxiliar['abonos_saldo_inicial'] =  (($strTipoArchivo == 'PDF') ? 
							          			   			 '$'.number_format($intSaldoInicialCtaA, 2, '.', ',') : $intSaldoInicialCtaA);
			...
			$arrAuxiliar['cargos_movimientos'] = (($strTipoArchivo == 'PDF') ? 
							          			   '$'.number_format($intCargosSdoActual, 2, '.', ',') : 
							          			   $intCargosSdoActual);

			$arrAuxiliar['abonos_movimientos'] = (($strTipoArchivo == 'PDF') ? 
							          			   '$'.number_format($intAbonosSdoActual, 2, '.', ',') : 
							          			   $intAbonosSdoActual);


			//Dependiendo de la naturaleza de la cuenta, asignar el saldo actual
			if ($arrCta->naturaleza == "DEUDORA")
			{
				//Si el tipo de archivo es PDF convertir cantidad a formato moneda
				$arrAuxiliar['cargos_saldo_actual'] = (($strTipoArchivo == 'PDF') ? 
							          			   	   '$'.number_format($intSaldoActual, 2, '.', ',') : 
							          			   		$intSaldoActual);
				$arrAuxiliar['abonos_saldo_actual'] = '';
			}
			else
			{
				$arrAuxiliar['cargos_saldo_actual'] = '';
				//Si el tipo de archivo es PDF convertir cantidad a formato moneda
				$arrAuxiliar['abonos_saldo_actual'] = (($strTipoArchivo == 'PDF') ? 
							          			   	   '$'.number_format($intSaldoActual, 2, '.', ',') : 
							          			   		$intSaldoActual);
			}
			...
			$intAcumCargosSdoActual = $intAcumCargosSdoInicial + $intAcumCargosMov;
			//Calcular total de abonos
			$intAcumAbonosSdoActual = $intAcumAbonosSdoInicial + $intAcumAbonosMov;

		    $arrDatos['acumulado_cargos_saldo_inicial'] = $intAcumCargosSdoInicial;
		    $arrDatos['acumulado_abonos_saldo_inicial'] = $intAcumAbonosSdoInicial;
			$arrDatos['acumulado_cargos_movimientos'] = $intAcumCargosMov;
			$arrDatos['acumulado_abonos_movimientos'] = $intAcumAbonosMov;
			$arrDatos['acumulado_cargos_saldo_actual'] = $intAcumCargosSdoActual;
			$arrDatos['acumulado_abonos_saldo_actual'] = $intAcumAbonosSdoActual;

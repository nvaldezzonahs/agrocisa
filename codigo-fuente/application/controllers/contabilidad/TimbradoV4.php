...
class TimbradoV4 extends MY_Controller {
	...
	{	
		...
		//Variable que se utiliza para asignar el acumulado de pagos
		$intMontoTotalPagos = 0;
		...
		if($otdMovimiento)
		{
			...
			$strCadena.= $otdMovimiento->CodigoExportacion."|";// Exportacion
			...
			$strCadena.= $otdMovimiento->codigo_postal."|";//DomicilioFiscalReceptor
			//$strCadena.= "|";//--c. ResidenciaFiscal
			//$strCadena.= "|";//--d. NumRegIdTrib
			$strCadena.= $otdMovimiento->RegimenFiscal."|";//RegimenFiscalReceptor
			...
			{ 
				...
				foreach ($otdDetalles as $arrDet)
				{
					...
					{
						//Variable que se utiliza para asignar el monto total de pagos
						$intMontoTotalPagos += number_format($arrDet->MontoTotalPagos, 4, '.', '');
					...
					//Objeto de Impuesto
					$strCadena.= $arrDet->ClaveObjetoImpuesto."|";
					$strArchivoDetalles.= "ObjetoImp=\"".$arrDet->ClaveObjetoImpuesto."\" ";
			...
			if ($strTipoReferencia != "PAGO")
			{
				...
				for ($intConIVA = 0; $intConIVA < $numEleIVA; $intConIVA++)
				{
					$strCadena.=$numSubtotalDesc."|";//Base
			...
			if($strTipoReferencia == "PAGO")
			{
				...
				{ 
					...
					//--1. Versión
					$strCadena.= "2.0|";
					$strComplemento.="<pago20:Pagos Version=\"2.0\">";
					//MontoTotalPagos
					$strCadena.= $intMontoTotalPagos."|";
					$strComplemento.="<pago20:Totales MontoTotalPagos=\"".$intMontoTotalPagos."\" ";
					$strComplemento.="/>";

					//Recorremos el arreglo 
					foreach ($otdDetallesPago as $arrDetP)
					{
						$strComplemento.="<pago20:Pago ";
						...
						{
							//Asignar valor 1 para evitar error
							$intTipoCambioP = 1;
						}
						...
						{ 
							...
							foreach ($otdDetallesRelPago as $arrDetRP)
							{
								...
								$strComplemento.="<pago20:DoctoRelacionado ";
								//--a. IdDocumento
								...
								$strCadena.= $intTipoCambioRP."|";
								$strComplemento.="EquivalenciaDR=\"".$intTipoCambioRP."\" ";
								...
								//--f. ObjetoImpDR
								$strCadena.= $arrDetRP->objeto_impuesto_sat."|";
								$strComplemento.="ObjetoImpDR=\"".$arrDetRP->objeto_impuesto_sat."\" />";
							}
						}//Cierre de verificación de detalles relacionados
						$strComplemento.="</pago20:Pago>";
					}
					$strComplemento.="</pago20:Pagos>";
					$strComplemento.="</cfdi:Complemento>";
			...
			//ECHO 'LA CADENA FORMADA ES '.$strCadena;
			...
			$strArchivo.= "xmlns:cfdi=\"http://www.sat.gob.mx/cfd/4\" ";
			$strArchivo.= "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" ";
			//Si el tipo de referencia corresponde a PAGO
			if ($strTipoReferencia == "PAGO")
			{
				$strArchivo.= "xmlns:pago20=\"http://www.sat.gob.mx/Pagos20\" ";
			
				$strArchivo.= "xsi:schemaLocation=\"http://www.sat.gob.mx/cfd/4 http://www.sat.gob.mx/sitio_internet/cfd/4/cfdv40.xsd http://www.sat.gob.mx/Pagos20 http://www.sat.gob.mx/sitio_internet/cfd/Pagos/Pagos20.xsd\" ";
			
			}
			else
			{
				$strArchivo.= "xsi:schemaLocation=\"http://www.sat.gob.mx/cfd/4 http://www.sat.gob.mx/sitio_internet/cfd/4/cfdv40.xsd\" ";
			}//Cierre de verificación del tipo de referencia

			$strArchivo.= "Version=\"4.0\" ";
			...
			$strArchivo.= "Exportacion=\"". $otdMovimiento->CodigoExportacion."\" ";
			//Si existe método de pago
			if ($otdMovimiento->MetodoPago != "")
			{
			...
			$strArchivo.= "DomicilioFiscalReceptor=\"".$otdMovimiento->codigo_postal."\" ";
			$strArchivo.= "RegimenFiscalReceptor=\"".$this->validaParaXML($otdMovimiento->RegimenFiscal)."\" ";
			...
			if ($strTipoReferencia != "PAGO")
			{
				...
				for ($intConIVA = 0; $intConIVA < $numEleIVA; $intConIVA++)
				{
					...
					$strArchivo.= "Base=\"".$numSubtotalDesc."\" ";
			...
			/*$strNombreArchivoPrueba = $strRuta.'pruebaRefaccionesHOY_'.$otdMovimiento->folio.'.xml';
	...
	public function get_pdf($intReferenciaID = NULL , $strTipoReferencia = NULL, $strTimbrar = NULL)
	{	
		...
		if($intReferenciaID == NULL)
		{
			...
			$intReferenciaID = $this->input->post('intReferenciaID');
			$strTipoReferencia = $this->input->post('strTipoReferencia');
			$strTimbrar = $this->input->post('strTimbrar');
		}
		...
		{	
			...
			//Régimen fiscal
			$pdf->SetXY(15, 56);
			$pdf->ClippedCell(22, 03, utf8_decode('RÉGIMEN FISCAL'));
			...
			//Código postal
			$pdf->SetXY(90, 46);
			$pdf->ClippedCell(30, 3, $otdMovimiento->codigo_postal);
			...
			//Régimen fiscal
			$pdf->SetXY(15, 59);
			$pdf->ClippedCell(92, 3, $otdMovimiento->regimen_fiscal);

			//Uso de CFDI
			$pdf->SetXY(15, 66);
			$pdf->MultiCell(92, 3, utf8_decode($otdMovimiento->uso_cfdi));

			//Asignar la posición del uso de CFDI
		    $intPosYUcfdi = $pdf->GetY();
			...
			if($otdMovimiento->exportacion != "")
			{
				//Exportación
				//Asigna el tipo y tamaño de letra
				$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
				$pdf->SetXY(108, $intPosY);
				$pdf->ClippedCell(32, 3, utf8_decode('EXPORTACIÓN'));
				//Asigna el tipo y tamaño de letra
				$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NORMAL, TAMANO_LETRA_TITULO_TABLA_PDF);
				$pdf->SetXY(140, $intPosY);
				$pdf->ClippedCell(60, 3, utf8_decode($otdMovimiento->exportacion));
			}
			...
			if ($strTipoReferencia == "PAGO")
			{
				...
				$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
				$pdf->SetXY(108, $intPosY);
				$pdf->ClippedCell(32, 3, utf8_decode('OBJETO DE IMPUESTO'));
				//Asigna el tipo y tamaño de letra
				$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NORMAL, TAMANO_LETRA_TITULO_TABLA_PDF);
				$pdf->SetXY(140, $intPosY);
				$pdf->ClippedCell(60, 3, utf8_decode($otdMovimiento->objeto_impuesto));
			...
			if($otdDetalles)
			{
				...
				{ 
					...
					//Si existe objeto de impuesto
					if($arrDet->objeto_impuesto != "")
					{
						$pdf->SetX(15);
						//Asigna el tipo y tamaño de letra
				        $pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, 5);
						//Objeto de impuesto
						$pdf->Cell(22, 4, 'OBJETO DE IMPUESTO:', 0, 0, 'L', 0);
						//Asigna el tipo y tamaño de letra
				        $pdf->SetFont(TIPO_LETRA_PDF, LETRA_NORMAL, 5);
					    $pdf->ClippedCell(50, 4, utf8_decode($arrDet->objeto_impuesto), 0, 0, 'L', 0);
							...
							{  
								...
								foreach ($otdDetallesRelPago as $arrDetRP)
								{
									...
									if($arrDetRP->objeto_impuesto != "")
									{
										...
										//Asigna el tipo y tamaño de letra
								        $pdf->SetFont(TIPO_LETRA_PDF, LETRA_NORMAL, 5);
									    $pdf->ClippedCell(50, 4, utf8_decode($arrDetRP->objeto_impuesto), 0, 0, 'L', 0);
	...
	public function get_pdf_facturas($intReferenciaID = NULL, $strTipoReferencia = NULL, $strTimbrar = NULL)
	{
		//Si no existe id de la referencia
		if($intReferenciaID == NULL)
		{

...
class Rep_resumen_auxiliares extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('contabilidad/rep_resumen_auxiliares', $arrDatos);
	}
	...
	/*Método para generar un reporte PDF con el resumen de auxiliares contables
	...
	public function get_reporte() 
	{
		...
		$pdf->strLinea1 =  utf8_decode('RESUMEN DE AUXILIARES CONTABLES ').$strTituloRangoFechas;
		...
		if($otdCuentas)
		{
			...
			$pdf->Row(array('TOTALES: ', 
								'$'.number_format($intAcumSaldoInicialCtaDeudora, 2), 
								'$'.number_format($intAcumSaldoInicialCtaAcreedora, 2), 
								'$'.number_format($intTotalCargos,2),
					           	'$'.number_format($intTotalAbonos,2),
					           	'$'.number_format($intAcumSaldoActualCtaDeudora,2),
					           	'$'.number_format($intAcumSaldoActualCtaAcreedora,2)), 
        ...
        $pdf->Output('resumen_auxiliares_contables_'.$strTipoReporte.'.pdf','I'); 

	}


	/*Método para generar un archivo XLS con el resumen de auxiliares contables
	 *dependiendo de los criterios de búsqueda proporcionados*/
	public function get_xls() 
	{
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'RESUMEN DE AUXILIARES CONTABLES '.$strTituloRangoFechas);
		...
		if($otdCuentas)
		{
			...
			foreach ($otdCuentas as $arrCta)
			{
			   ...
			   //Dependiendo de la naturaleza de la cuenta, asignar saldos
				...
				else if($strTipoReporte == 'SALDO')//Sólo cuentas con saldo
				{
					...
					//Si existe saldo de la cuenta contable
					//if (($intSaldoActual >= 1) OR ($intSaldoActual <= -1))//Validación anterio
					//Se cambia la condición porque quieren saldos negativos
					if($intSaldoActual > 0 OR  $intSaldoActual < 0)
					{
						//Asignar SI para agregar datos de la cuenta
			...
			$objExcel->getActiveSheet()
                     ...
                     ->setCellValue('C'.$intFila, $intAcumSaldoInicialCtaDeudora)
		        	 ->setCellValue('D'.$intFila, $intAcumSaldoInicialCtaAcreedora)
		        	 ->setCellValue('E'.$intFila, $intTotalCargos)
		        	 ->setCellValue('F'.$intFila, $intTotalAbonos)
		        	 ->setCellValue('G'.$intFila, $intAcumSaldoActualCtaDeudora)
		        	 ->setCellValue('H'.$intFila, $intAcumSaldoActualCtaAcreedora);
            ...
            //Cambiar alineación de las siguientes celdas
		    $objExcel->getActiveSheet()
		        	 ->getStyle('B'.$intFilaTotales.':'.'B'.$intFilaTotales)
		        	 ->getAlignment()
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'resumen_auxiliares_contables_'.$strTipoReporte.'.xls', 
        								    'resumen de auxiliares', $intFila);

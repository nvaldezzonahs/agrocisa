...
class Sat_tipo_factor extends MY_Controller {
	...
	function __construct()
	{
		...
		//Cargamos el modelo
		$this->load->model('contabilidad/sat_tipo_factor_model', 'tipos');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('contabilidad/sat_tipo_factor', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		foreach ($result['tipos_factor'] as $arrDet)
		{
		...
		$arrDatos = array('rows' => $result['tipos_factor'],
	...
	{ 
		...
		$objSatTipoFactor = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento) 
		$objSatTipoFactor->intTipoFactorID = $this->input->post('intTipoFactorID');
		$objSatTipoFactor->strDescripcion = trim($this->input->post('strDescripcion'));
		$objSatTipoFactor->strDescripcionAnterior = trim($this->input->post('strDescripcionAnterior'));
		$objSatTipoFactor->intUsuarioID = $this->session->userdata('usuario_id');
		
        //Definir las reglas de validación
		//Validar que la descripción sea única
        if (($objSatTipoFactor->intTipoFactorID == '') OR 
        	($objSatTipoFactor->strDescripcionAnterior != $objSatTipoFactor->strDescripcion))
        {
            $this->form_validation->set_rules('strDescripcion', 'descripción',
            								  'required|is_unique[sat_tipo_factor.descripcion]');
        }
        else
        {
		...
		else
		{
			...
			if (is_numeric($objSatTipoFactor->intTipoFactorID))
			{
				$bolResultado = $this->tipos->modificar($objSatTipoFactor);
			}
			else
			{ 
				$bolResultado = $this->tipos->guardar($objSatTipoFactor);
			}
	...
	{
	    //Definir las reglas de validación
		$this->form_validation->set_rules('intTipoFactorID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intTipoFactorID');
	...
	{	            
		...
		$pdf->strLinea1 = 'LISTADO DE TIPOS DE FACTORES';
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE TIPOS DE FACTORES');
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'tipos_factor.xls', 'tipos de factor', $intFila);
	}
}
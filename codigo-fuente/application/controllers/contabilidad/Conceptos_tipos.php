...
class Conceptos_tipos extends MY_Controller {
	...
	function __construct()
	{
		...
		//Cargamos el modelo
		$this->load->model('contabilidad/conceptos_tipos_model', 'tipos');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('contabilidad/conceptos_tipos', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		foreach ($result['tipos_conceptos'] as $arrDet)
		{
		...
		$arrDatos = array('rows' => $result['tipos_conceptos'],
	...
	{ 
		...
		$objConceptoTipo = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objConceptoTipo->intConceptoTipoID = $this->input->post('intConceptoTipoID');
		$objConceptoTipo->strDescripcion = mb_strtoupper(trim($this->input->post('strDescripcion')));
		$objConceptoTipo->strDescripcionAnterior = mb_strtoupper(trim($this->input->post('strDescripcionAnterior')));
		$objConceptoTipo->intUsuarioID = $this->session->userdata('usuario_id');
		
        //Definir las reglas de validación
		//Validar que la descripción sea única
        if (($objConceptoTipo->intConceptoTipoID == '') OR 
        	($objConceptoTipo->strDescripcionAnterior != $objConceptoTipo->strDescripcion))
        {
            $this->form_validation->set_rules('strDescripcion', 'descripción',
            								  'required|is_unique[conceptos_tipos.descripcion]');
        }
        else
        {
		...
		else
		{
			...
			if (is_numeric($objConceptoTipo->intConceptoTipoID))
			{
				$bolResultado = $this->tipos->modificar($objConceptoTipo);
			}
			else
			{ 
				$bolResultado = $this->tipos->guardar($objConceptoTipo);
			}
	...
	{
	    //Definir las reglas de validación
		$this->form_validation->set_rules('intConceptoTipoID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intConceptoTipoID');
	...
	{	            
		...
		$pdf->strLinea1= 'LISTADO DE TIPOS DE CONCEPTOS';
		...
		$pdf->Output('tipos_conceptos.pdf','I'); 
	}
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE TIPOS DE CONCEPTOS');
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'tipos_conceptos.xls', 'tipos', $intFila);
	}
}
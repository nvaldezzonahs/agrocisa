...
class Cuentas_presupuestos extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('contabilidad/cuentas_presupuestos', $arrDatos);
	}
	...
	{ 
        ...
        {	
    		...
    		//Definir valores del array auxiliar de información (para cada mes)
           	...
           	if ($strNuevoID == '')
           	{
            	$arrAuxiliar["cuenta_id"] = $intCuentaID;
            	$arrAuxiliar["anio"] = $strAnio;	
            ...
            array_push($arrImportes, $arrAuxiliar); 
        }   

		//Si existe un presupuesto actualizamos los datos de los registros, de lo contrario, se guarda un presupuesto nuevo
		if ($strNuevoID == '')
		{
			$bolResultado = $this->presupuestos->guardar($arrImportes);
		}
		else
		{
			$bolResultado = $this->presupuestos->modificar($intCuentaID, $strAnio, $arrImportes);
		}
	...
	public function get_datos()
	{
		...
		//Seleccionar los datos del presupuesto que coincide con los parámetros enviados
	  	$otdResultado = $this->presupuestos->buscar($intCuentaID, $strAnio);

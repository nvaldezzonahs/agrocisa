...
class Salidas_maquinaria_traspaso extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('maquinaria/salidas_maquinaria_traspaso_model', 'movimientos');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('maquinaria/salidas_maquinaria_traspaso', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		$result = $this->movimientos->filtro($this->input->post('dteFechaInicial'),
											 ...
											 $this->input->post('intSucursalDestinoID'),
	...
	public function guardar()
	{
		...
		$objSalidaTraspaso = json_decode($this->input->post('salidaTraspaso'));		
		$intMovimientoMaquinariaID = $objSalidaTraspaso->intMovimientoMaquinariaID;
		...
		{
			$bolResultado = $this->movimientos->modificar($intMovimientoMaquinariaID, $objSalidaTraspaso);
		}
		else
		{
			...
			{	
				$bolResultado = $this->movimientos->guardar($strFolio, $objSalidaTraspaso); 
	...
	{	            
		...
		$intSucursalDestinoID = $this->input->post('intSucursalDestinoID');
		...
		$otdResultado = $this->movimientos->buscar(NULL, $dteFechaInicial, $dteFechaFinal, $intSucursalDestinoID,  
		...
		$pdf->strLinea1 =  'LISTADO DE SALIDAS DE MAQUINARIA POR TRASPASO '.$strTituloRangoFechas;
		...
		{		
			...
			{ 
				...
				$pdf->Row(array($arrCol->folio, 
								$arrCol->fecha, 
								utf8_decode($arrCol->sucursalDestino),
	...
	public function get_reporte_registro() 
	{
		...
		if($otdResultado)
		{
			...
			//Sucursal
			$pdf->SetXY(27, 46);
			$pdf->ClippedCell(30, 3, utf8_decode($otdResultado->sucursalDestino));
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, 'SALIDA DE MAQUINARIA POR TRASPASO', 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto

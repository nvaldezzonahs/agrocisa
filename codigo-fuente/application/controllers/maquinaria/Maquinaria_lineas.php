...
class Maquinaria_lineas extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('maquinaria/maquinaria_lineas', $arrDatos);
	}
	...
	{ 
		...
		$objLinea->intMaquinariaLineaID = $this->input->post('intMaquinariaLineaID');
		$objLinea->strDescripcion = mb_strtoupper(trim($this->input->post('strDescripcion')));
		$objLinea->strDescripcionAnterior = mb_strtoupper(trim($this->input->post('strDescripcionAnterior')));
		$objLinea->intUsuarioID = $this->session->userdata('usuario_id');
		
        //Definir las reglas de validación
		//Validar que la descripción sea única
        if (($objLinea->intMaquinariaLineaID == '') OR 
        	($objLinea->strDescripcionAnterior != $objLinea->strDescripcion))
        {
            $this->form_validation->set_rules('strDescripcion', 'línea',
            								  'required|is_unique[maquinaria_lineas.descripcion]');
        }
        else
        {
        	$this->form_validation->set_rules('strDescripcion', 'línea', 'required');
        }
		...
		else
		{
			...
			if (is_numeric($objLinea->intMaquinariaLineaID))
			{
	...
	{
	    //Definir las reglas de validación
		$this->form_validation->set_rules('intMaquinariaLineaID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intMaquinariaLineaID');
	    	...
	    	if ($otdResultado)
			{
				...
				foreach ($otdResultado as $arrCol)
				{
		        	$arrDatos[] = array('value' => $arrCol->descripcion, 
		        						'data' => $arrCol->maquinaria_linea_id);
				}
	    	}
	...
	{	            
		...
		$pdf->strLinea1 = utf8_decode('LISTADO DE LÍNEAS DE MAQUINARIA');
		//Asigna el tipo y tamaño de letra para la cabecera de la tabla
		$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
		//Crea los titulos de la cabecera
		$pdf->arrCabecera = array(utf8_decode('LÍNEA'), 'ESTATUS');
		...
		$pdf->Output('lineas_maquinaria.pdf','I'); 
	}
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE LÍNEAS DE MAQUINARIA');
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'lineas_maquinaria.xls', 'líneas', $intFila);
	}
}
...
class Movimientos_entradas_maquinaria_traspaso extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('maquinaria/movimientos_entradas_maquinaria_traspaso', $arrDatos);
	}
	...
	public function guardar()
	{
		...
		{
			$bolResultado = $this->movimientos->modificar($intMovimientoMaquinariaID, $objEntradaTraspaso);
		}
		else
		{
	...
	public function get_reporte($dteFechaInicial, $dteFechaFinal, $intSucursalOrigenID, $strEstatus, $strDetalles, $strBusqueda = NULL) 
	{	            
		...
		$result = $this->movimientos->filtro($dteFechaInicial, $dteFechaFinal, $intSucursalOrigenID,  $strEstatus, $strBusqueda,NULL, NULL);
		...
		$pdf->strLinea1 =  'LISTADO DE MOVIMIENTOS DE ENTRADA POR TRASPASO '.$strTituloRangoFechas;
		
		//Si existe id de la sucursal destino
		if($intSucursalOrigenID > 0)
		{
			//Seleccionar los datos del mecánico que coincide con el id
			$otdSucursal =  $this->sucursales->buscar($intSucursalOrigenID, NULL, NULL);
	...
	public function get_reporte_registro($intMovimientoMaquinariaID) 
	{
		...
		$pdf->SetXY(80, 15);
		$pdf->ClippedCell(120, 3, utf8_decode('ENTRADAS DE MAQUINARIA POR TRASPASO'), 0, 0, 'C', TRUE);
		...
		{	 
			...
			//Sucursal de origen
			$pdf->SetXY(111, 23);
			$pdf->ClippedCell(60, 3, utf8_decode($otdResultado->sucursalSalida));
			//Sucursal de destino
			$pdf->SetXY(111, 27);
			$pdf->ClippedCell(60, 3, utf8_decode($otdResultado->sucursalDestino));
			//CHOFER
			$pdf->SetXY(111, 31);
	 ...
	 *dependiendo del criterio de búsqueda proporcionado*/
	public function get_xls($dteFechaInicial, $dteFechaFinal, $intSucursalOrigenID,$strEstatus, $strDetalles, $strBusqueda = NULL) 
	{	
		//Quitar espacios vacíos y decodificar cadena cifrada
		$strBusqueda = trim(urldecode($strBusqueda));
		//Quitar espacios vacíos y decodificar cadena cifrada

...
class Salidas_maquinaria_ajuste extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('maquinaria/salidas_maquinaria_ajuste_model', 'movimientos');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('maquinaria/salidas_maquinaria_ajuste', $arrDatos);
	}
	...
	public function guardar()
	{
		...
		$objSalidaAjuste = json_decode($this->input->post('salidaAjuste'));		
		$intMovimientoMaquinariaID = $objSalidaAjuste->intMovimientoMaquinariaID;
		...
		{
			$bolResultado = $this->movimientos->modificar($intMovimientoMaquinariaID, $objSalidaAjuste);
		}
		else
		{
			...
			{	
				$bolResultado = $this->movimientos->guardar($strFolio, $objSalidaAjuste); 
	...
	{	            
		...
		$result = $this->movimientos->filtro($dteFechaInicial, $dteFechaFinal, NULL, NULL);
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE MOVIMIENTOS DE SALIDA POR AJUSTE ').$strTituloRangoFechas;
		...
		$pdf->Output('salidas_maquinaria_ajuste.pdf','I'); 
	}
	...
	public function get_reporte_registro($intMovimientoMaquinariaID) 
	{
		...
		$strNombreArchivo  = 'salidas_maquinaria_ajuste_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		$pdf->SetXY(80, 15);
		$pdf->ClippedCell(120, 3, utf8_decode('SALIDAS DE MAQUINARIA POR AJUSTE'), 0, 0, 'C', TRUE);
		...
		{	 
			...
			if($otdDetalles)
			{
				...
				//Establece el ancho de las columnas de cabecera
				$arrAchura = array(15, 100, 15, 15, 15, 25);
				//Establece la alineación de las celdas de la tabla
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE SALIDAS DE MAQUINARIA POR AJUSTE'.$strTituloRangoFechas);
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'salidas_maquinaria_ajuste.xls', 'salidas maquinaria por ajuste', $intFila);
	}

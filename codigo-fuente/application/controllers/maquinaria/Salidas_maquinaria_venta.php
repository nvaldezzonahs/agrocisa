...
class Salidas_maquinaria_venta extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('maquinaria/salidas_maquinaria_venta_model', 'movimientos');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('maquinaria/salidas_maquinaria_venta', $arrDatos);
	}
	...
	public function guardar()
	{
		...
		$objSalidaVenta = json_decode($this->input->post('salidaVenta'));		
		$intMovimientoMaquinariaID = $objSalidaVenta->intMovimientoMaquinariaID;
		...
		{
			$bolResultado = $this->movimientos->modificar($intMovimientoMaquinariaID, $objSalidaVenta);
		}
		else
		{
			...
			{	
				$bolResultado = $this->movimientos->guardar($strFolio, $objSalidaVenta); 
	...
	//Método para verificar si una factura cuenta con una salida por compra ACTIVA
	public function verificar_salida_compra()
	{
		//Array que se utiliza para enviar datos a la vista
		$arrDatos = array('row' => NULL);
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intReferenciaID');
		//Seleccionar los datos del registro que coincide con el id
	    $otdResultado = $this->movimientos->verificar_salida_compra($intID);
	...
	public function get_reporte($dteFechaInicial, $dteFechaFinal, $intClienteDestinoID,$strEstatus,$strDetalles,$strBusqueda = NULL) 
	{	            
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE MOVIMIENTOS DE SALIDA POR VENTA ').$strTituloRangoFechas;
		...
		$pdf->arrCabecera = array('FOLIO', 'FECHA', 'FACTURA', 'CLIENTE', 'ESTATUS');
		//Establece el ancho de las columnas de cabecera
		$pdf->arrAnchura = array(20, 20, 20, 108, 20, 20);
		//Establece la alineación de las celdas de la tabla
		$pdf->arrAlineacion = array('L', 'C', 'L', 'L', 'C');
		//Establece la alineación de las celdas de la tabla detalles
	    $arrAlineacionDetalles = array('L', 'L', 'L', 'L');
	    //Establece el ancho de las columnas de la tabla detalles
		$arrAnchuraDetalles = array(20, 65, 20, 20);
		...
		{		
			...
			{ 
		        ...
		        $arrAuxiliar = array();
				//Seleccionar los detalles del registro
				$otdFactura = $this->facturas->buscar($arrCol->referencia_id);
				//Seleccionar detalles de la factura
				$otdFacturaDetalles = $this->facturas->buscar_detalles($arrCol->referencia_id);
				
				//Verificar si existe información de los detalles 
				//Se agrega la informacion al reporte... se utiliza utf8 para acentos y tildes
				if($otdFactura){
					//Si la maquinaria contiene componentes agregarlos al reporte
					if($otdFacturaDetalles){
						//Recorremos el arreglo 
						foreach ($otdFacturaDetalles as $arrDet)
						{
							//Definir valores del array auxiliar de información (para cada detalle)
							$arrAuxiliar["codigo"] = $arrDet->componente_codigo;
							$arrAuxiliar["descripcion_corta"] = utf8_decode($arrDet->componente_descripcion_corta);
			                $arrAuxiliar["serie"] = $arrDet->componente_serie;
			                $arrAuxiliar["motor"] = $arrDet->componente_motor;
				...
				if($arrDetalles && $strDetalles == 'SI')
				{
					...
					$pdf->Row(array(utf8_decode($otdFactura->codigo), 
								utf8_decode($otdFactura->descripcion_corta),  
								utf8_decode($otdFactura->serie),
								utf8_decode($otdFactura->motor)), 
			        ...
			        foreach ($arrDetalles as $arrDet) 
			        {
					   //Se agrega la informacion al reporte... se utiliza utf8 para acentos y tildes
					    $pdf->Row(array(
					    				...
					    				$arrAnchuraDetalles,  'ClippedCell');
					}
		...
		$pdf->Output('salidas_maquinaria_venta.pdf','I'); 
	}
	...
	public function get_reporte_registro($intMovimientoMaquinariaID) 
	{
		...
		$id_factura = $otdResultado->referencia_id;
		$otdFactura = $this->facturas->buscar($id_factura);
		//Seleccionar detalles de la factura
		$otdFacturaDetalles = $this->facturas->buscar_detalles($id_factura);
		...
		$strNombreArchivo  = 'salidas_maquinaria_venta_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		$pdf->SetXY(80, 15);
		$pdf->ClippedCell(120, 3, utf8_decode('SALIDAS DE MAQUINARIA POR VENTA'), 0, 0, 'C', TRUE);
		...
		//FACTURA DE VENTA
		$pdf->SetXY(80, 23);
		$pdf->ClippedCell(35, 3, utf8_decode('FACTURA'));
		//CLIENTE
		$pdf->SetXY(80, 27);
		$pdf->ClippedCell(35, 3, 'CLIENTE');

		//Información de la salida de maquinaria por venta
		...
		{	 
			...
			//------------------------------------------------------------------------------------------------------------------------
	        //---------- DETALLES DE LA SALIDA DE MAQUINARIA POR VENTA
	        //------------------------------------------------------------------------------------------------------------------------	
			$pdf->SetTextColor(COLOR_TEXTO); //establece el color de texto
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			...
			if($otdFactura)
			{
				...
				//Crea los titulos de la cabecera
				$arrCabecera = array(utf8_decode('Código'), utf8_decode('Descripción'), utf8_decode('Serie'), utf8_decode('Motor'));
				//Establece el ancho de las columnas de cabecera
				$arrAnchura = array(15, 100, 35, 35);
				//Establece la alineación de las celdas de la tabla
				...
				if($otdFactura){
					...
					if($otdFacturaDetalles){
						...
						{ 
							$pdf->SetX(15);
							//Se agrega la informacion al reporte... se utiliza utf8 para acentos y tildes
							$pdf->Row(array(utf8_decode($arrDet->componente_codigo), 
											utf8_decode($arrDet->componente_descripcion_corta),  
										    utf8_decode($arrDet->componente_serie),
										    utf8_decode($arrDet->componente_motor)),	
											$arrAlineacion,'ClippedCell'
									  );
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE SALIDAS DE MAQUINARIA POR VENTA '.$strTituloRangoFechas);
        ...
        if($strDetalles == 'SI')
        {
        	...
        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('F'.$intPosEncabezados.':L'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
			...
			{   
				...
				if($strDetalles == 'SI')
				{
					...
					if($otdDetalles)
					{
						...
						foreach ($otdDetalles as $arrDet)
						{
			                ...
			                $arrDetalles[$intContMov]["maquinaria_linea"] = $arrDet->descripcion_corta;
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValueExplicit('C'.$intFila, $arrCol->factura, PHPExcel_Cell_DataType::TYPE_STRING)
		                         ->setCellValueExplicit('D'.$intFila, $arrCol->cliente, PHPExcel_Cell_DataType::TYPE_STRING)
		                         ->setCellValueExplicit('E'.$intFila, $arrCol->estatus, PHPExcel_Cell_DataType::TYPE_STRING);
					...
					if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValue('I'.$intFila, $arrDetalles[$intContDet]['descripcion_corta'])
		                         ->setCellValue('J'.$intFila, $arrDetalles[$intContDet]['descripcion_corta'])
       ...
       $this->get_pie_pagina_archivo_excel($objExcel, 'salidas_maquinaria_venta.xls', 'salidas maquinaria venta', $intFila);
	}

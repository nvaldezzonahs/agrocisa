...
class Rep_comisiones extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('servicio/rep_comisiones', $arrRegistro);
	}
	...
	/*Método para generar un reporte PDF con las comisiones de los servicios
	...
	{	
		...
		$otdResultado = $this->ordenes->buscar_comisiones($dteFechaInicial, $dteFechaFinal); 
		...
		$pdf->strLinea1 = 'REPORTE DE COMISIONES DE SERVICIO';
		...
		{	
			...
			{ 
				...
				if (($arrCol->AbonoParcial > 0) && (($arrCol->Total - $arrCol->AbonoTotal) <= 0.1))
				{
					...
					if ($arrCol->sucursal_id != $intSucursalIDActual)
					{
						...
						if ($intSucursalIDActual > 0)
						{
							...
							//Seleccionar los abonos que tiene la sucursal
		               	 	$otdAbonos = $this->ordenes->buscar_abonos_sucursal($dteFechaInicial, $dteFechaFinal, 
		               	 														$intSucursalIDActual);
							...
							//Asignar datos al array
		               	 	...
		               	 	$this->get_totales($pdf, 'PDF', $arrAcumulados, $otdAbonos, $arrAlineacion2, $arrAnchura2);
					...
					if ($arrCol->mecanico_id != $intMecanicoIDActual)
					{
						...
						if ($intMecanicoIDActual > 0 && $intSucursalIDActual == $arrCol->sucursal_id)
						{
							...
							//Asignar datos al array
		               	 	array_push($arrAcumulados, $arrAuxiliar); 
		               	 	//Hacer un llamado a la función para agregar totales y subtotales
		               	 	$this->get_totales($pdf, 'PDF', $arrAcumulados, NULL, $arrAlineacion2, $arrAnchura2);
		...
		$pdf->Output('comisiones_servicios.pdf','I'); 
	}




	/*Método para generar un archivo XLS con las comisiones de los servicios
	...
	public function get_xls() 
	{
					...
					if ($arrCol->sucursal_id != $intSucursalIDActual)
					{
						...
						if ($intSucursalIDActual > 0)
						{
							...
							//Asignar datos al array
		               	 	array_push($arrAcumulados, $arrAuxiliar); 
		               	 	//Hacer un llamado a la función para agregar totales y subtotales
		               	  	$this->get_totales($objExcel, 'EXCEL', $arrAcumulados, $otdAbonos, NULL, NULL, 
		               	 					   $intFila);
						...
						$objExcel->getActiveSheet()->setCellValue('A7', 'REPORTE DE COMISIONES DE SERVICIO ');
					...
					if ($arrCol->mecanico_id != $intMecanicoIDActual)
					{
						...
						if ($intMecanicoIDActual > 0 && $intSucursalIDActual == $arrCol->sucursal_id)
						{
							...
							//Asignar datos al array
		               	 	array_push($arrAcumulados, $arrAuxiliar); 
		               	 	//Hacer un llamado a la función para agregar totales y subtotales
		               	 	$this->get_totales($objExcel, 'EXCEL', $arrAcumulados, NULL, NULL, NULL, 
		               	 					   $intFila);
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'comisiones_servicios.xls', 'comisiones', $intNumMaxRegistros); 
		
	}


	//Función que se utiliza para regresar los subtotales y totales de los servicios de un mecánico
	public function get_totales($lib, $strTipoArchivo, $arrAcumulados, $otdAbonos = NULL,
								...
								$intFilaExcel = NULL)
	{
		...
		//Establece el ancho de las columnas de los abonos recuperados
		$arrAnchuraAbonosRec = array(28, 21);
		...
		//Si existe información de los abonos
		if($otdAbonos)
		{
			//Recorremos el arreglo 
		...
		foreach ($arrAcumulados as $arrAcum)
		{
			...
			if($strTipoArchivo == 'PDF')
			{
				...
				//Se agrega la información del total
		   		...
		   		//Si existe objeto con los abonos de la sucursal
		   		if($otdAbonos != NULL)
		   		{
		   			//Establece el ancho de las columnas
					$lib->SetWidths($arrAnchuraAbonosRec);
		   		
		   			//Se agrega la información de los abonos recuperados
					$lib->Row(array('TOTAL RECUPERADO:', '$'.number_format($intAcumAbonos, 2, '.', ',')), 
							    $arrAlineacionTotales, 'ClippedCell');
			...
			else //Si el tipo de archivo es Excel
			{
				...
				//Asignar el indice de la fila para escribir el total
				$intFilaTotal = $intFilaExcel;

				//Agregar información del total
				$lib->getActiveSheet()
                         ->setCellValue('A'.$intFilaTotal, 'TOTAL:')
                         ->setCellValue('C'.$intFilaTotal, $intTotal);

                //Incrementar el indice para escribir los datos del abono recuperado
				$intFilaExcel++;

                //Asignar el indice de la fila para escribir el total de abonos recuperados
				$intFilaRecuperado = $intFilaExcel;

                //Si existe objeto con los abonos de la sucursal
		   		if($otdAbonos != NULL)
		   		{
		   			//Agregar información de los abonos recuperados
					$lib->getActiveSheet()
	                     ->setCellValue('A'.$intFilaRecuperado, 'TOTAL RECUPERADO:')
	                     ->setCellValue('C'.$intFilaRecuperado, $intAcumAbonos);
	            ...
	            $lib->getActiveSheet()
	        		->getStyle('C'.$intFilaTotal)
	        		->getNumberFormat()
	        		->setFormatCode('$#,##0.00');  

	        	$lib->getActiveSheet()
	        		->getStyle('C'.$intFilaRecuperado)
	        		->getNumberFormat()
	        		->setFormatCode('$#,##0.00'); 
	            ...
	            $lib->getActiveSheet()
	                ->getStyle('A'.$intFilaTotal.':C'.$intFilaTotal)
	                ->applyFromArray($arrStyleBold);

	            $lib->getActiveSheet()
	                ->getStyle('A'.$intFilaRecuperado.':C'.$intFilaRecuperado)
	                ->applyFromArray($arrStyleBold);

...
class Trabajos_foraneos extends MY_Controller {
	...
	function __construct()
	{
		...
		//Cargamos el modelo de trabajos foráneos
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('servicio/trabajos_foraneos', $arrDatos);
	}
	...
	{ 
		...
		$objTrabajoForaneo->intTrabajoForaneoID = $this->input->post('intTrabajoForaneoID');
		...
		$objTrabajoForaneo->strFactura = mb_strtoupper($this->input->post('strFactura'));
		...
		$objTrabajoForaneo->intOrdenReparacionID = $this->input->post('intOrdenReparacionID');
		$objTrabajoForaneo->strObservaciones = mb_strtoupper($this->input->post('strObservaciones'));
		...
		$objTrabajoForaneo->strProductoServicioID = $this->input->post('strProductoServicioID');
		$objTrabajoForaneo->strUnidadID = $this->input->post('strUnidadID');
		$objTrabajoForaneo->strObjetoImpuestoID = $this->input->post('strObjetoImpuestoID');
		...
		$objTrabajoForaneo->strPreciosUnitarios = $this->input->post('strPreciosUnitarios');
		...
		if (is_numeric($objTrabajoForaneo->intTrabajoForaneoID))
		{
		...
		{ 
			...
			{	
				...
				/*Quitar '_'  de la cadena (resultadoTransaccion_trabajoForaneoID) para obtener el resultado de la transacción del movimiento en la BD y el id del registro 
				 */
			    list($bolResultado, $objTrabajoForaneo->intTrabajoForaneoID) = explode("_", $bolResultado); 
		...
		if($bolResultado !== NULL)
		{
			...
			if($bolResultado)
			{
				...
				$arrDatos = array('resultado' => TRUE,
							 	  ...
							 	  'trabajo_foraneo_id' => $objTrabajoForaneo->intTrabajoForaneoID,
							      'mensaje' => MSJ_GUARDAR);
			}
			else
			{
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intTrabajoForaneoID');
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intTrabajoForaneoID', 'ID', 'required|integer');
	...
	{	            
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE TRABAJOS FORÁNEOS ').$strTituloRangoFechas;
		...
		{	
			...
			{ 
				...
				$otdDetalles = $this->trabajos->buscar_detalles($arrCol->trabajo_foraneo_id);
				...
				//Prospecto
				$pdf->Cell(13, 4, utf8_decode('CLIENTE:'), 0, 0, 'L', 0);
				$pdf->ClippedCell(45, 4, utf8_decode($arrCol->prospecto), 0, 0, 'L', 0);
		...
		$pdf->Output('trabajos_foraneos.pdf','I'); 
	}
	...
	{	
		//Variables que se utilizan para recuperar los valores de la vista
		$intTrabajoForaneoID = $this->input->post('intTrabajoForaneoID');     
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->trabajos->buscar($intTrabajoForaneoID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->trabajos->buscar_detalles($intTrabajoForaneoID);
		//Seleccionar los datos de las tasa de IEPS de los detalles del registro
		$otdTasasIeps = $this->trabajos->buscar_tasas_ieps_detalles($intTrabajoForaneoID);
		...
		$strNombreArchivo  = 'trabajo_foraneo_';
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('TRABAJO FORÁNEO'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
			...
			//Cliente
			$pdf->SetXY(108, 55);
			$pdf->ClippedCell(32, 3, utf8_decode('CLIENTE'));
			//Orden de compra
			$pdf->SetXY(108, 58);
			...
			$pdf->SetXY(135, 55);
			$pdf->ClippedCell(60, 3, utf8_decode($strCliente));
			//Orden de compra
			$pdf->SetXY(135, 58);
			...
			if($otdDetalles)
			{
				...
				$arrCabecera = array(utf8_decode('Descripción'), 'Cantidad',  
									'Costo', 'Costo Ext.', 'Subtotal', 'Precio');
				//Establece el ancho de las columnas de cabecera
				$arrAnchura = array(70, 27, 28, 20, 20, 20);
				//Establece la alineación de las celdas de la tabla
				$arrAlineacion = array('L', 'R', 'R', 'R', 'R', 'R');
				...
				{ 
					...
					/***Cálculos correspondientes al precio unitario***/
					//Calcular precio
					$intPrecio = number_format($intCantidad, 2, '.', '') * $intPrecioUnitario;

					//Calcular importe de IVA
				    $intImporteIvaPrecio = $intPrecio * $intPorcentajeIva;
					...
					if($intTasaCuotaIeps > 0)
					{
						...
						{
							//Calcular importe de IEPS
							$intImporteIepsPrecio = $intPrecio * $intPorcentajeIeps;
						}
					}


					//Incrementar acumulados
					$intAcumSubtotalPrecio += $intPrecio;
					$intAcumIvaPrecio += $intImporteIvaPrecio;
					$intAcumIepsPrecio += $intImporteIepsPrecio;
					...
					$pdf->Row(array(utf8_decode($arrDet->concepto), 
									...
									number_format($intPrecio,$this->intNumDecimales)), 
				...
				//Subtotal del precio
				$pdf->SetX(180);
				$pdf->ClippedCell(20, 3, '$'.number_format($intAcumSubtotalPrecio,$this->intNumDecimales), 0, 0, 'R');
				$pdf->Ln(); //Deja un salto de línea
				$intPosYObs = $pdf->GetY();
				...
				//IVA del precio
				$pdf->SetX(180);
				$pdf->ClippedCell(20, 3, '$'.number_format($intAcumIvaPrecio,$this->intNumDecimales), 0, 0, 'R');
				$pdf->Ln(); //Deja un salto de línea
				...
				//IEPS del precio
				$pdf->SetX(180);
				$pdf->ClippedCell(20, 3, '$'.number_format($intAcumIepsPrecio,$this->intNumDecimales), 0, 0, 'R');
				$pdf->Ln(); //Deja un salto de línea
				...
				//Total del precio
				$pdf->SetX(180);
				$pdf->ClippedCell(20, 3, '$'.number_format($intTotalPrecio,$this->intNumDecimales), 0, 0, 'R');
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE TRABAJOS FORÁNEOS '.$strTituloRangoFechas);
		//Si existe id del proveedor
		if($intProveedorID > 0)
		{   //Seleccionar los datos del proveedor que coincide con el id
			$otdProveedor =  $this->proveedores->buscar($intProveedorID);
        ...
        $objExcel->setActiveSheetIndex(0)
        		 ...
        		 ->setCellValue('J'.$intPosEncabezados, 'CLIENTE')
        		 ->setCellValue('K'.$intPosEncabezados, 'ORDEN DE COMPRA')
        ...
        if($strDetalles == 'SI')
        {
        	 //Combinar las siguientes celdas
	       	$objExcel->setActiveSheetIndex(0)
                     ...
                     ->setCellValue('R'.$intPosEncabezados, 'CODIGO SAT')
                     ->setCellValue('S'.$intPosEncabezados, 'UNIDAD SAT')
			         ->setCellValue('T'.$intPosEncabezados,'COSTO UNITARIO')
			         ->setCellValue('U'.$intPosEncabezados,'PRECIO UNITARIO')
			         ->setCellValue('V'.$intPosEncabezados, 'SUBTOTAL');

        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('P'.$intPosEncabezados.':V'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
			...
			{   
				...
				if($otdDetalles)
				{
					...
					foreach ($otdDetalles as $arrDet)
					{
			        	...
			        	$arrDetalles[$intContDetTrab]['producto_servicio'] = $arrDet->producto_servicio;
			        	$arrDetalles[$intContDetTrab]['unidad']= $arrDet->unidad;
			    ...
			    { 
					...
					$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValue('J'.$intFila, $arrCol->prospecto)
		                         ->setCellValueExplicit('K'.$intFila, $arrCol->folio_orden_compra, PHPExcel_Cell_DataType::TYPE_STRING)
						...
						if($arrDetalles && $strDetalles == 'SI')
						{
							...
							$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValue('R'.$intFila,  $arrDetalles[$intContDet]['producto_servicio'])
		                         ->setCellValue('S'.$intFila,  $arrDetalles[$intContDet]['unidad'])
						         ->setCellValue('T'.$intFila,  $arrDetalles[$intContDet]['costo_unitario'])
						         ->setCellValue('U'.$intFila,  $arrDetalles[$intContDet]['precio_unitario'])
						         ->setCellValue('V'.$intFila,  $arrDetalles[$intContDet]['subtotal']);
						}
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'trabajos_foraneos.xls', 'trabajos foráneos', 
        									$intFila);
	}
	...
	*********************************************************************************************************************/
	...
	public function get_paginacion_detalles()
	{
		...
		$config['cur_page'] =  $this->input->post('intPagina');
		//Seleccionar los datos que coinciden con los criterios de búsqueda
		$result = $this->trabajos->filtro_detalles($this->input->post('intOrdenReparacionID'),
								                   $config['per_page'],
								                   $config['cur_page']);
		...
		foreach ($result["registros"] as $arrDet) 
		{
			...
			if($arrDet->estatus == 'ACTIVO')
			{
				...
				//Calcular importe de IVA
				$intImporteIva = $intSubTotal * $intPorcentajeIva;
				...
				if($intTasaCuotaIeps > 0)
				{
					...
					{
						//Calcular importe de IEPS
						$intImporteIeps = $intSubTotal * $intPorcentajeIeps;
					}
				}

...
class Ordenes_compra_servicio extends MY_Controller {
	...
	function __construct()
	{
	    ...
	    $this->archivo['strCarpetaDestino'] = './archivos/ordenes_compra_servicio/';
	    //Asignar ruta de la carpeta temporal
	    $this->archivo['strCarpetaTemporal'] = './orden_compra_servicio_temporal_';
		//Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Cargamos el modelo de ordenes de compra
		$this->load->model('servicio/ordenes_compra_servicio_model', 'ordenes');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('servicio/ordenes_compra_servicio', $arrDatos);
	}
	...
	*********************************************************************************************************************/
	...
	public function get_paginacion()
	{
		...
		foreach ($result['ordenes'] as $arrDet)
		{
            ...
            $intOrdenCompraServicioID = $arrDet->orden_compra_servicio_id;
            //Concatenar id del registro que hace referencia a la carpeta donde se encuentra el archivo
			$strNombreCarpeta = $this->archivo['strCarpetaDestino'].$intOrdenCompraServicioID;

            //Asignar el nombre del archivo que le corresponde al registro
		    $strNombreArchivo = $this->get_verifar_archivo_registro($strNombreCarpeta, $intOrdenCompraServicioID);
	...
	{ 
		...
		$objOrdenCompra->intOrdenCompraServicioID = $this->input->post('intOrdenCompraServicioID');
		...
		$objOrdenCompra->intOrdenReparacionID = $this->input->post('intOrdenReparacionID');
		...
		if (is_numeric($objOrdenCompra->intOrdenCompraServicioID))
		{
		...
		{ 
			...
			else
			{
				 ...
				 */
			    list($bolResultado, $objOrdenCompra->intOrdenCompraServicioID, $objOrdenCompra->strFolioConsecutivo) = explode("_", $bolResultado); 
			}
		}
		...
		if($bolResultado !== NULL)
		{
			...
			if($bolResultado)
			{
				...
				$arrDatos = array('resultado' => TRUE,
							 	  ...
							 	  'orden_compra_servicio_id' => $objOrdenCompra->intOrdenCompraServicioID,
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intOrdenCompraServicioID');
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intOrdenCompraServicioID', 'ID', 'required|integer');
	...
	//Método para regresar todos los registros autorizados en un autocomplete
	public function autocomplete()
	{
		...
		if(isset($strDescripcion))
		{
			...
			$otdResultado = $this->ordenes->autocomplete($strDescripcion, $intOrdenReparacionID);
	    	...
	    	if ($otdResultado)
			{
				...
				{	
					...
					$arrDatos[] = array('value' => $arrCol->orden_compra_servicio, 
	        					        'data' => $arrCol->orden_compra_servicio_id);
				}
	    	}
	...
	{
		//Variables que se utilizan para recuperar los valores de la vista 
		$intOrdenCompraServicioID = $_POST["intOrdenCompraServicioID_ordenes_compra_servicio_servicio"];
		$strBotonArchivoID = $_FILES["archivo_varios_ordenes_compra_servicio_servicio"];
		...
		$this->subir_archivo_reg($strBotonArchivoID, $this->archivo['strCarpetaPrincipal'], 
							     ...
							     $strNombreCarpeta, $intOrdenCompraServicioID, TRUE);
	}
	...
	{
		//Variables que se utilizan para recuperar los valores de la vista
		$intOrdenCompraServicioID = $this->input->post('intOrdenCompraServicioID');
		$strFolio = $this->input->post('strFolio');

		//Definir ubicación de la carpeta
		$strNombreCarpeta = $this->archivo['strCarpetaDestino'].$intOrdenCompraServicioID;
		//Asignar nombre de la carpeta zip
		$strNombreCarpetaZIP = 'OCS_'.$strFolio.'.zip';
	...
	public function enviar_correo_electronico_proveedor()
	{
        ...
        $strRuta = $this->get_reporte_registro($intOrdenCompraServicioID, 'SI');

        //Array que se utiliza para enviar correo electrónico
		$arrDatos =  array('intReferenciaID' => $intOrdenCompraServicioID,
						   'strTitulo' => utf8_decode('Solicitud de Orden de Compra Servicio'),
						   'strRuta' => $strRuta,
	...
	public function eliminar_carpeta_temporal($intOrdenCompraServicioID)
	{
		//Definir ubicación de la carpeta
		$strNombreCarpeta = $this->archivo['strCarpetaTemporal'].$intOrdenCompraServicioID; 
	...
	{	            
		...
		$pdf->strLinea1 =  'LISTADO DE ORDENES DE COMPRA SERVICIO'.$strTituloRangoFechas;
		...
		{	
			...
			{ 
				...
				//Seleccionar los detalles del registro
				$otdDetalles = $this->ordenes->buscar_detalles($arrCol->orden_compra_servicio_id);
				...
				//Orden de reparación
				$pdf->Cell(18, 4, utf8_decode('ORDEN DE REP.:'), 0, 0, 'L', 0);
				$pdf->ClippedCell(15, 4, utf8_decode($arrCol->folio_orden_reparacion), 0, 0, 'L', 0);
				//Prospecto
				$pdf->Cell(10, 4, utf8_decode('CLIENTE:'), 0, 0, 'L', 0);
				$pdf->ClippedCell(45, 4, utf8_decode($arrCol->prospecto_rep), 0, 0, 'L', 0);
				//Moneda
				$pdf->Cell(12, 4, 'MONEDA:', 0, 0, 'L', 0);
			    $pdf->ClippedCell(7, 4, utf8_decode($arrCol->codigo_moneda), 0, 0, 'L', 0);
		    	//Tipo de cambio
		    	$pdf->Cell(6, 4, 'T.C.:', 0, 0, 'L', 0);
			    $pdf->ClippedCell(11, 4,'$'.number_format($arrCol->tipo_cambio, 4, '.', ','), 0, 0, 'R', 0);
				//Fecha de entrega
		...
		$pdf->Output('ordenes_compra_servicio.pdf','I'); 
	}

	//Método para generar un reporte PDF con la información de un registro 
	public function get_reporte_registro($intOrdenCompraServicioID  = NULL, $strEnviarCorreo = NULL) 
	{	  
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->ordenes->buscar($intOrdenCompraServicioID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->ordenes->buscar_detalles($intOrdenCompraServicioID);
		...
		$strNombreArchivo  = 'orden_compra_servicio_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, 'ORDEN DE COMPRA SERVICIO', 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
			...
			//Cliente
			$pdf->SetXY(135, 58);
			$pdf->ClippedCell(60, 3, utf8_decode($otdResultado->prospecto_rep));
			//Condiciones de pago
			$pdf->SetXY(140, 61);
			...
			if($otdDetalles)
			{
				...
				$pdf->SetXY(15, 74);
				//Crea los titulos de la cabecera
				$arrCabecera = array('Cantidad', utf8_decode('Descripción'), 'Unitario', 
									  'Descuento', 'Importe');
				//Establece el ancho de las columnas de cabecera
		...
		{	
			//Definir ubicación de la carpeta
			$strCarpetaDestino =  $this->archivo['strCarpetaTemporal'].$intOrdenCompraServicioID.'/'; 
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE ORDENES DE COMPRA SERVICIO '.$strTituloRangoFechas);
		//Si existe id del proveedor
		if($intProveedorID > 0)
		{   //Seleccionar los datos del proveedor que coincide con el id
			$otdProveedor =  $this->proveedores->buscar($intProveedorID);
			...
			{   
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					$objExcel->setActiveSheetIndex(0)
		                     ...
		                     ->setCellValue('J'.$intFila, $arrCol->prospecto_rep)
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'ordenes_compra_servicio.xls', 'ordenes de compra', $intFila);
	}

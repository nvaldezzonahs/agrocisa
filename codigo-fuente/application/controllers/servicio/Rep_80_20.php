...
class Rep_80_20 extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('servicio/rep_80_20_model', 'ventas');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('servicio/rep_80_20', $arrDatos);
	}
	...
	public function get_reporte()
	{
		...
		{
			//Hacer un llamado a la función para generar reporte de servicio
			$this->get_reporte_servicio($strSucursales, $dteFechaInicial, $dteFechaFinal);
		}
	}
	...
	{	        
		...
		$pdf->arrAlineacion = array('L', 'L', 'L', 'L', 'R', 'R', 'R', 'R', 'R', 'R', 'R');
	...
	/*Método para generar un reporte PDF con el listado de SERVICIOS dependiendo de los criterios de búsqueda proporcionados*/
	public function get_reporte_servicio($strSucursales, $dteFechaInicial, $dteFechaFinal) 
	{	        
		//Variable que se utiliza pra asignar el servicio actual
		$strServicioActual = '';
        //Variable que se utiliza para definir título de rango de fechas
        $strTituloRangoFechas = '';
		//Variable que se utiliza para contar el número de registros
		...
		//Variable para asignar el resultado
		$otdVentas = $this->ventas->ventas_servicio($dteFechaInicial, $dteFechaFinal, $strSucursales);
		//Se crea una instancia de la clase PDF
		$pdf = new PDF();//orientación horizontal

		//Asignar el valor de la descripción (título de la lista de registros) del reporte dependiendo del tipo de reporte
		$pdf->strLinea1 = 'VENTAS DE SERVICIOS 80-20';
		//Asignar el valor de la línea dos del título
		$pdf->strLinea2 =  'LISTADO DE MOVIMIENTOS '.$strTituloRangoFechas;
		//Asignar el valor de la línea tres del título
		$pdf->strLinea3 = utf8_decode('SUCURSALES: '.trim($strTituloSucursales));
		...
		$pdf->intSucursalID = $this->session->userdata('sucursal_id');
		//Crea los titulos de la cabecera
		$pdf->arrCabecera = array('SERVICIO',  'SUBTOTAL', 'IVA', 'IEPS', 'TOTAL', 'NO. FACT.', '%PART.', '%ACUM.');
		//Establece el ancho de las columnas de cabecera
		...
		{	
			...
			foreach ($otdVentas as $arrCol)
			{
				...
				//Si es el primer registro y no existe id del cliente
      			if($intContador === 0 && $arrCol->servicio === NULL)
      			{
					//Asignar maquinaria actual
      				$strServicioActual = $arrCol->servicio;
      			}

				//Si la maquinaria actual es igual a '' (primera maquinaria)
	      		if ($strServicioActual === '' && $arrCol->servicio != '')
	      		{
				...
				//Si la maquinaria actual es diferente al anterior
	      		if($strServicioActual != $arrCol->servicio && $arrCol->servicio !== NULL)
	      		{
	      			//Limpiar las siguientes variables (por cada maquinaria recorrida)
      				$strServicioActual = $arrCol->servicio;
	      		}
				...
				$pdf->SetTextColor(0); //Establecer el color de texto por defecto
				//Se agrega la informacion al reporte... se utiliza utf8 para acentos y tildes
                $pdf->Row(array(utf8_decode($arrCol->servicio),
                				'$'.number_format($arrCol->subtotal,2),
                				'$'.number_format($arrCol->iva, 2),
                				'$'.number_format($arrCol->ieps, 2),
                				'$'.number_format($arrCol->total, 2),
	...
	/*Método para generar un archivo XLS con el listado de los registros dependiendo del criterio de búsqueda proporcionado*/
   	public function get_xls()
   	{
		...
		{
			//Hacer un llamado a la función para generar reporte de servicio
			$this->get_xls_servicio($dteFechaInicial, $dteFechaFinal, $strSucursales);
		}
   	}
	...
	/*Método para generar un archivo XLS solo de SERVICIO con el listado de los registros dependiendo del criterio de búsqueda proporcionado*/
	public function get_xls_servicio($dteFechaInicial, $dteFechaFinal, $strSucursales) 
	{	
		//Variable que se utiliza para asignar título del rango de fechas
		$strTituloRangoFechas = '';
		//Variable para modificiar el titulo del reporte
		...
		//Se agrega el título del archivo
		$objExcel->setActiveSheetIndex(0)
				 ->setCellValue('A7', 'VENTAS DE SERVICIOS 80-20')
				 ->setCellValue('A8', 'LISTADO DE MOVIMIENTOS '.$strTituloRangoFechas)
				 ->setCellValue('A9', 'SUCURSALES: '.$strTituloSucursales);
     	...
     	//Se agregan las columnas de cabecera
		$objExcel->setActiveSheetIndex(0)
		     ->setCellValue('A'.$intPosEncabezados, 'SERVICIO')
       ...
       //Combinar las siguientes celdas
		...
		{	
			...
			{   
				...
				//Agregar información del registro
				$objExcel->setActiveSheetIndex(0)
					 		 ->setCellValue('A'.$intFila, $arrCol->servicio)

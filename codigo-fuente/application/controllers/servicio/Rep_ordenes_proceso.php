...
class Rep_ordenes_proceso extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('servicio/rep_ordenes_proceso', $arrDatos);
	}
	...
	public function get_reporte() 
	{
		...
		$otdResultado = $this->ordenes->buscar_ordenes_proceso($dteFechaCorte, $strSucursales, 
										   					   $strServiciosTipos, $intProspectoID,  
										   					   $intMecanicoID);  
		...
		$pdf->arrCabecera = array('FOLIO', 'CLIENTE', 'MAQUINARIA', 'SERIE', 'FECHA');
		//Establece el ancho de las columnas de cabecera
		$pdf->arrAnchura = array(18, 62, 60, 35, 15);
		...
		$pdf->strLinea1 =  'LISTADO DE ORDENES EN PROCESO AL '.$strTituloFecha;
			...
			foreach ($otdResultado as $arrCol)
			{
				...
				$pdf->Row(array($arrCol->folio, utf8_decode($arrCol->prospecto), 
							   utf8_decode($arrCol->maquinaria_descripcion), utf8_decode($arrCol->serie),
						   	   $arrCol->fecha), $pdf->arrAlineacion, 'ClippedCell');
		...
		$pdf->Output('ordenes_reparacion_en_proceso.pdf','I');	   
	}
	...
	public function get_xls() 
	{
	   ...
	   //Se crea una instancia de la clase phpExcel
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE ORDENES EN PROCESO AL '.$strTituloFecha)
        ...
        $objExcel->setActiveSheetIndex(0)
        		 ...
        		 ->setCellValue('F'.$intPosEncabezados, 'DOMICILIO')
        		 ->setCellValue('G'.$intPosEncabezados, 'TIPO DE SERVICIO')
        		 ->setCellValue('H'.$intPosEncabezados, 'UBICACIÓN')
        		 ->setCellValue('I'.$intPosEncabezados, 'TIPO DE REPARACIÓN')
		...
		if ($otdResultado)
		{
			...
			{ 
				...
				$objExcel->setActiveSheetIndex(0)
                         ...
                         ->setCellValue('F'.$intFila, $strDomicilio)
                         ->setCellValue('G'.$intFila, $arrCol->servicio_tipo)
                         ->setCellValue('H'.$intFila, $arrCol->ubicacion)
                         ->setCellValue('I'.$intFila, $arrCol->tipo_reparacion)
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'ordenes_reparacion_en_proceso.xls', 'ordenes en proceso', $intFila);
								 ...
								 $intAcumGralPrecioGS)
	{
		...
		if($otdSalidasRefacciones)
		{
	        ...
	        foreach ($otdSalidasRefacciones as $arrSal) 
	        {
        		...
        		$arrAuxiliar["costo"] = $intCostoUnitario;
                $arrAuxiliar["precio"] = $intPrecioUnitario;

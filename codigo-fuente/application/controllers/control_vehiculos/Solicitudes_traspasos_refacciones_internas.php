...
class Solicitudes_traspasos_refacciones_internas extends MY_Controller {
	//Constructor de la clase
	function __construct()
	{
		parent::__construct();
		//Cargamos el modelo de solicitudes de traspasos de refacciones internas
		$this->load->model('control_vehiculos/solicitudes_traspasos_refacciones_internas_model', 'solicitudes');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('control_vehiculos/solicitudes_traspasos_refacciones_internas', $arrDatos);
	}
	...
	*********************************************************************************************************************/
	...
	{ 
		...
		$objSolicitudTraspaso->intSolicitudTraspasoRefaccionesInternasID = $this->input->post('intSolicitudTraspasoRefaccionesInternasID');
		...
		if (is_numeric($objSolicitudTraspaso->intSolicitudTraspasoRefaccionesInternasID))
		{
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intSolicitudTraspasoRefaccionesInternasID');
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intSolicitudTraspasoRefaccionesInternasID', 'ID', 'required|integer');
	    	...
	    	if ($otdResultado)
			{
				...
				foreach ($otdResultado as $arrCol)
				{
		        	$arrDatos[] = array('value' => $arrCol->folio, 
		        						'data' => $arrCol->solicitud_traspaso_refacciones_internas_id);
				}
	    	}
	...
	public function get_reporte($dteFechaInicial, $dteFechaFinal, $strEstatus, 
	...
	{	            
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE SOLICITUDES A ALMACÉN ').$strTituloRangoFechas;
		//Crea los titulos de la cabecera
		$pdf->arrCabecera = array('FOLIO', 'FECHA', 'OBSERVACIONES', 'ESTATUS');
		//Establece el ancho de las columnas de cabecera
		$pdf->arrAnchura = array(20, 18, 112, 40);
		...
		{	
			...
			{ 
				...
				$pdf->Row(array($arrCol->folio, $arrCol->fecha,  utf8_decode($arrCol->observaciones), 
				...
				{
					//Seleccionar los detalles del registro
					$otdDetalles = $this->solicitudes->buscar_detalles($arrCol->solicitud_traspaso_refacciones_internas_id);
		...
		$pdf->Output('solicitudes_almacen.pdf','I'); 
	}

	//Método para generar un reporte PDF con la información de un registro 
	public function get_reporte_registro($intSolicitudTraspasoRefaccionesInternasID) 
	{	            
		//Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->solicitudes->buscar($intSolicitudTraspasoRefaccionesInternasID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->solicitudes->buscar_detalles($intSolicitudTraspasoRefaccionesInternasID);
		...
		$strNombreArchivo  = 'solicitud_almacen_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetXY(15, 42);
			$pdf->ClippedCell(185, 3, utf8_decode('SOLICITUD A ALMACÉN'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
			...
			//Estatus
			$pdf->SetXY(120, 46);
			$pdf->ClippedCell(50, 3, $otdResultado->estatus);
	 ...
	 *dependiendo del criterio de búsqueda proporcionado*/
	public function get_xls($dteFechaInicial, $dteFechaFinal,$strEstatus, 
							$strDetalles, $strBusqueda = NULL) 
	{	
		//Quitar espacios vacíos y decodificar cadena cifrada
		$strBusqueda = trim(urldecode($strBusqueda));
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE SOLICITUDES A ALMACÉN '.$strTituloRangoFechas);
        ...
        if($strDetalles == 'SI')
        {
        	 //Combinar las siguientes celdas
	       	$objExcel->setActiveSheetIndex(0)
			         ...
			         ->setCellValue('H'.$intPosEncabezados, 'REFACCIONES LINEA')
			         ->setCellValue('I'.$intPosEncabezados, 'REFACCIONES MARCA');

        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('E'.$intPosEncabezados.':I'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
				    ...
				    if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValue('H'.$intFila, $arrDetalles[$intContDet]['refacciones_linea'])
		                         ->setCellValue('I'.$intFila, $arrDetalles[$intContDet]['refacciones_marca']);
					}
       ...
       $this->get_pie_pagina_archivo_excel($objExcel, 'solicitudes_almacen.xls', 
        									'solicitudes a almacén', $intFila);
	}
}
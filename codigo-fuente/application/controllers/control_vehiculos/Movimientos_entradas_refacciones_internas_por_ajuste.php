...
class Movimientos_entradas_refacciones_internas_por_ajuste extends MY_Controller {
	//Variable para identificar el tipo de movimiento
	var $intTipoMovimiento = ENTRADA_REFACCIONES_INTERNAS_POR_AJUSTE; 
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('control_vehiculos/movimientos_entradas_refacciones_internas_por_ajuste', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		$result = $this->movimientos->filtro_entrada_por_ajuste($this->intTipoMovimiento,
	...
	{ 
		...
		if (is_numeric($intMovimientoRefaccionesInternasID))
		{
			...
			$bolResultado = $this->movimientos->modificar_entrada_por_ajuste($intMovimientoRefaccionesInternasID, 
		...
		{ 
			...
			{	
				$bolResultado = $this->movimientos->guardar_entrada_por_ajuste($strFolio, 
																	$this->intTipoMovimiento, 
																	$dteFecha, 
																    $strObservaciones, 
																    $strRefaccionInternaID, 
	...
	public function get_datos()
	{
	    ...
	    $otdResultado = $this->movimientos->buscar_entrada_por_ajuste($intID, $this->intTipoMovimiento);
		...
		if($otdResultado)
		{
			...
			$otdDetalles = $this->movimientos->buscar_detalles_entrada_por_ajuste($intID, $this->intTipoMovimiento);
	...
	public function get_renglones_movimiento(){
		...
		//Seleccionar los datos de los detalles
		$otdRenglones = $this->movimientos->buscar_renglones_movimiento($intID);
		
		//Si existen detalles del registro, se asignan al array
		if($otdRenglones)
		{
			$arrDatos['renglones'] = $otdRenglones;
		}
	...
	{	            
		...
		$otdResultado = $this->movimientos->buscar_entrada_por_ajuste(NULL, $this->intTipoMovimiento, $dteFechaInicial, $dteFechaFinal);
		...
		$pdf->strLinea1 =  'LISTADO DE ENTRADAS DE REFACCIONES INTERNAS POR AJUSTE '.$strTituloRangoFechas;
		//Crea los titulos de la cabecera
		$pdf->arrCabecera = array('FOLIO', 'FECHA', 'CANTIDAD', 'DESCUENTO', 'SUBTOTAL', 'IVA', 'IEPS', 'TOTAL', 'ESTATUS');
		...
		{	
			...
			{ 
				...
				$otdDetalles = $this->movimientos->buscar_detalles_entrada_por_ajuste($arrCol->movimiento_refacciones_internas_id);
		...
		$pdf->Output('entradas_refacciones_internas_por_ajuste.pdf','I'); 
	}
	...
	{	            
		...
		$this->load->helper('hlpfpdf');
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->movimientos->buscar_entrada_por_ajuste($intMovimientoRefaccionesInternasID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->movimientos->buscar_detalles_entrada_por_ajuste($intMovimientoRefaccionesInternasID, $this->intTipoMovimiento);
		...
		$strNombreArchivo  = 'entrada_refacciones_internas_por_ajuste_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
	        ...
	        //------------------------------------------------------------------------------------------------------------------------
			//------------------------------------------------------------------------------------------------------------------------
	        //---------- DATOS DEL MOVIMIENTO DE ENTRADA POR AJUSTE DE REFACCIONES INTERNAS
	        //------------------------------------------------------------------------------------------------------------------------
			//Encabezado
			...
			$pdf->SetXY(15, 42);
			$pdf->ClippedCell(185, 3, utf8_decode('ENTRADA DE REFACCIONES INTERNAS POR AJUSTE'), 0, 0, 'C', TRUE);
			...
			if($otdDetalles)
			{
				...
				$arrCabecera = array(utf8_decode('Código'), utf8_decode('Descripción'), 
								     'Unidad', 'Cantidad', 'P. Unitario', 'Descuento', 'Subtotal', 
								     'IVA', 'IEPS', 'Importe');
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)->setCellValue('A7', 'LISTADO DE ENTRADAS DE REFACCIONES INTERNAS POR AJUSTE '.$strTituloRangoFechas);
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'entradas_refacciones_internas_por_ajuste.xls', 'entradas por ajuste', $intFila);

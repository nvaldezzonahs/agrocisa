...
class Movimientos_entradas_refacciones_internas_traspaso extends MY_Controller {
	///Variable para identificar el tipo de movimiento
	var $intTipoMovimiento = ENTRADA_REFACCIONES_INTERNAS_TRASPASO;
	...
	function __construct()
	{
		...
		$this->load->model('refacciones/movimientos_refacciones_model', 'referencia');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('control_vehiculos/movimientos_entradas_refacciones_internas_traspaso', $arrDatos);
	}
	...
	*********************************************************************************************************************/
	...
	//Verifica la existencia de la salida por traspaso vehicular
	...
	public function set_estatus()
	{
		...
		else
		{
	        ...
	        //Hacer un llamado al método para modificar el historial de inventario de la salida por traspaso vehicular
			$bolResultadoSalida = $this->referencia->modificar_historial_inventario_refacciones_salida_traspaso($intReferenciaID); 
			//Si no se obtienen errores al ejecutar el proceso
	        if($bolResultadoSalida)
	        {
	        	 //Hacer un llamado al método para modificar el estatus del registro
				$bolResultado = $this->movimientos->set_estatus($intMovimientoRefaccionesInternasID, $this->intTipoMovimiento, $intPolizaID);
				//Si no se obtienen errores al ejecutar el proceso
				if($bolResultado)
				{
					//Enviar el mensaje de éxito al formulario
	...
	{	            
		...
		{	
			...
			{ 
				...
				$otdDetalles = $this->movimientos->buscar_detalles_entrada_traspaso($arrCol->movimiento_refacciones_internas_id);
				...
				$pdf->Row(array($arrCol->folio, $arrCol->fecha, utf8_decode($arrCol->sucursal), 
		...
		$pdf->Output('entradas_refacciones_internas_traspaso.pdf','I'); 
	}
	...
	{	            
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->movimientos->buscar_entrada_traspaso($intMovimientoRefaccionesInternasID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->movimientos->buscar_detalles_entrada_traspaso($intMovimientoRefaccionesInternasID);
		...
		$strNombreArchivo  = 'entrada_refacciones_internas_traspaso_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('ENTRADA DE REFACCIONES INTERNAS POR TRASPASO'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE ENTRADAS DE REFACCIONES INTERNAS POR TRASPASO '.$strTituloRangoFechas);
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'entradas_refacciones_internas_traspaso.xls', 
        									'entradas de refacciones', $intFila);
	}

	/*******************************************************************************************************************
	Funciones de la tabla movimientos_refacciones_intenas_detalles
	*********************************************************************************************************************/
	...
	public function get_datos_detalles()
	{
	    ...
	    $otdResultado = $this->movimientos->buscar_detalles_entrada_traspaso($intMovimientoRefaccionesInternasID, 
	    																	 $intReferenciaID);
		//Si existen datos, asignar los datos recuperados en el array
		if($otdResultado)
		{

...
class Rep_detallado extends MY_Controller {
	...
	function __construct()
	{
		parent::__construct();
		//Cargamos el modelo
		$this->load->model('control_vehiculos/rep_detallado_model', 'rep_ordenes');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('control_vehiculos/Rep_detallado', $arrDatos);
	}
	...
	public function get_reporte($dteFechaCorte, $intSucursalID, $intVehiculoID, $intMecanicoID) 
	{
		...
		$pdf->strLinea1 =  'LISTADO DETALLADO DE ORDENES DE TRABAJO INTERNAS HASTA EL: '.$strTituloFecha;
		//Establece los títulos de la cabecera
		$pdf->arrCabecera = array(utf8_decode('FOLIO'), utf8_decode('CÓDIGO'), utf8_decode('VEHÍCULO'), 
								  'PLACAS', 'FECHA', 'ESTATUS');
		//Establece el ancho de las columnas de cabecera
		$pdf->arrAnchura = array(25, 50, 35, 30, 30, 20);
		//Establece la alineación de las celdas de la tabla
		$pdf->arrAlineacion = array('L', 'L', 'L', 'L', 'C', 'C');
		//Establece la alineación de las celdas de la tabla trabajos foráneos
		$arrAnchuraTrabajosForaneos = array(25, 50, 35, 30, 30, 20);
		//Establece la alineación de las celdas de la tabla trabajos foráneos
		$arrAlineacionTrabajosForaneos = array('L', 'L', 'L', 'L', 'C', 'C');
		//Establece la alineación de las celdas de la tabla salidas de refacciones
		$arrAnchuraServicios = array(25, 50, 35, 30, 30, 20);
		//Establece la alineación de las celdas de la tabla salidas de refacciones
		$arrAlineacionServicios = array('L', 'L', 'L', 'L', 'C', 'C');
		...
		if ($otdResultado)
		{
			...
			foreach ($otdResultado as $arrCol)
			{
				...
				$pdf->Row(array($arrCol->folio, utf8_decode($arrCol->codigo), 
				...
				$pdf->ClippedCell(180, 4, utf8_decode('CAUSA: ').utf8_decode($arrCol->falla), 0, 0, 'L', 0);$pdf->Ln();

				//Verificar si existe información de las salidas de refacciones
				if($otdSalidasRefacciones)
				{
					//Establece la alineación de las celdas de la tabla salidas de refacciones
					$arrAnchuraSalidasRefacciones = array(25, 50, 35, 30, 30, 20);
					//Establece la alineación de las celdas de la tabla salidas de refacciones
					$arrAlineacionSalidasRefacciones = array('L', 'L', 'L', 'L', 'C', 'C');
								...
								if($arrDet2->estatus == 'ACTIVO')
								{
									...
									if($arrDet2->tipo == 'SALIDA'){ //Importe $0.00
										...
										$pdf->Row(array($arrDet2->folio, 'REFACCIONES', $arrDet2->fecha, 
								    			  ...
								    			  $arrAnchuraSalidasRefacciones, 'ClippedCell');

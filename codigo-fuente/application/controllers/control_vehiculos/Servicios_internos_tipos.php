...
class Servicios_internos_tipos extends MY_Controller {
	//Constructor de la clase
	function __construct()
	{
		parent::__construct();
		//Cargamos el modelo
		$this->load->model('control_vehiculos/servicios_internos_tipos_model', 'tipos');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('control_vehiculos/servicios_internos_tipos', $arrDatos);
	}
	...
	{ 
		...
		$objServicioInternoTipo = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista 
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objServicioInternoTipo->intServicioInternoTipoID = $this->input->post('intServicioInternoTipoID');
		$objServicioInternoTipo->strDescripcion = mb_strtoupper(trim($this->input->post('strDescripcion')));
		$objServicioInternoTipo->strDescripcionAnterior = mb_strtoupper(trim($this->input->post('strDescripcionAnterior'))); 
		$objServicioInternoTipo->intUsuarioID = $this->session->userdata('usuario_id');
		
        //Definir las reglas de validación
		//Validar que el código sea único
        if (($objServicioInternoTipo->intServicioInternoTipoID == '') OR 
        	($objServicioInternoTipo->strDescripcionAnterior != $objServicioInternoTipo->strDescripcion))
        {
            $this->form_validation->set_rules('strDescripcion', 'descripción', 
            						          'required|is_unique[servicios_internos_tipos.descripcion]');
        }
        else
        {
		...
		else
		{
			...
			if (is_numeric($objServicioInternoTipo->intServicioInternoTipoID))
			{
				$bolResultado = $this->tipos->modificar($objServicioInternoTipo);
			}
			else
			{ 
				$bolResultado = $this->tipos->guardar($objServicioInternoTipo);
			}
	...
	{
	    //Definir las reglas de validación
		$this->form_validation->set_rules('intServicioInternoTipoID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intServicioInternoTipoID');
	    	...
	    	if ($otdResultado)
			{
				...
				foreach ($otdResultado as $arrCol)
				{
		        	$arrDatos[] = array('value' => $arrCol->descripcion, 
		        					    'data' => $arrCol->servicio_interno_tipo_id);
				}
	    	}
	...
	{	            
		...
		$pdf->strLinea1 = 'LISTADO DE TIPOS DE SERVICIOS INTERNOS';
		...
		$pdf->Output('tipos_servicios_internos.pdf','I'); 
	}
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE TIPOS DE SERVICIOS INTERNOS');
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'tipos_servicios_internos.xls', 'tipos de servicios', $intFila);
	}
}
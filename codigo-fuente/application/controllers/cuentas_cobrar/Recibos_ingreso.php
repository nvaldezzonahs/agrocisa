...
class Recibos_ingreso extends MY_Controller {
	...
	function __construct()
	{
	    ...
	    $this->archivo['strCarpetaTemporal'] = './recibos_ingreso_temporal_';
		//Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Cargamos el modelo de recibos de ingreso
		$this->load->model('cuentas_cobrar/recibos_ingreso_model', 'recibos');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('cuentas_cobrar/recibos_ingreso', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		$result = $this->recibos->filtro($this->input->post('dteFechaInicial'),
		...
		foreach ($result['recibos'] as $arrDet)
		{
		...
		$arrDatos = array('rows' => $result['recibos'],
	...
	{ 
		...
		$objReciboIngreso = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objReciboIngreso->intReciboIngresoID = $this->input->post('intReciboIngresoID');
		$objReciboIngreso->dteFecha = $this->input->post('dteFecha');
		$objReciboIngreso->intMonedaID = $this->input->post('intMonedaID');
		$objReciboIngreso->intTipoCambio = $this->input->post('intTipoCambio');
		$objReciboIngreso->intProspectoID = $this->input->post('intProspectoID');
		$objReciboIngreso->strRazonSocial =  mb_strtoupper(trim($this->input->post('strRazonSocial')));
		$objReciboIngreso->strRfc =  mb_strtoupper(trim($this->input->post('strRfc')));
		$objReciboIngreso->strCalle =  mb_strtoupper(trim($this->input->post('strCalle')));
		$objReciboIngreso->strNumeroExterior = mb_strtoupper($this->input->post('strNumeroExterior'));
		$objReciboIngreso->strNumeroInterior = mb_strtoupper($this->input->post('strNumeroInterior'));
		$objReciboIngreso->strCodigoPostal = $this->input->post('strCodigoPostal');
		$objReciboIngreso->strColonia =  mb_strtoupper(trim($this->input->post('strColonia')));
		$objReciboIngreso->strLocalidad =  mb_strtoupper(trim($this->input->post('strLocalidad')));
		$objReciboIngreso->strMunicipio = mb_strtoupper(trim($this->input->post('strMunicipio')));
		$objReciboIngreso->strEstado =  mb_strtoupper(trim($this->input->post('strEstado')));
		$objReciboIngreso->strPais = mb_strtoupper(trim($this->input->post('strPais')));
		$objReciboIngreso->intFormaPagoID = $this->input->post('intFormaPagoID');
		$objReciboIngreso->strObservaciones = mb_strtoupper(trim($this->input->post('strObservaciones')));
		$objReciboIngreso->intSucursalID = $this->session->userdata('sucursal_id');
		$objReciboIngreso->intUsuarioID = $this->session->userdata('usuario_id');
		//Datos de los detalles
		$objReciboIngreso->strReferencias = $this->input->post('strReferencias');
		$objReciboIngreso->strReferenciaID = $this->input->post('strReferenciaID');
		$objReciboIngreso->strPrecios = $this->input->post('strPrecios'); 
		$objReciboIngreso->strTasaCuotaIva = $this->input->post('strTasaCuotaIva'); 
		$objReciboIngreso->strIvas = $this->input->post('strIvas');
		$objReciboIngreso->strTasaCuotaIeps = $this->input->post('strTasaCuotaIeps'); 
		$objReciboIngreso->strIeps = $this->input->post('strIeps');
		...
		if (is_numeric($objReciboIngreso->intReciboIngresoID))
		{
			$bolResultado = $this->recibos->modificar($objReciboIngreso);
		}
		else
		{ 
			//Hacer un llamado a la función para generar el folio consecutivo del proceso
			$objReciboIngreso->strFolio = $this->get_folio_consecutivo($intProcesoMenuID, 10);
			//Si no existe folio del proceso
			if($objReciboIngreso->strFolio == '')
			{
			...
			{
				$bolResultado = $this->recibos->guardar($objReciboIngreso); 
				/*Quitar '_'  de la cadena (resultadoTransaccion_reciboIngresoID) para obtener el resultado de la transacción del movimiento en la BD y el id del registro 
				 */
			    list($bolResultado, $objReciboIngreso->intReciboIngresoID) = explode("_", $bolResultado); 
			}
		}
		...
		if($bolResultado !== NULL)
		{
			...
			{ 
				...
				$arrDatos = array('resultado' => TRUE,
							 	  ...
							 	  'recibo_ingreso_id' => $objReciboIngreso->intReciboIngresoID,
							      'mensaje' => MSJ_GUARDAR);
			}
			else
			{
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intReciboIngresoID');
		//Seleccionar los datos del registro que coincide con el id
	    $otdResultado = $this->recibos->buscar($intID);
		//Si existen datos, asignar los datos recuperados en el array
		if($otdResultado)
		{
			$arrDatos['row'] = $otdResultado;
			//Seleccionar los detalles del registro
			$otdDetalles = $this->recibos->buscar_detalles($intID);
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intReciboIngresoID', 'ID', 'required|integer');
		...
		else
		{
			...
			$bolResultado = $this->recibos->set_estatus($intID, $intPolizaID);
			//Si no se obtienen errores al ejecutar el proceso
			if($bolResultado)
			{
				//Enviar el mensaje de éxito al formulario
	...
	{
		//Variables que se utilizan para recuperar los valores de la vista
		$intReciboIngresoID = $this->input->post('intReciboIngresoID');
        ...
        $strRuta = $this->get_reporte_registro($intReciboIngresoID, 'SI');

        //Array que se utiliza para enviar correo electrónico
		$arrDatos =  array('intReferenciaID' => $intReciboIngresoID,
						   'strTitulo' => utf8_decode('Recibo de ingreso'),
						   'strRuta' => $strRuta,
						   'strCorreoElectronico'  => $strCorreoElectronico,
						   'strCopiaCorreoElectronico' => $strCopiaCorreoElectronico,
						   'strComentarios' => 'Recibo de ingreso.');
	...
	public function eliminar_carpeta_temporal($intReciboIngresoID)
	{
        //Definir ubicación de la carpeta
		$strNombreCarpeta = $this->archivo['strCarpetaTemporal'].$intReciboIngresoID; 
	...
	{	            
		...
		$otdResultado = $this->recibos->buscar(NULL, $dteFechaInicial, $dteFechaFinal, $intProspectoID, 
											   $strEstatus, $strBusqueda);
		//Seleccionar los datos de las monedas que coinciden con el parámetro enviado
		$otdMonedas = $this->recibos->buscar_distintas_monedas($dteFechaInicial, $dteFechaFinal, $intProspectoID,
														       $strEstatus, $strBusqueda);
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE RECIBOS DE INGRESO ').$strTituloRangoFechas;
	    ...
	    $arrAlineacionDetalles = array('L', 'L', 'L', 'L', 'L', 'R',  'R', 'R');
	    //Establece el ancho de las columnas de la tabla detalles
		$arrAnchuraDetalles = array(16, 15, 20, 10, 40, 17,  16, 18);
		...
		{	
			...
			{ 
				...
				{
					//Seleccionar los detalles del registro
					$otdDetalles = $this->recibos->buscar_detalles($arrCol->recibo_ingreso_id);
					...
					if($otdDetallesFactura)
					{
				        ...
				        foreach ($otdDetallesFactura as $arrDet)
				        {
						    ...
						    $pdf->Row(array($arrDet->folio, $arrDet->fecha, $arrDet->referencia, 
						    				$arrDet->metodo_pago, $strImpuestos, 
		...
		$pdf->Output('recibos_ingreso.pdf','I'); 
	}

	//Método para generar un reporte PDF con la información de un registro 
	public function get_reporte_registro($intReciboIngresoID = NULL, $strEnviarCorreo = NULL) 
	{	     
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->recibos->buscar($intReciboIngresoID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->recibos->buscar_detalles($intReciboIngresoID);
		...
		$strNombreArchivo  = 'recibo_ingreso_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('RECIBO DE INGRESO'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
			...
			//Forma de pago
			$pdf->SetXY(108, 52);
			$pdf->ClippedCell(25, 3, 'FORMA DE PAGO');
			...
			//Forma de pago
			$pdf->SetXY(135, 52);
			$pdf->ClippedCell(29, 3, utf8_decode($otdResultado->forma_pago));

			//------------------------------------------------------------------------------------------------------------------------
	        //---------- DETALLES DEL RECIBO DE INGRESO
	        //------------------------------------------------------------------------------------------------------------------------	
			$pdf->SetTextColor(COLOR_TEXTO); //establece el color de texto
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			...
			if($otdDetallesFactura)
			{
				//Tabla con los detalles del recibo de ingreso
				$pdf->SetXY(15, 74);
				//Crea los titulos de la cabecera
				$arrCabecera = array(utf8_decode('Referencia'),'Folio factura',
									 'Fecha factura', 'M. pago', 'Precio', 
									 'IVA', 'IEPS', 'Importe');
				//Establece el ancho de las columnas de cabecera
				$arrAchura = array(30, 20, 20, 15, 25, 25, 25, 25);
				//Establece la alineación de las celdas de la tabla
				$arrAlineacion = array('L', 'L', 'C', 'C', 'R', 'R', 'R', 'R');
		        ...
		        foreach ($otdDetallesFactura as $arrDet) 
		        {
				   ...
				   //Se agrega la informacion al reporte... se utiliza utf8 para acentos y tildes
				    $pdf->Row(array(utf8_decode($arrDet->referencia),
				    				$arrDet->folio,
				    				$arrDet->fecha,
				    				$arrDet->metodo_pago,  
		...
		{	
			...
			//Definir ubicación de la carpeta
			$strCarpetaDestino =  $this->archivo['strCarpetaTemporal'].$intReciboIngresoID.'/'; 
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE RECIBOS DE INGRESO '.$strTituloRangoFechas);
        ...
        if($strDetalles == 'SI')
        {
        	//Combinar las siguientes celdas
	       	$objExcel->setActiveSheetIndex(0)
                     ->setCellValue('N'.$intPosEncabezados, 'FOLIO')
			         ->setCellValue('O'.$intPosEncabezados, 'FECHA')
			         ->setCellValue('P'.$intPosEncabezados, 'REFERENCIA')
			         ->setCellValue('Q'.$intPosEncabezados, 'MÉTODO DE PAGO')
			         ->setCellValue('R'.$intPosEncabezados,'IVA %')
			         ->setCellValue('S'.$intPosEncabezados,'IEPS %')
			         ->setCellValue('T'.$intPosEncabezados, 'TOTAL')
			         ->setCellValue('U'.$intPosEncabezados, 'ABONO')
			         ->setCellValue('V'.$intPosEncabezados, 'SALDO');

        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('N'.$intPosEncabezados.':V'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
		...
		{	
			...
			{   
			    ...
			    if($strDetalles == 'SI')
			    {
					...
					if($otdDetallesFactura)
					{
						...
						foreach ($otdDetallesFactura as $arrDet)
						{
							...
							$arrDetalles[$intContDetIng]["folio"] = $arrDet->folio;
							$arrDetalles[$intContDetIng]["fecha"] = $arrDet->fecha;
							$arrDetalles[$intContDetIng]["referencia"] = $arrDet->referencia;
							$arrDetalles[$intContDetIng]["porcentaje_iva"] = $arrDet->porcentaje_iva;
							$arrDetalles[$intContDetIng]["porcentaje_ieps"] = $arrDet->porcentaje_ieps;
							$arrDetalles[$intContDetIng]["total_factura"] = $arrDet->total_factura_auxiliar;
							$arrDetalles[$intContDetIng]["metodo_pago"] = $arrDet->metodo_pago;
			                $arrDetalles[$intContDetIng]["abono"] = $arrDet->abono;
			                $arrDetalles[$intContDetIng]["saldo_factura"] = $arrDet->saldo_factura;
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
								 ->setCellValueExplicit('N'.$intFila, $arrDetalles[$intContDet]['folio'], PHPExcel_Cell_DataType::TYPE_STRING)
								 ->setCellValue('O'.$intFila, $arrDetalles[$intContDet]['fecha'])
								 ->setCellValue('P'.$intFila, $arrDetalles[$intContDet]['referencia'])
						         ->setCellValue('Q'.$intFila, $arrDetalles[$intContDet]['metodo_pago'])
						         ...
						         ->setCellValue('T'.$intFila, $arrDetalles[$intContDet]['total_factura'])
						         ->setCellValue('U'.$intFila, $arrDetalles[$intContDet]['abono'])
						         ->setCellValue('V'.$intFila, $arrDetalles[$intContDet]['saldo_factura']);
					}
           ...
           $objExcel->getActiveSheet()
            ...
            $objExcel->getActiveSheet()
		        	 ->getStyle('R'.$intFilaInicial.':'.'V'.$intFila)
		        	 ->getAlignment()
		        	 ->applyFromArray($arrStyleAlignmentRight);
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'recibos_ingreso.xls', 'recibos de ingreso', $intFila);
	}

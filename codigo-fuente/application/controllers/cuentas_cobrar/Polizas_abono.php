...
class Polizas_abono extends MY_Controller {
	...
	function __construct()
	{
	    ...
	    $this->archivo['strCarpetaTemporal'] = './polizas_abono_temporal_';
		//Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Cargamos el modelo de pólizas de abono
		$this->load->model('cuentas_cobrar/polizas_abono_model', 'polizas');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('cuentas_cobrar/polizas_abono', $arrDatos);
	}
	...
	{ 
		...
		$objPolizaAbono = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objPolizaAbono->intPolizaAbonoID = $this->input->post('intPolizaAbonoID');
		$objPolizaAbono->dteFecha = $this->input->post('dteFecha');
		$objPolizaAbono->intAnticipoID = $this->input->post('intAnticipoID');
		$objPolizaAbono->intMonedaID = $this->input->post('intMonedaID');
		$objPolizaAbono->intTipoCambio = $this->input->post('intTipoCambio');
		$objPolizaAbono->intProspectoID = $this->input->post('intProspectoID');
		$objPolizaAbono->strRazonSocial =  mb_strtoupper(trim($this->input->post('strRazonSocial')));
		$objPolizaAbono->strRfc =  mb_strtoupper(trim($this->input->post('strRfc')));
		$objPolizaAbono->strCalle =  mb_strtoupper(trim($this->input->post('strCalle')));
		$objPolizaAbono->strNumeroExterior = mb_strtoupper($this->input->post('strNumeroExterior'));
		$objPolizaAbono->strNumeroInterior = mb_strtoupper($this->input->post('strNumeroInterior'));
		$objPolizaAbono->strCodigoPostal = $this->input->post('strCodigoPostal');
		$objPolizaAbono->strColonia =  mb_strtoupper(trim($this->input->post('strColonia')));
		$objPolizaAbono->strLocalidad =  mb_strtoupper(trim($this->input->post('strLocalidad')));
		$objPolizaAbono->strMunicipio = mb_strtoupper(trim($this->input->post('strMunicipio')));
		$objPolizaAbono->strEstado =  mb_strtoupper(trim($this->input->post('strEstado')));
		$objPolizaAbono->strPais = mb_strtoupper(trim($this->input->post('strPais')));
		$objPolizaAbono->strObservaciones = mb_strtoupper(trim($this->input->post('strObservaciones')));
		$objPolizaAbono->intSucursalID = $this->session->userdata('sucursal_id');
		$objPolizaAbono->intUsuarioID = $this->session->userdata('usuario_id');
		//Datos de los detalles
		$objPolizaAbono->strReferencias = $this->input->post('strReferencias');
		$objPolizaAbono->strReferenciaID = $this->input->post('strReferenciaID');
		$objPolizaAbono->strPrecios = $this->input->post('strPrecios'); 
		$objPolizaAbono->strTasaCuotaIva = $this->input->post('strTasaCuotaIva'); 
		$objPolizaAbono->strIvas = $this->input->post('strIvas');
		$objPolizaAbono->strTasaCuotaIeps = $this->input->post('strTasaCuotaIeps'); 
		$objPolizaAbono->strIeps = $this->input->post('strIeps');
		...
		if (is_numeric($objPolizaAbono->intPolizaAbonoID))
		{
			$bolResultado = $this->polizas->modificar($objPolizaAbono);
		}
		else
		{ 
			//Hacer un llamado a la función para generar el folio consecutivo del proceso
			$objPolizaAbono->strFolio = $this->get_folio_consecutivo($intProcesoMenuID, 10);
			//Si no existe folio del proceso
			if($objPolizaAbono->strFolio == '')
			{
			...
			{
				$bolResultado = $this->polizas->guardar($objPolizaAbono); 
				/*Quitar '_'  de la cadena (resultadoTransaccion_polizaAbonoID) para obtener el resultado de la transacción del movimiento en la BD y el id del registro 
				 */
			    list($bolResultado, $objPolizaAbono->intPolizaAbonoID) = explode("_", $bolResultado); 
		...
		if($bolResultado !== NULL)
		{
			...
			{ 
				...
				$arrDatos = array('resultado' => TRUE,
							 	  ...
							 	  'poliza_abono_id' => $objPolizaAbono->intPolizaAbonoID,
							      'mensaje' => MSJ_GUARDAR);
			}
			else
			{
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intPolizaAbonoID');
		...
		if($otdResultado)
		{
			...
			$arrSaldoAnticipo = $this->get_saldo_anticipo($otdResultado->anticipo_id, $intID, 'poliza_abono', 'polizas_aplicacion');
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intPolizaAbonoID', 'ID', 'required|integer');
		...
		else
		{
			...
			$bolResultado = $this->polizas->set_estatus($intID, $intPolizaID);
			//Si no se obtienen errores al ejecutar el proceso
			if($bolResultado)
			{
				//Enviar el mensaje de éxito al formulario
	...
	{
		//Variables que se utilizan para recuperar los valores de la vista
		$intPolizaAbonoID = $this->input->post('intPolizaAbonoID');
        ...
        $strRuta = $this->get_reporte_registro($intPolizaAbonoID, 'SI');

         //Array que se utiliza para enviar correo electrónico
		$arrDatos =  array('intReferenciaID' => $intPolizaAbonoID,
						   'strTitulo' => utf8_decode('Póliza de abono'),
						   'strRuta' => $strRuta,
						   'strCorreoElectronico'  => $strCorreoElectronico,
						   'strCopiaCorreoElectronico' => $strCopiaCorreoElectronico,
						   'strComentarios' => 'Póliza de abono.');
	...
	public function eliminar_carpeta_temporal($intPolizaAbonoID)
	{
		  //Definir ubicación de la carpeta
		$strNombreCarpeta = $this->archivo['strCarpetaTemporal'].$intPolizaAbonoID; 
	...
	{	            
		...
		$otdResultado = $this->polizas->buscar(NULL, $dteFechaInicial, $dteFechaFinal, $intProspectoID, 
											   $strEstatus, $strBusqueda);
		//Seleccionar los datos de las monedas que coinciden con el parámetro enviado
		$otdMonedas = $this->polizas->buscar_distintas_monedas($dteFechaInicial, $dteFechaFinal, $intProspectoID,
														       $strEstatus, $strBusqueda);
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE PÓLIZAS DE ABONO ').$strTituloRangoFechas;
		...
		{	
			...
			//Recorremos el arreglo para obtener la información de las pólizas
			...
			{ 
		       ...
		       //Variable que se utiliza para asignar el acumulado del subtotal
				...
				{
					//Seleccionar los detalles del registro
					$otdDetalles = $this->polizas->buscar_detalles($arrCol->poliza_abono_id);
					//Hacer un llamado a la función para obtener detalles de la referencia
		...
		$pdf->Output('polizas_abono.pdf','I'); 
	}

	//Método para generar un reporte PDF con la información de un registro 
	public function get_reporte_registro($intPolizaAbonoID = NULL, $strEnviarCorreo = NULL) 
	{	    
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->polizas->buscar($intPolizaAbonoID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->polizas->buscar_detalles($intPolizaAbonoID);
		...
		$strNombreArchivo  = 'poliza_abono_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('PÓLIZA DE ABONO'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
			...
			//------------------------------------------------------------------------------------------------------------------------
	        //---------- DETALLES DE LA PÓLIZA DE ABONO
	        //------------------------------------------------------------------------------------------------------------------------	
			$pdf->SetTextColor(COLOR_TEXTO); //establece el color de texto
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			...
			if($otdDetallesFactura)
			{
				//Tabla con los detalles de la póliza de abono
				$pdf->SetXY(15, 74);
				//Crea los titulos de la cabecera
		...
		{	
            ...
            //Definir ubicación de la carpeta
			$strCarpetaDestino =  $this->archivo['strCarpetaTemporal'].$intPolizaAbonoID.'/'; 
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE PÓLIZAS DE ABONO '.$strTituloRangoFechas);
        ...
        if($strDetalles == 'SI')
        {
        	//Combinar las siguientes celdas
	       	$objExcel->setActiveSheetIndex(0)
			         ...
			         ->setCellValue('P'.$intPosEncabezados, 'MÉTODO DE PAGO')
			         ->setCellValue('Q'.$intPosEncabezados,'IVA %')
			         ->setCellValue('R'.$intPosEncabezados,'IEPS %')
			         ->setCellValue('S'.$intPosEncabezados, 'TOTAL')
			         ->setCellValue('T'.$intPosEncabezados, 'ABONO')
			         ->setCellValue('U'.$intPosEncabezados, 'SALDO');
		...
		{	
			    ...
			    if($strDetalles == 'SI')
			    {
					...
					if($otdDetallesFactura)
					{
						...
						foreach ($otdDetallesFactura as $arrDet)
						{
							...
							$arrDetalles[$intContDetPol]["folio"] = $arrDet->folio;
							$arrDetalles[$intContDetPol]["fecha"] = $arrDet->fecha;
							$arrDetalles[$intContDetPol]["referencia"] = $arrDet->referencia;
							$arrDetalles[$intContDetPol]["porcentaje_iva"] = $arrDet->porcentaje_iva;
							$arrDetalles[$intContDetPol]["porcentaje_ieps"] = $arrDet->porcentaje_ieps;
							$arrDetalles[$intContDetPol]["total_factura"] = $arrDet->total_factura_auxiliar;
							$arrDetalles[$intContDetPol]["metodo_pago"] = $arrDet->metodo_pago;
			                $arrDetalles[$intContDetPol]["abono"] = $arrDet->abono;
			                $arrDetalles[$intContDetPol]["saldo_factura"] = $arrDet->saldo_factura;
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
						         ...
						         ->setCellValue('P'.$intFila, $arrDetalles[$intContDet]['metodo_pago'])
						         ->setCellValueExplicit('Q'.$intFila, $arrDetalles[$intContDet]['porcentaje_iva'], PHPExcel_Cell_DataType::TYPE_STRING)
						         ->setCellValueExplicit('R'.$intFila, $arrDetalles[$intContDet]['porcentaje_ieps'], PHPExcel_Cell_DataType::TYPE_STRING)
						         ->setCellValue('S'.$intFila, $arrDetalles[$intContDet]['total_factura'])
						         ->setCellValue('T'.$intFila, $arrDetalles[$intContDet]['abono'])
						         ->setCellValue('U'.$intFila, $arrDetalles[$intContDet]['saldo_factura']);
					}
            ...
            $objExcel->getActiveSheet()
            		 ->getStyle('S'.$intFilaInicial.':'.'U'.$intFila)
            		 ->getNumberFormat()
            		 ->setFormatCode('$#,##0.00');
           ...
           $objExcel->getActiveSheet()
            ...
            $objExcel->getActiveSheet()
		        	 ->getStyle('Q'.$intFilaInicial.':'.'U'.$intFila)
		        	 ->getAlignment()
		        	 ->applyFromArray($arrStyleAlignmentRight);
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'polizas_abono.xls', 'pólizas de abono', $intFila);
	}

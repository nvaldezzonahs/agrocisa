...
class Documentos_pagos extends MY_Controller {
	//Constructor de la clase
	function __construct()
	{
		parent::__construct();
		//Cargamos el modelo
		$this->load->model('cuentas_cobrar/documentos_pagos_model', 'documentos');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('cuentas_cobrar/documentos_pagos', $arrDatos);
	}
	...
	{ 
		...
		$objDocumentoPago = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objDocumentoPago->intDocumentoPagoID = $this->input->post('intDocumentoPagoID');
		//Datos de los Documentos_pagos
		$objDocumentoPago->strDescripcion = mb_strtoupper(trim($this->input->post('strDescripcion'))); 
		$objDocumentoPago->strDescripcionAnterior = mb_strtoupper(trim($this->input->post('strDescripcionAnterior')));
		$objDocumentoPago->strGeneraPagare = mb_strtoupper(trim($this->input->post('strGeneraPagare'))); 
		$objDocumentoPago->intUsuarioID = $this->session->userdata('usuario_id');		
 		//Definir las reglas de validación
		//Validar que la descripción sea única
        if (($objDocumentoPago->intDocumentoPagoID == '') OR 
        	($objDocumentoPago->strDescripcionAnterior != $objDocumentoPago->strDescripcion))
        {

        	$this->form_validation->set_rules('strDescripcion', 'documento', 
        									  'required|is_unique[documentos_pagos.descripcion]');
        }
        else
        {
		...
		else
		{
			...
			if (is_numeric($objDocumentoPago->intDocumentoPagoID))
			{
				$bolResultado = $this->documentos->modificar($objDocumentoPago);
			}
			else
			{ 
				$bolResultado = $this->documentos->guardar($objDocumentoPago);
			}
	...
	{
	    //Definir las reglas de validación
		$this->form_validation->set_rules('intDocumentos_pagosID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intDocumentos_pagosID');
	...
	public function autocomplete()
	{
		...
		if(isset($strDescripcion))
		{
			...
			$otdResultado = $this->documentos->autocomplete($strDescripcion);
	    	...
	    	if ($otdResultado)
			{
				...
				{
		        	$arrDatos[] = array(
		        						'value' => $arrCol->descripcion, 
		        						'data' => $arrCol->documento_pago_id
		        						);
				}
	    	}
	...
	{	            
		...
		$pdf->strLinea1 =  'LISTADO DE DOCUMENTOS PAGOS';
		//Crea los titulos de la cabecera
		$pdf->arrCabecera = array('DOCUMENTO', 'GENERA PAGARE','ESTATUS');
		//Establece el ancho de las columnas de cabecera
		$pdf->arrAnchura = array(140, 30, 20);
		//Establece la alineación de las celdas de la tabla
		$pdf->arrAlineacion = array('L', 'C', 'C');
		...
		{	
			...
			{ 
				...
				$pdf->Row(array(utf8_decode($arrCol->descripcion), $arrCol->genera_pagare ,$arrCol->estatus), 
								$pdf->arrAlineacion);
		...
		$pdf->Output('documentos_pagos.pdf','I'); 
	}
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE DOCUMENTOS PAGOS');
		//Se agregan las columnas de cabecera
        $objExcel->setActiveSheetIndex(0)
                 ->setCellValue('A'.$intPosEncabezados, 'DESCRIPCIÓN')
                 ->setCellValue('B'.$intPosEncabezados, 'GENERA PAGARE')
		...
		{	
			...
			{   
				...
				$objExcel->setActiveSheetIndex(0)
                         ...
                         ->setCellValue('B'.$intFila, $arrCol->genera_pagare)
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'documentos_pagos.xls', 'documentos', $intFila);
	}
}
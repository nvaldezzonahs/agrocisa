...
class Notas_cargo_digitales extends MY_Controller {
	...
	function __construct()
	{
	    ...
	    $this->archivo['strCarpetaDestino'] = './archivos/notas_cargo_digitales/';
	    //Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Cargamos el modelo de notas de cargo digitales
		$this->load->model('cuentas_cobrar/notas_cargo_digitales_model', 'notas');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('cuentas_cobrar/notas_cargo_digitales', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		foreach ($result['notas'] as $arrDet)
		{
			...
			$strNombreCarpeta = $this->archivo['strCarpetaDestino'].$arrDet->nota_cargo_digital_id;
	...
	{ 
		...
		$objNotaCargoDigital = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objNotaCargoDigital->intNotaCargoDigitalID = $this->input->post('intNotaCargoDigitalID');
		$objNotaCargoDigital->dteFecha = $this->input->post('dteFecha');
		$objNotaCargoDigital->intMonedaID = $this->input->post('intMonedaID');
		$objNotaCargoDigital->intTipoCambio = $this->input->post('intTipoCambio');
		$objNotaCargoDigital->intProspectoID = $this->input->post('intProspectoID');
		$objNotaCargoDigital->strRazonSocial =  mb_strtoupper(trim($this->input->post('strRazonSocial')));
		$objNotaCargoDigital->strRfc =  mb_strtoupper(trim($this->input->post('strRfc')));
		$objNotaCargoDigital->intRegimenFiscalID = (($this->input->post('intRegimenFiscalID') !== '') ? 
							          			   	 $this->input->post('intRegimenFiscalID') : NULL);
		$objNotaCargoDigital->strCalle =  mb_strtoupper(trim($this->input->post('strCalle')));
		$objNotaCargoDigital->strNumeroExterior = mb_strtoupper($this->input->post('strNumeroExterior'));
		$objNotaCargoDigital->strNumeroInterior = mb_strtoupper($this->input->post('strNumeroInterior'));
		$objNotaCargoDigital->strCodigoPostal = $this->input->post('strCodigoPostal');
		$objNotaCargoDigital->strColonia =  mb_strtoupper(trim($this->input->post('strColonia')));
		$objNotaCargoDigital->strLocalidad =  mb_strtoupper(trim($this->input->post('strLocalidad')));
		$objNotaCargoDigital->strMunicipio = mb_strtoupper(trim($this->input->post('strMunicipio')));
		$objNotaCargoDigital->strEstado =  mb_strtoupper(trim($this->input->post('strEstado')));
		$objNotaCargoDigital->strPais = mb_strtoupper(trim($this->input->post('strPais')));
		$objNotaCargoDigital->intFormaPagoID = $this->input->post('intFormaPagoID');
		$objNotaCargoDigital->intMetodoPagoID = $this->input->post('intMetodoPagoID');
		$objNotaCargoDigital->intUsoCfdiID = $this->input->post('intUsoCfdiID');
		//Si no existe id del tipo de relación asignar valor nulo
		$objNotaCargoDigital->intTipoRelacionID = (($this->input->post('intTipoRelacionID') !== '') ? 
						   	   						  $this->input->post('intTipoRelacionID') : NULL);
		//Si no existe id de la exportación asignar valor nulo
		$objNotaCargoDigital->intExportacionID = (($this->input->post('intExportacionID') !== '') ? 
						   	   					  $this->input->post('intExportacionID') : NULL);

		$objNotaCargoDigital->strObservaciones = mb_strtoupper(trim($this->input->post('strObservaciones')));
	    $objNotaCargoDigital->intSucursalID = $this->session->userdata('sucursal_id');
		$objNotaCargoDigital->intUsuarioID = $this->session->userdata('usuario_id');
		//Datos de los detalles
		$objNotaCargoDigital->strReferencias = $this->input->post('strReferencias');
		$objNotaCargoDigital->strReferenciaID = $this->input->post('strReferenciaID');
		$objNotaCargoDigital->strConceptos = $this->input->post('strConceptos');
		$objNotaCargoDigital->strObjetoImpuestoSat = $this->input->post('strObjetoImpuestoSat');
		$objNotaCargoDigital->strPrecios = $this->input->post('strPrecios'); 
		$objNotaCargoDigital->strTasaCuotaIva = $this->input->post('strTasaCuotaIva'); 
		$objNotaCargoDigital->strIvas = $this->input->post('strIvas');
		$objNotaCargoDigital->strTasaCuotaIeps = $this->input->post('strTasaCuotaIeps'); 
		$objNotaCargoDigital->strIeps = $this->input->post('strIeps');
		//Datos de los CFDI relacionados
		$objNotaCargoDigital->strCfdiRelacionado = $this->input->post('strCfdiRelacionado'); 
		$objNotaCargoDigital->strTiposRelacion = $this->input->post('strTiposRelacion'); 
		...
		if (is_numeric($objNotaCargoDigital->intNotaCargoDigitalID))
		{
			$bolResultado = $this->notas->modificar($objNotaCargoDigital);
		}
		else
		{ 
			//Hacer un llamado a la función para generar el folio consecutivo del proceso
			$objNotaCargoDigital->strFolio = $this->get_folio_consecutivo($intProcesoMenuID, 10);
			//Si no existe folio del proceso
			if($objNotaCargoDigital->strFolio == '')
			{
			...
			{
				$bolResultado = $this->notas->guardar($objNotaCargoDigital); 

				/*Quitar '_'  de la cadena (resultadoTransaccion_notaCargoDigitalID) para obtener el resultado de la transacción del movimiento en la BD y el id del registro 
				 */
			    list($bolResultado, $objNotaCargoDigital->intNotaCargoDigitalID) = explode("_", $bolResultado);
			}
		}
		...
		if($bolResultado !== NULL)
		{
			...
			{ 
				...
				$arrDatos = array('resultado' => TRUE,
							 	  ...
							 	  'nota_cargo_digital_id' => $objNotaCargoDigital->intNotaCargoDigitalID,
							      'mensaje' => MSJ_GUARDAR);
			}
			else
			{
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intNotaCargoDigitalID');
	    	...
	    	if ($otdResultado)
			{
				...
				foreach ($otdResultado as $arrCol)
				{
		        	$arrDatos[] = array('value' => $arrCol->folio, 
		        						'data' => $arrCol->nota_cargo_digital_id, 
		        						'uuid' => $arrCol->uuid);
				}
	    	}
	...
	{	            
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE NOTAS DE CARGO DIGITALES ').$strTituloRangoFechas;
		...
		{	
			...
			{ 
				...
				{
					//Seleccionar los detalles del registro
					$otdDetalles = $this->notas->buscar_detalles($arrCol->nota_cargo_digital_id);
					//Hacer un llamado a la función para obtener detalles de la referencia
		...
		$pdf->Output('notas_cargo_digitales.pdf','I'); 
	}
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE NOTAS DE CARGO DIGITALES '.$strTituloRangoFechas);
        ...
        if($strDetalles == 'SI')
        {
        	 //Combinar las siguientes celdas
	       	$objExcel->setActiveSheetIndex(0)
			         ...
			         ->setCellValue('AA'.$intPosEncabezados, 'CARGO')
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
						         ...
						         ->setCellValue('AA'.$intFila, $arrDetalles[$intContDet]['cargo'])
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'notas_cargo_digitales.xls', 'notas de cargo', $intFila);
	}

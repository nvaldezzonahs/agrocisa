...
class Documentos_clientes extends MY_Controller {
	...
	function __construct()
	{
		...
		//Cargamos el modelo
		$this->load->model('cuentas_cobrar/documentos_clientes_model', 'documentos');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('cuentas_cobrar/documentos_clientes', $arrDatos);
	}
	...
	public function get_activos()
	{
		...
		$strTipoPersona = $this->input->post('strTipoPersona');
		...
		$result = $this->documentos->buscar(NULL, NULL, NULL, 'ACTIVO', $strTipoPersona);
		...
		foreach ($result as $arrDet)
		{
			...
			//Si se envía el id buscamos los archivos correspondientes
			if ($intID > 0)
			{ 
				...
				if($strTipo == 'Cliente')
				{
		    		...
		    		$strNombreArchivo = $this->get_verifar_archivo_registro($strNombreCarpeta, 
		    															    $arrDet->documento_cliente_id);
				}
			}
	...
	{ 
		...
		$objDocumentoCliente = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objDocumentoCliente->intDocumentoClienteID = $this->input->post('intDocumentoClienteID');
		$objDocumentoCliente->strDescripcion = mb_strtoupper(trim($this->input->post('strDescripcion')));
		$objDocumentoCliente->strDescripcionAnterior = mb_strtoupper(trim($this->input->post('strDescripcionAnterior')));
		$objDocumentoCliente->strSolicitar = $this->input->post('strSolicitar');
		$objDocumentoCliente->intUsuarioID = $this->session->userdata('usuario_id');

        //Definir las reglas de validación
		//Validar que la descripción sea única
        if (($objDocumentoCliente->intDocumentoClienteID == '') OR 
        	($objDocumentoCliente->strDescripcionAnterior != $objDocumentoCliente->strDescripcion))
        {

        	$this->form_validation->set_rules('strDescripcion', 'documento', 
        									  'required|is_unique[documentos_clientes.descripcion]');
        }
        else
        {
		...
		else
		{
			...
			if (is_numeric($objDocumentoCliente->intDocumentoClienteID))
			{
				$bolResultado = $this->documentos->modificar($objDocumentoCliente);
			}
			else
			{ 
				$bolResultado = $this->documentos->guardar($objDocumentoCliente);
			}
	...
	{
	    //Definir las reglas de validación
		$this->form_validation->set_rules('intDocumentoClienteID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intDocumentoClienteID');
		    $strEstatus = $this->input->post('strEstatus');
	...
	{	            
		...
		$pdf->arrCabecera = array('DOCUMENTO', 'SOLICITAR', 'ESTATUS');
		...
		{	
			...
			{ 
				...
				$pdf->Row(array(utf8_decode($arrCol->descripcion), 
								$arrCol->solicitar, $arrCol->estatus), 
								$pdf->arrAlineacion);
	...
	{	
        ...
        $objExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue('B'.$intPosEncabezados, 'SOLICITAR')
		...
		{	
			...
			{   
				...
				$objExcel->setActiveSheetIndex(0)
                         ...
                         ->setCellValue('B'.$intFila, $arrCol->solicitar)
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'documentos_clientes.xls', 'documentos', $intFila);
	}
}
...
class Ordenes_compra extends MY_Controller {
	...
	function __construct()
	{
	    ...
	    $this->archivo['strCarpetaDestino'] = './archivos/ordenes_compra/';
	    //Asignar ruta de la carpeta temporal
	    $this->archivo['strCarpetaTemporal'] = './orden_compra_temporal_';
	    //Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Cargamos el modelo de ordenes de compra
		$this->load->model('cuentas_pagar/ordenes_compra_model', 'ordenes');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('cuentas_pagar/ordenes_compra', $arrDatos);
	}
	...
	*********************************************************************************************************************/
	...
	public function get_paginacion()
	{
		...
		foreach ($result['ordenes'] as $arrDet)
		{
            ...
            $intOrdenCompraID = $arrDet->orden_compra_id;
            //Concatenar id del registro que hace referencia a la carpeta donde se encuentra el archivo
			$strNombreCarpeta = $this->archivo['strCarpetaDestino'].$intOrdenCompraID;

            //Asignar el nombre del archivo que le corresponde al registro
		    $strNombreArchivo = $this->get_verifar_archivo_registro($strNombreCarpeta, $intOrdenCompraID);
	...
	{ 
		...
		$objOrdenCompra->intOrdenCompraID = $this->input->post('intOrdenCompraID');
		...
		$objOrdenCompra->intSucursalID = (($this->input->post('intSucursalID') !== '') ? 
										  $this->input->post('intSucursalID') : NULL);

		//Si no existe id del departamento asignar valor nulo
		$objOrdenCompra->intDepartamentoID = (($this->input->post('intDepartamentoID') !== '') ? 
										       $this->input->post('intDepartamentoID') : NULL);
		...
		$objOrdenCompra->strSucursalID = $this->input->post('strSucursalID'); 
		$objOrdenCompra->strModuloID = $this->input->post('strModuloID'); 
		$objOrdenCompra->strGastoTipoID = $this->input->post('strGastoTipoID'); 
		$objOrdenCompra->strVehiculoID = $this->input->post('strVehiculoID'); 
		...
		$objOrdenCompra->strRetencionIsr = $this->input->post('strRetencionIsr'); 
		$objOrdenCompra->strRetencionesIsrUnitarios = $this->input->post('strRetencionesIsrUnitarios'); 
		$objOrdenCompra->strRetencionIva = $this->input->post('strRetencionIva'); 
		$objOrdenCompra->strRetencionesIvaUnitarios = $this->input->post('strRetencionesIvaUnitarios'); 
		...
		if (is_numeric($objOrdenCompra->intOrdenCompraID))
		{
		...
		{ 
			...
			else
			{
				 ...
				 */
			    list($bolResultado, $objOrdenCompra->intOrdenCompraID, $objOrdenCompra->strFolioConsecutivo) = explode("_", $bolResultado); 
			}
		}
		...
		if($bolResultado !== NULL)
		{
			...
			if($bolResultado)
			{
				...
				$arrDatos = array('resultado' => TRUE,
							 	  ...
							 	  'orden_compra_id' => $objOrdenCompra->intOrdenCompraID,
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intOrdenCompraID');
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intOrdenCompraID', 'ID', 'required|integer');
	...
	public function autocomplete()
	{
		...
		if(isset($strDescripcion))
		{
			...
			//Hacer un llamado al método para obtener todos los registros (autorizados) 
			//que coincidan con la descripción
			$otdResultado = $this->ordenes->autocomplete($strDescripcion, $strTipo);
	    	...
	    	if ($otdResultado)
			{
				...
				{	
					//Dependiendo del tipo agrega datos al array
					if($strTipo == 'tasas')
					{
						//Variable que se utiliza para asignar el id de la orden de compra
						$intReferenciaID = $arrCol->referencia_id;
						//Variable que se utiliza para asignar el tipo de referencia (GENERAL/MAQUINARIA/REFACCIONES/ESPECIAL) de la orden de compra
						...
						if($otdImportesTasa)
						{
							...
							if (($otdImportesTasa->saldo >= 1) OR ($otdImportesTasa->saldo <= -1))
							{
								...
								$arrDatos[] = array('value' => $arrCol->orden_compra, 
					        					    'data' => $arrCol->referencia_id, 
					        						'proveedor_id' => $arrCol->proveedor_id,
					        						'proveedor' => $arrCol->proveedor,
					        						'factura' => $arrCol->factura, 
					        						'subtotal' => number_format($otdImportesTasa->saldo_subtotal,
					        												    $this->intNumDecimalesPrecioUnitario), 
					        						'iva' => number_format($otdImportesTasa->saldo_iva,
					        											   $this->intNumDecimalesIvaUnitario, '.',''),
					        						'ieps' => number_format($otdImportesTasa->saldo_ieps, 
					        												$this->intNumDecimalesIepsUnitario, '.',''),
					        						'total' => number_format($otdImportesTasa->saldo,
					        												 $this->intNumDecimales, '.',''),
	...
	{
		//Variables que se utilizan para recuperar los valores de la vista 
		$intOrdenCompraID = $_POST["intOrdenCompraID_ordenes_compra_cuentas_pagar"];
		$strBotonArchivoID = $_FILES["archivo_varios_ordenes_compra_cuentas_pagar"];
		...
		$this->subir_archivo_reg($strBotonArchivoID, $this->archivo['strCarpetaPrincipal'], 
							     ...
							     $strNombreCarpeta, $intOrdenCompraID, TRUE);
	}
    ...
    public function descargar_archivos()
	{
		...
		$strNombreCarpetaZIP = 'OC_'.$strFolio.'.zip';
	...
	public function enviar_correo_electronico_proveedor()
	{
        ...
        $strRuta = $this->get_reporte_registro($intOrdenCompraID, 'SI');

         //Array que se utiliza para enviar correo electrónico
		$arrDatos =  array('intReferenciaID' => $intOrdenCompraID,
						   'strTitulo' => utf8_decode('Solicitud de Orden de Compra'),
						   'strRuta' => $strRuta,
	...
	public function eliminar_carpeta_temporal($intOrdenCompraID)
	{
		//Definir ubicación de la carpeta
		$strNombreCarpeta = $this->archivo['strCarpetaTemporal'].$intOrdenCompraID; 
	...
	{	 
		...
		//Array que se utiliza para asignar la retención de ISR por estatus
		$arrRetencionIsrEstatus = array();
		//Array que se utiliza para asignar la retención de IVA por estatus
		$arrRetencionIvaEstatus = array(); 
		...
		$arrIepsMoneda = array(); 
		//Array que se utiliza para asignar la retención de ISR por moneda
		$arrRetencionIsrMoneda = array(); 
		//Array que se utiliza para asignar la retención de IVA por moneda
		$arrRetencionIvaMoneda = array(); 
		...
		//Variable que se utiliza para asignar el acumulado de la retención de ISR por estatus
		$intAcumRetencionIsrEstatus = 0;
		//Variable que se utiliza para asignar el acumulado de la retención de IVA por estatus
		$intAcumRetencionIvaEstatus = 0;
		...
		$pdf->strLinea1 =  'LISTADO DE ORDENES DE COMPRA '.$strTituloRangoFechas;
		...
		$arrAnchuraDetalles = array(16, 59, 35, 20, 20, 20, 20);
	   ...
	   //Establece el ancho de las columnas de la tabla tipo de gasto
		$arrAnchuraTipoGasto =  array(40, 35, 40, 75);
		...
		{	
			...
			foreach ($arrEstatus as $arrEst)
			{
				...
				$arrRetencionIsrEstatus[$arrEst] = 0;
				$arrRetencionIvaEstatus[$arrEst] = 0;
			...
			foreach ($otdMonedas as $arrMon)
			{
				...
				foreach ($arrEstatus as $arrEst)
				{
					...
					$arrRetencionIsrMoneda[$arrMon->moneda_id][$arrEst] = 0;
					$arrRetencionIvaMoneda[$arrMon->moneda_id][$arrEst] = 0;
			...
			{ 
				...
				$intAcumIeps = 0;
				//Variable que se utiliza para asignar el acumulado de la retención de ISR
				$intAcumRetencionIsr = 0;
				//Variable que se utiliza para asignar el acumulado de la retención de IVA
				$intAcumRetencionIva = 0;
				//Asignar el importe retenido de ISR (proveedor)
				$intRetencionIsrProv = ($arrCol->importe_retenido / $arrCol->tipo_cambio);

				//Seleccionar los detalles del registro
				$otdDetalles = $this->ordenes->buscar_detalles($arrCol->orden_compra_id);
				...
				if($otdDetalles)
				{
					...
					foreach ($otdDetalles as $arrDet)
					{
						...
						$intRetencionIsrUnitario = ($arrDet->retencion_isr_unitario / $arrCol->tipo_cambio);
						$intRetencionIvaUnitario = ($arrDet->retencion_iva_unitario / $arrCol->tipo_cambio);
						...
						$intImporteRetencionIsr = 0;
						//Variable que se utiliza para asignar el importe de la retención de ISR con formato moneda
						$strImporteRetencionIsr = '';
						//Variable que se utiliza para asignar el importe de la retención de IVA
						$intImporteRetencionIva = 0;
						//Variable que se utiliza para asignar el importe de la retención de IVA con formato moneda
					    $strImporteRetencionIva = '';
						...
						//Si existe importe de la retención de ISR unitario
						if($intRetencionIsrUnitario > 0)
						{
						    ...
						    //Asignar importe de la retención con formato de moneda
						    $strImporteRetencionIsr = '$'.number_format($intImporteRetencionIsr,$this->intNumDecimales);
						}


						//Si existe importe de la retención de IVA unitario
						if($intRetencionIvaUnitario > 0)
						{
						    ...
						    //Asignar importe de la retención con formato de moneda
						    $strImporteRetencionIva = '$'.number_format($intRetencionIvaUnitario,$this->intNumDecimales);
						}
		                ...
		                $arrAuxiliar["retencion_isr"] =  $strImporteRetencionIsr;
		                $arrAuxiliar["retencion_iva"] = $strImporteRetencionIva;
						...
						$intAcumRetencionIsr += $intImporteRetencionIsr;
						$intAcumRetencionIva += $intImporteRetencionIva;
					}
				...
				$intTotal -= $intAcumRetencionIsr;
				//Decrementar importe de la retención de IVA
				$intTotal -= $intAcumRetencionIva;
				...
				$arrSubtotalEstatus[$arrCol->estatus] += ($intAcumSubtotal * $arrCol->tipo_cambio);
		      	...
		      	$arrRetencionIsrEstatus[$arrCol->estatus] += ($intAcumRetencionIsr * $arrCol->tipo_cambio);
		      	$arrRetencionIvaEstatus[$arrCol->estatus] += ($intAcumRetencionIva * $arrCol->tipo_cambio);
		      	...
		      	if($arrCol->moneda_id != MONEDA_BASE)
		      	{
			      	...
			      	$arrRetencionIsrMoneda[$arrCol->moneda_id][$arrCol->estatus] += $intAcumRetencionIsr;
			      	$arrRetencionIvaMoneda[$arrCol->moneda_id][$arrCol->estatus] += $intAcumRetencionIva;
			    ...
			    if($arrCol->sucursal_id > 0)
			    {
			    	//Concatenar datos para cargar a
					$strCargarA = $arrCol->departamento . ', '.$arrCol->sucursal;

			    	//Cargar a 
			   		$pdf->Cell(13, 4, 'CARGAR A:', 0, 0, 'L', 0);
					$pdf->ClippedCell(57, 4, utf8_decode($strCargarA), 0, 0, 'L', 0);
			    }
				...
				if($arrDetalles && $strDetalles == 'SI')
				{
			        ...
			        foreach ($arrDetalles as $arrDet) 
			        {
					    ...
					    $pdf->Row(array($arrDet['cantidad'], $arrDet['concepto'], 
					    				$arrDet['gasto'], $arrDet['precio_unitario'],  
					    				$arrDet['subtotal'], $arrDet['retencion_isr'], 
					    				$arrDet['retencion_iva']), $arrAlineacionDetalles, 'ClippedCell');
			...
			//Crea los titulos de la cabecera
			$arrCabeceraResumen = array('ESTATUS', 'SUBTOTAL', 'IVA', 'IEPS', 'RET. DE ISR',
										'RET. DE IVA', 'TOTAL');
			//Establece el ancho de las columnas de cabecera
			...
			foreach ($arrEstatus as $arrEst)
			{
				...
				if($arrSubtotalEstatus[$arrEst] > 0)
				{
					...
					$pdf->Row(array($arrEst,
			    				    ...
			    				    '$'.number_format($arrRetencionIsrEstatus[$arrEst],$this->intNumDecimales), 
			    				    '$'.number_format($arrRetencionIvaEstatus[$arrEst],$this->intNumDecimales), 
					...
					if($arrEst != 'INACTIVO' &&  $arrEst != 'RECHAZADO')
					{
						...
						$intAcumRetencionIsrEstatus += $arrRetencionIsrEstatus[$arrEst];
						$intAcumRetencionIvaEstatus += $arrRetencionIvaEstatus[$arrEst];
           ...
           //Acumulado del IEPS
            $pdf->Cell(25,3,'$'.number_format($intAcumIepsEstatus,$this->intNumDecimales), 0, 0, 'R');
             //Acumulado de la retención de ISR
            $pdf->Cell(25,3,'$'.number_format($intAcumRetencionIsrEstatus,$this->intNumDecimales), 0, 0, 'R');
             //Acumulado de la retención de IVA
            $pdf->Cell(25,3,'$'.number_format($intAcumRetencionIvaEstatus,$this->intNumDecimales), 0, 0, 'R');
			...
			foreach ($otdMonedas as $arrMon) 
			{
				...
				$pdf->SetTextColor(COLOR_TEXTO); //establece el color de texto
				//Creación de la tabla Resumen
				$pdf->ClippedCell(175, 4, 'RESUMEN POR MONEDA '.$arrMon->descripcion, 0, 0, 'C', TRUE);
			    $pdf->Ln(4);//Deja un salto de linea
				//Recorre el array de titulos de encabezado para crearlos
				for ($intCont = 0; $intCont < count($arrCabeceraResumen); $intCont++)
				{
				...
				foreach ($arrEstatus as $arrEst)
				{
					...
					if($arrSubtotalMoneda[$arrMon->moneda_id][$arrEst] > 0)
					{
						...
						$pdf->Row(array($arrEst,
				    				    ...
				    				    '$'.number_format($arrRetencionIsrMoneda[$arrMon->moneda_id][$arrEst],$this->intNumDecimales),
				    				    '$'.number_format($arrRetencionIvaMoneda[$arrMon->moneda_id][$arrEst],$this->intNumDecimales),
						...
						if($arrEst != 'INACTIVO' &&  $arrEst != 'RECHAZADO')
						{
							...
							$intAcumRetencionIsrEstatus += $arrRetencionIsrMoneda[$arrMon->moneda_id][$arrEst];
							$intAcumRetencionIvaEstatus += $arrRetencionIvaMoneda[$arrMon->moneda_id][$arrEst];
		...
		$pdf->Output('ordenes_compra.pdf','I'); 
	}

	//Método para generar un reporte PDF con la información de un registro 
	public function get_reporte_registro($intOrdenCompraID  = NULL, $strEnviarCorreo = NULL) 
	{	            
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->ordenes->buscar($intOrdenCompraID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->ordenes->buscar_detalles($intOrdenCompraID);
		...
		//Anchura especificada para la impresion del renglon correspondiente a retenciones
		$arrAnchuraRetenciones = array(18,175);
		$arrAlineacionRetenciones = array('L', 'L');
		...
		$strNombreArchivo  = 'orden_compra_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, 'ORDEN DE COMPRA', 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
			...
			if($otdResultado->sucursal_id > 0)
			{
				...
				$pdf->SetXY(124, 58);
				//Concatenar datos para cargar a
				$strCargarA = $otdResultado->departamento . ', '.$otdResultado->sucursal;
				$pdf->ClippedCell(60, 3, utf8_decode($strCargarA));
			}
			...
			if($otdDetalles)
			{
				...
				//Establece el ancho de las columnas de la tabla tipo de gasto
				$arrAnchuraTipoGasto = array(40, 35, 40, 70);
				//Establece la alineación de las celdas de la tabla tipo de gasto
				$arrAlineacionTipoGasto = array('L', 'L', 'L', 'L');
				...
				{ 
					...
					$intRetencionIsrUnitario = ($arrDet->retencion_isr_unitario / $otdResultado->tipo_cambio);
					$intRetencionIvaUnitario = ($arrDet->retencion_iva_unitario / $otdResultado->tipo_cambio);
					$intSubTotalUnitario = $intPrecioUnitario;
					//Convertir cantidad a seis decimales
					$intPorcentajeIva = number_format($arrDet->porcentaje_iva, 6, '.', '');
					$intPorcentajeIeps = number_format($arrDet->porcentaje_ieps, 6, '.', '');
					...
					if($intRetencionIsrUnitario > 0)
					{
					    ...
					    $arrISR[$numEleISR][0] = $arrDet->retencion_isr;
						$arrISR[$numEleISR][1] = $intImporteRetencionIsr;
						$numEleISR++;
					}
					...
					if($intRetencionIvaUnitario > 0)
					{
					    ...
					    $arrIVA[$numEleIVA][0] = $arrDet->retencion_iva;
						$arrIVA[$numEleIVA][1] = $intImporteRetencionIva;
						$numEleIVA++;
					}
					...
					//---------- ISR
					for ($intConISR = 0; $intConISR < $numEleISR; $intConISR++)
					{
						$strCadenaRetenciones.= "Impuesto:";//--a. Impuesto
						$strCadenaRetenciones.= $arrISR[$intConISR][0]." ISR|";//--a. Impuesto
						$strCadenaRetenciones.= "Importe:";//--d. Importe
						$strCadenaRetenciones.= number_format($arrISR[$intConISR][1], 2, '.', '');//--d. Importe
					}

					//---------- IVA
					for ($intConIVA = 0; $intConIVA < $numEleIVA; $intConIVA++)
					{
						//Si existe retención de ISR
						if($numEleISR > 0)
						{
							$strCadenaRetenciones.= "|";
						}
						...
						$strCadenaRetenciones.= "Impuesto:";//--a. Impuesto
						$strCadenaRetenciones.= $arrIVA[$intConIVA][0]." IVA|";//--a. Impuesto
						$strCadenaRetenciones.= "Importe:";//--d. Importe
						$strCadenaRetenciones.= number_format($arrIVA[$intConIVA][1], 2, '.', '');//--d. Importe
					}

					//Si se cumple la sentencia
					if($strCadenaRetenciones != '')
					{
						//Asignar posición
						$pdf->SetX(15);
						//Establece el ancho de las columnas
						$pdf->SetWidths($arrAnchuraRetenciones);
						$pdf->Row(array('Retenciones:|Negrita', $strCadenaRetenciones.'|Negrita'),  
									    $arrAlineacionRetenciones, 'ClippedCell');
				...
				{
					//Decrementar uno para obtener última tasa de ISR
					 $intIndiceIsr = $numEleISR - 1;
					//Retención de ISR
					//Asigna el tipo y tamaño de letra
					$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
					$pdf->SetX(135);
					$pdf->ClippedCell(50, 3, utf8_decode('RET. ISR'). ' '.$arrISR[$intIndiceIsr][0]);
					//Asigna el tipo y tamaño de letra
					$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NORMAL, TAMANO_LETRA_TITULO_TABLA_PDF);
					$pdf->SetX(175);
					$pdf->ClippedCell(25, 3, '$'.number_format($intAcumRetencionIsr,$this->intNumDecimales), 0, 0, 'R');
					$pdf->Ln(); //Deja un salto de línea
				...
				{
					//Decrementar uno para obtener última tasa de IVA
					 $intIndiceIva = $numEleIVA - 1;

					//Retención de IVA
					//Asigna el tipo y tamaño de letra
					$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
					$pdf->SetX(135);
					$pdf->ClippedCell(50, 3, utf8_decode('RET. IVA').' '.$arrIVA[$intIndiceIva][0]);
					//Asigna el tipo y tamaño de letra
					$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NORMAL, TAMANO_LETRA_TITULO_TABLA_PDF);
					$pdf->SetX(175);
					$pdf->ClippedCell(25, 3, '$'.number_format($intAcumRetencionIva,$this->intNumDecimales), 0, 0, 'R');
					$pdf->Ln(); //Deja un salto de línea
		...
		{	
            //Definir ubicación de la carpeta
			$strCarpetaDestino =  $this->archivo['strCarpetaTemporal'].$intOrdenCompraID.'/'; 
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE ORDENES DE COMPRA '.$strTituloRangoFechas);
		//Si existe id del proveedor
		if($intProveedorID > 0)
		{   //Seleccionar los datos del proveedor que coincide con el id
			$otdProveedor =  $this->proveedores->buscar($intProveedorID);
        ...
        $objExcel->setActiveSheetIndex(0)
        		 ...
        		 ->setCellValue('H'.$intPosEncabezados, 'RETENCIÓN DE ISR')
        		 ->setCellValue('I'.$intPosEncabezados, 'RETENCIÓN DE IVA')
        		 ->setCellValue('J'.$intPosEncabezados, 'TOTAL')
        		 ->setCellValue('K'.$intPosEncabezados, 'MONEDA')
        		 ->setCellValue('L'.$intPosEncabezados, 'T.C.')
        		 ->setCellValue('M'.$intPosEncabezados, 'ENTREGA')
        		 ->setCellValue('N'.$intPosEncabezados, 'VENCIMIENTO')
        		 ->setCellValue('O'.$intPosEncabezados, 'CONDICIONES')
        		 ->setCellValue('P'.$intPosEncabezados, 'CARGAR A')
        ...
        if($strDetalles == 'SI')
        {
        	 //Combinar las siguientes celdas
	       	$objExcel->setActiveSheetIndex(0)
			         ...
			         ->setCellValue('V'.$intPosEncabezados, 'SUCURSAL')
			         ->setCellValue('W'.$intPosEncabezados, 'DEPARTAMENTO')
			         ->setCellValue('X'.$intPosEncabezados, 'GASTO')
			         ->setCellValue('Y'.$intPosEncabezados, 'VEHÍCULO')
			         ->setCellValue('Z'.$intPosEncabezados,'PRECIO UNITARIO')
			         ->setCellValue('AA'.$intPosEncabezados, 'SUBTOTAL');

        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('S'.$intPosEncabezados.':AA'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
		...
		{	
			...
			{   
				...
				if($otdDetalles)
				{
					...
					foreach ($otdDetalles as $arrDet)
					{
			        	...
			        	$arrDetalles[$intContDetOrd]['tipo_gasto'] = $arrDet->tipo_gasto;
			        	$arrDetalles[$intContDetOrd]['sucursal'] = $arrDet->sucursal;
			        	$arrDetalles[$intContDetOrd]['modulo'] = $arrDet->modulo;
			        	$arrDetalles[$intContDetOrd]['gasto'] = $arrDet->gasto;
			        	$arrDetalles[$intContDetOrd]['vehiculo'] = $strVehiculo;
				...
				//Variable que se utiliza para concatenar los datos del departamento y la sucursal
				$strCargarA = '';
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					$objExcel->setActiveSheetIndex(0)
	                         ...
	                         ->setCellValue('H'.$intFila, $intAcumRetencionIsr)
	                         ->setCellValue('I'.$intFila, $intAcumRetencionIva)
	                         ->setCellValue('J'.$intFila, $intTotal)
	                         ->setCellValue('K'.$intFila, $arrCol->moneda)
	                         ->setCellValue('L'.$intFila, $arrCol->tipo_cambio)
	                         ->setCellValue('M'.$intFila, $arrCol->fecha_entrega)
	                         ->setCellValue('N'.$intFila, $arrCol->fecha_vencimiento)
	                         ->setCellValue('O'.$intFila, $arrCol->condiciones_pago)
	                         ->setCellValue('P'.$intFila, $strCargarA)
					...
					if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
						         ...
						         ->setCellValue('V'.$intFila, $arrDetalles[$intContDet]['sucursal'])
						         ->setCellValue('W'.$intFila, $arrDetalles[$intContDet]['modulo'])
						         ->setCellValue('X'.$intFila, $arrDetalles[$intContDet]['gasto'])
						         ->setCellValue('Y'.$intFila, $arrDetalles[$intContDet]['vehiculo'])
						         ->setCellValue('Z'.$intFila, $arrDetalles[$intContDet]['precio_unitario'])
						         ->setCellValue('AA'.$intFila, $arrDetalles[$intContDet]['subtotal']);
					}
            ...
            $objExcel->getActiveSheet()
           	...
           	$objExcel->getActiveSheet()
            		 ->getStyle('Z'.$intFilaInicial.':'.'AA'.$intFila)
            		 ->getNumberFormat()
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'ordenes_compra.xls', 'ordenes de compra', $intFila);
	}

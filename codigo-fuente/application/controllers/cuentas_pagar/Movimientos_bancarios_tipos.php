...
class Movimientos_bancarios_tipos extends MY_Controller {
	//Constructor de la clase
	function __construct()
	{
		parent::__construct();
		//Cargamos el modelo
		$this->load->model('cuentas_pagar/movimientos_bancarios_tipos_model', 'tipos');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('cuentas_pagar/movimientos_bancarios_tipos', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		foreach ($result['tipos_movimientos'] as $arrDet)
		{
		...
		$arrDatos = array('rows' => $result['tipos_movimientos'],
	...
	{ 
		...
		$objMovimientoBancarioTipo = new stdClass();
		//Variables que se utilizan para recuperar los valores de la vista
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objMovimientoBancarioTipo->intMovimientoBancarioTipoID = $this->input->post('intMovimientoBancarioTipoID');
		$objMovimientoBancarioTipo->strDescripcion = mb_strtoupper(trim($this->input->post('strDescripcion')));
		$objMovimientoBancarioTipo->strDescripcionAnterior = mb_strtoupper(trim($this->input->post('strDescripcionAnterior')));
		$objMovimientoBancarioTipo->intUsuarioID = $this->session->userdata('usuario_id');
		
        //Definir las reglas de validación
		//Validar que la descripción sea única
        if (($objMovimientoBancarioTipo->intMovimientoBancarioTipoID == '') OR 
        	($objMovimientoBancarioTipo->strDescripcionAnterior != $objMovimientoBancarioTipo->strDescripcion))
        {
            $this->form_validation->set_rules('strDescripcion', 'descripción',
            								  'required|is_unique[movimientos_bancarios_tipos.descripcion]');
        }
        else
        {
		...
		else
		{
			...
			if (is_numeric($objMovimientoBancarioTipo->intMovimientoBancarioTipoID))
			{
				$bolResultado = $this->tipos->modificar($objMovimientoBancarioTipo);
			}
			else
			{ 
				$bolResultado = $this->tipos->guardar($objMovimientoBancarioTipo);
			}
	...
	{
	    //Definir las reglas de validación
		$this->form_validation->set_rules('intMovimientoBancarioTipoID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intMovimientoBancarioTipoID');
	...
	public function get_combo_box()
	{
		...
		$arrDatos['movimientos_bancarios_tipos'] = $this->tipos->get_combo_box();
		//Enviar datos a la vista del formulario
		$this->output->set_content_type('application/json')->set_output(json_encode($arrDatos));
	}
	...
	{	            
		...
		$pdf->strLinea1= 'LISTADO DE TIPOS DE MOVIMIENTOS BANCARIOS';
		...
		$pdf->Output('tipos_movimientos_bancarios.pdf','I'); 
	}
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE TIPOS DE MOVIMIENTOS BANCARIOS');
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'tipos_movimientos_bancarios.xls', 'tipos', $intFila);
	}
}
...
class Rep_inventario extends MY_Controller {
	...
	{
		parent::__construct();
		//Cargamos el modelo de inventario de refacciones
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/rep_inventario', $arrDatos);
	}
								...
								$strLocalizacionInicial = NULL, $strLocalizacionFinal = NULL) 
	{
		...
		$pdf->arrCabecera = array(utf8_decode('LÍNEA'), utf8_decode('CÓDIGO'), utf8_decode('DESCRIPCIÓN'), 
								 'UNIDAD', utf8_decode('LOCALIZACIÓN'), 'FECHA', 'KARDEX', 'COSTO', 'IMPORTE');
		...
		if($otdExistenciaInicial)
		{
			...
			foreach ($otdExistenciaInicial as $arrEx) 
			{
				...
				if (($strExistencia ==  'SI') AND ($numCantidadActual < 0.01) AND ($numCantidadActual > -0.01))
				{
	...
	//Función que se utiliza para regresar los acumulados de los movimientos (entradas, salidas y facturas) de una refacción

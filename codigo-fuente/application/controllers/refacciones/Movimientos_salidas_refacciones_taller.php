...
class Movimientos_salidas_refacciones_taller extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/movimientos_salidas_refacciones_taller', $arrDatos);
	}
	...
	*********************************************************************************************************************/
	...
	public function get_paginacion()
	{
		...
		$result = $this->movimientos->filtro_salida_taller($this->intTipoMovimiento,
	...
	{ 
		...
		$objMovimiento->strBackOrderID = $this->input->post('strBackOrderID'); 
		...
		if (is_numeric($objMovimiento->intMovimientoRefaccionesID))
		{
			...
			$bolResultado = $this->movimientos->modificar_salida_taller($objMovimiento);
		}
		else
		{ 
			...
			{	
				$bolResultado = $this->movimientos->guardar_salida_taller($objMovimiento); 
	...
	public function get_datos()
	{
	    ...
	    $otdResultado = $this->movimientos->buscar_salida_taller($intID);
	...
	public function set_estatus()
	{
		...
		else
		{
			...
			if($bolResultado)
			{
				//Hacer un llamado al método para modificar el estatus (y el back_order) de la requisición de refacciones
				$bolResultadoRequisicion = $this->set_estatus_requisicion_refacciones($intMovimientoRefaccionesID,
																					  $intReferenciaID);
	...
	{	            
		...
		$otdResultado = $this->movimientos->buscar_salida_taller(NULL, $this->intTipoMovimiento, 
		...
		$otdMonedas = $this->movimientos->buscar_distintas_monedas_salida_taller($this->intTipoMovimiento, 
																				 $dteFechaInicial, 
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE SALIDAS DE REFACCIONES POR TALLER ').$strTituloRangoFechas;
		...
		$pdf->arrCabecera = array('FOLIO', 'CLIENTE', 'FECHA', utf8_decode('REQUISICIÓN'), 
							 utf8_decode('NO. DE ORDEN'), 'TOTAL', 'ESTATUS');
	    ...
	    $arrAlineacionDetalles = array('R', 'R', 'L', 'L', 'L', 'L', 'R', 'R', 'R');
	    //Establece el ancho de las columnas de la tabla detalles
		$arrAnchuraDetalles = array(16, 16, 8, 20, 42, 20, 22, 22, 22 );
		...
		{	
			...
			{ 
				...
				$otdDetalles = $this->movimientos->buscar_detalles_salida_taller($arrCol->movimiento_refacciones_id);
		...
		$pdf->Output('salidas_refacciones_taller.pdf','I'); 
	}
	...
	{	            
		...
		$intMovimientoRefaccionesID = $this->input->post('intMovimientoRefaccionesID');   
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->movimientos->buscar_salida_taller($intMovimientoRefaccionesID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->movimientos->buscar_detalles_salida_taller($intMovimientoRefaccionesID);
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('SALIDA DE REFACCIONES POR TALLER'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
			...
			if($otdDetalles)
			{
				...
				$arrCabecera = array(utf8_decode('Línea'), utf8_decode('Código'), 
									...
									'Solicitado', 'Surtido', 'Costo', 'Subtotal', 'Precio');
				//Establece el ancho de las columnas de cabecera
				$arrAnchura = array(8, 20, 41, 20, 18, 18, 20, 20, 20);
				//Establece la alineación de las celdas de la tabla
			...
			$pdf->SetX(160);
			$pdf->ClippedCell(20, 3, '$'.$intTotal, 0, 0, 'R');
			//Acumulado del precio
			$pdf->SetX(180);
			$pdf->ClippedCell(20, 3, '$'.$intAcumPrecio, 0, 0, 'R');
			$pdf->Ln(); //Deja un salto de línea
			$intPosY = $pdf->GetY();
			//Observaciones
			//Asigna el tipo y tamaño de letra
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE SALIDAS DE REFACCIONES POR TALLER '.$strTituloRangoFechas);
		//Si existe id del prospecto
		if($intProspectoID > 0)
		{   
			//Seleccionar los datos del prospecto que coincide con el id
        ...
        $objExcel->setActiveSheetIndex(0)
        		 ...
        		 ->setCellValue('D'.$intPosEncabezados, 'REQUISICIÓN')
        ...
        if($strDetalles == 'SI')
        {
        	//Combinar las siguientes celdas
	       	$objExcel->setActiveSheetIndex(0)
			         ...
			         ->setCellValue('S'.$intPosEncabezados, 'PRECIO UNITARIO')
			         ->setCellValue('T'.$intPosEncabezados, 'COSTO UNITARIO')
			         ->setCellValue('U'.$intPosEncabezados, 'SUBTOTAL');

        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('K'.$intPosEncabezados.':U'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
			...
			{   
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValueExplicit('D'.$intFila, $arrCol->folio_requisicion, PHPExcel_Cell_DataType::TYPE_STRING)
					...
					if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
						         ...
						         ->setCellValue('S'.$intFila, $arrDetalles[$intContDet]['precio_unitario'])
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'salidas_refacciones_taller.xls', 
	...
	*********************************************************************************************************************/
	...
	public function get_datos_detalles()
	{
	    ...
	    $otdResultado = $this->movimientos->buscar_detalles_salida_taller($intMovimientoRefaccionesID, $intReferenciaID);
		//Si existen datos, asignar los datos recuperados en el array
		if($otdResultado)
		{
	...
	public function get_paginacion_detalles()
	{
		...
		$config['cur_page'] =  $this->input->post('intPagina');
		//Seleccionar los datos que coinciden con los criterios de búsqueda
		$result = $this->movimientos->filtro_detalles_salida_taller($this->intTipoMovimiento,
																	$this->input->post('intOrdenReparacionID'),
										                            $config['per_page'],
										                            $config['cur_page']);
		...
		foreach ($result['detalles'] as $arrDet)
		{
            ...
            $arrDet->precio_unitario = number_format($intPrecioUnitario,$this->intNumDecimales);
	...
	*********************************************************************************************************************/
	//Método para modificar el estatus de un registro
	public function set_estatus_requisicion_refacciones($intMovimientoRefaccionesID, $intRequisicionRefaccionesID)
	{
		...
		//Variable que se utiliza para saber si se actualizó el estatus de la requisición de refacciones
		$bolResultadoRequisicion = FALSE;

		//Seleccionar el total de movimientos activos que coinciden con el id de la requisición de refacciones
		$otdMovimientos = $this->movimientos->buscar_total_movimientos_requisicion_refacciones($this->intTipoMovimiento, 
																							   $intRequisicionRefaccionesID);
		//Asignar el total de movimientos activos 
		$intTotalMovimientos = $otdMovimientos->total_movimientos;
	
		//Si no existen movimientos activos de la requisición de refacciones
		if($intTotalMovimientos == 0)
		{
			//Cambiar el estatus de la requisición de la refacción
		    $strEstatusRequisicion = 'ACTIVO';

			//Hacer un llamado al método para eliminar los datos de los pedidos pendientes (back_order) de la requisición
		    $bolResultadoBackOrder = $this->movimientos->eliminar_pedidos_requisicion_refacciones($intRequisicionRefaccionesID);
		}
		else
		{
			...
			if($otdDetalles)
			{
				...
				{	
					...
					$intBackOrderID = $arrDet->back_order_id;
					$strEstatus = 'ACTIVO';

					//Calcular cantidad pendiente por surtir
					$intCantidad =  $intCantidadPendiente + $intCantidadSurtida;

					//Hacer un llamado al método para modificar los datos del pedido pendiente (back_order)
					$bolResultadoPedido = $this->movimientos->modificar_pedido_pendiente($intBackOrderID, 
																						 $intCantidad, 
																						 $strEstatus);
		...
		if($bolResultadoBackOrder == TRUE)
		{
			...
			$bolResultadoRequisicion = $this->movimientos->set_estatus_requisicion_refacciones($intRequisicionRefaccionesID, 

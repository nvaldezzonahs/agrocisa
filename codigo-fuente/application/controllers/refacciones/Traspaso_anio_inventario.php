...
class Traspaso_anio_inventario extends MY_Controller {
	//Constructor de la clase
	function __construct()
	{
		parent::__construct();
		//Cargamos el modelo
		$this->load->model('refacciones/refacciones_inventario_model', 'inventario');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/traspaso_anio_inventario', $arrDatos);
	}
	...
	//Método para regresar las refacciones con existencia/diferencia en el inventario
	public function get_diferencia_refacciones()
	{
		//Array que se utiliza para enviar datos a la vista
		$arrDatos = array('mensaje_01' => NULL, 
						 'mensaje_02' => NULL);
		//Variables que se utilizan para recuperar los valores de la vista 
		$strAnioInventario = $this->input->post('strAnioInventario');
		$strAnioTraspaso = $this->input->post('strAnioTraspaso');
		
		//Seleccionar las refacciones (año del inventario) que tienen diferencia en existencia y disponibilidad
		$otdDiferenciasAnioInv = $this->inventario->buscar_diferencia_refacciones($strAnioInventario);

		//Seleccionar las refacciones (año del traspaso) que tienen existencia y disponibilidad
		$otdExistenciasAnioTraspaso = $this->inventario->buscar_diferencia_refacciones($strAnioTraspaso, 'existencia');
		
		//Si existen diferencias de existencia en el año del inventario
		if($otdDiferenciasAnioInv)
		{
			$strMensaje = '<b>No es posible realizar el traspaso de año del inventario, existen refacciones con existencia y disponibilidad distintas:</b><br>';

			//Hacer un llamado a la función para asignar lista de refacciones con diferencia de existencia (año del inventario)
			$strMensaje .= $this->get_lista_refacciones($otdDiferenciasAnioInv);


			$arrDatos['mensaje_01'] = $strMensaje;
		}
		...
		if($otdExistenciasAnioTraspaso)
		{
			...
			$arrDatos['mensaje_02'] = $strMensaje;
		}
	...
	public function get_lista_refacciones($otdInventario)
	{
		...
		foreach ($otdInventario as $arrRef)
		{
			...
			$strListaRefacciones .= '<br>';
			$strListaRefacciones .= ' Disponible: '.$arrRef->disponible_existencia;
			$strListaRefacciones .= '<br>';

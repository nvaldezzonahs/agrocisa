...
class Movimientos_salidas_refacciones_consumo_interno extends MY_Controller {
	///Variable para identificar el tipo de movimiento
	var $intTipoMovimiento = SALIDA_REFACCIONES_CONSUMO_INTERNO; 
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/movimientos_salidas_refacciones_consumo_interno', $arrDatos);
	}
	...
	*********************************************************************************************************************/
	...
	{	            
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE SALIDAS DE REFACCIONES POR CONSUMO INTERNO ').$strTituloRangoFechas;
		...
		$pdf->arrCabecera = array('FOLIO', 'DEPARTAMENTO', 'FECHA', 'AUTORIZA', 'TOTAL', 'ESTATUS');
	   ...
	   //Establece el ancho de las columnas de la tabla tipo de gasto
		...
		{	
			...
			{ 
				...
				//Seleccionar los detalles del registro
				$otdDetalles = $this->movimientos->buscar_detalles_salida_consumo_interno($arrCol->movimiento_refacciones_id);
				...
				$pdf->Row(array($arrCol->folio, utf8_decode($arrCol->departamento), 
								$arrCol->fecha, utf8_decode($arrCol->empleado), 
		...
		$pdf->Output('salidas_refacciones_consumo_interno.pdf','I'); 
	}
	...
	{	            
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->movimientos->buscar_salida_consumo_interno($intMovimientoRefaccionesID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->movimientos->buscar_detalles_salida_consumo_interno($intMovimientoRefaccionesID);
		...
		$strNombreArchivo  = 'salida_refacciones_consumo_interno_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('SALIDA DE REFACCIONES POR CONSUMO INTERNO'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE SALIDAS DE REFACCIONES POR CONSUMO INTERNO '.$strTituloRangoFechas);
		//Si existe id del departamento
		if($intDepartamentoID > 0)
		{   //Seleccionar los datos del departamento que coincide con el id
			$otdDepartamento =  $this->departamentos->buscar($intDepartamentoID);
        ...
        $objExcel->setActiveSheetIndex(0)
        		 ...
        		 ->setCellValue('D'.$intPosEncabezados, 'AUTORIZA')
        ...
        if($strDetalles == 'SI')
        {
        	 //Combinar las siguientes celdas
	       	$objExcel->setActiveSheetIndex(0)
                     ...
                     ->setCellValue('I'.$intPosEncabezados, 'CÓDIGO DE LÍNEA')
                     ->setCellValue('J'.$intPosEncabezados, 'CÓDIGO')
			         ->setCellValue('K'.$intPosEncabezados, 'DESCRIPCIÓN')
			         ->setCellValue('L'.$intPosEncabezados, 'REFACCIONES LINEA')
			         ->setCellValue('M'.$intPosEncabezados, 'REFACCIONES MARCA')
			         ->setCellValue('N'.$intPosEncabezados, 'LOCALIZACIÓN')
			         ->setCellValue('O'.$intPosEncabezados, 'TIPO DE GASTO')
			         ->setCellValue('P'.$intPosEncabezados, 'SUCURSAL')
			         ->setCellValue('Q'.$intPosEncabezados, 'DEPARTAMENTO')
			         ->setCellValue('R'.$intPosEncabezados, 'GASTO')
			         ->setCellValue('S'.$intPosEncabezados,'COSTO UNITARIO')
			         ->setCellValue('T'.$intPosEncabezados, 'SUBTOTAL');

        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('H'.$intPosEncabezados.':T'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
			...
			{   
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValueExplicit('D'.$intFila, $arrCol->empleado, PHPExcel_Cell_DataType::TYPE_STRING)
					...
					if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
								 ...
								 ->setCellValueExplicit('I'.$intFila, $arrDetalles[$intContDet]['codigo_linea'], PHPExcel_Cell_DataType::TYPE_STRING)
		                         ->setCellValueExplicit('J'.$intFila, $arrDetalles[$intContDet]['codigo'], PHPExcel_Cell_DataType::TYPE_STRING)
		                         ->setCellValue('K'.$intFila, $arrDetalles[$intContDet]['descripcion'])
		                         ->setCellValue('L'.$intFila, $arrDetalles[$intContDet]['refacciones_linea'])
		                         ->setCellValue('M'.$intFila, $arrDetalles[$intContDet]['refacciones_marca'])
		                         ->setCellValue('N'.$intFila, $arrDetalles[$intContDet]['localizacion'])
		                         ->setCellValue('O'.$intFila, $arrDetalles[$intContDet]['tipo_gasto'])
		                         ->setCellValue('P'.$intFila, $arrDetalles[$intContDet]['sucursal'])
		                         ->setCellValue('Q'.$intFila, $arrDetalles[$intContDet]['modulo'])
		                         ->setCellValue('R'.$intFila, $arrDetalles[$intContDet]['gasto'])
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'salidas_refacciones_consumo_interno.xls', 

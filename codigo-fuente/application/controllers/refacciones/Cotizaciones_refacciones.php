...
class Cotizaciones_refacciones extends MY_Controller {
	...
	function __construct()
	{
	    ...
	    $this->archivo['strCarpetaTemporal'] = './cotizacion_refacciones_temporal_';
	    //Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Cargamos el modelo de cotizaciones
		$this->load->model('refacciones/cotizaciones_refacciones_model', 'cotizaciones');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/cotizaciones_refacciones', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		foreach ($result['cotizaciones'] as $arrDet)
		{
			...
			if($arrDet->estatus == 'ACTIVO')
			{
				...
				//Si el registro cuenta con factura/pedido/remision/etc.
				if($arrDet->FacturaID > 0 || $arrDet->PedidoID > 0 || $arrDet->RemisionID > 0 )
				{
			...
			else if($arrDet->estatus == 'INACTIVO')
			{
				//Si el usuario cuenta con el permiso de acceso VER REGISTRO
				if (in_array('VER REGISTRO', $arrPermisos))
				{
			...
			else //Si el registro cuenta con factura/pedido/remision/etc.
			{
				//Si el usuario cuenta con el permiso de acceso VER REGISTRO
				if (in_array('VER REGISTRO', $arrPermisos))
				{
	...
	{ 
		...
		$objCotizacionRefacciones = new stdClass();

		//Variables que se utilizan para recuperar los valores de la vista
		//Datos de la cotización
		//Convertir a mayúsculas haciendo un llamado a la función mb_strtoupper (para tomar encuenta las letras con acento)
		$objCotizacionRefacciones->intCotizacionRefaccionesID = $this->input->post('intCotizacionRefaccionesID');
		$objCotizacionRefacciones->dteFecha = $this->input->post('dteFecha');
		$objCotizacionRefacciones->intMonedaID = $this->input->post('intMonedaID');
		$objCotizacionRefacciones->intTipoCambio = $this->input->post('intTipoCambio');
		$objCotizacionRefacciones->intProspectoID = $this->input->post('intProspectoID');
		$objCotizacionRefacciones->intVendedorID = $this->input->post('intVendedorID');
		$objCotizacionRefacciones->strMadurez = $this->input->post('strMadurez');
		$objCotizacionRefacciones->intEstrategiaID = $this->input->post('intEstrategiaID');
		$objCotizacionRefacciones->strTipo = $this->input->post('strTipo');
		$objCotizacionRefacciones->intGastosPaqueteria = $this->input->post('intGastosPaqueteria');
		$objCotizacionRefacciones->intGastosPaqueteriaIva = $this->input->post('intGastosPaqueteriaIva');
		$objCotizacionRefacciones->strObservaciones = mb_strtoupper(trim($this->input->post('strObservaciones')));
		$objCotizacionRefacciones->strNotas = mb_strtoupper(trim($this->input->post('strNotas')));
		$objCotizacionRefacciones->intSucursalID = $this->session->userdata('sucursal_id');
		$objCotizacionRefacciones->intUsuarioID = $this->session->userdata('usuario_id');
		
		//Datos de los detalles
		$objCotizacionRefacciones->strRefaccionID = $this->input->post('strRefaccionID'); 
		$objCotizacionRefacciones->strCodigos = $this->input->post('strCodigos'); 
		$objCotizacionRefacciones->strDescripciones = $this->input->post('strDescripciones'); 
		$objCotizacionRefacciones->strCantidades = $this->input->post('strCantidades'); 
		$objCotizacionRefacciones->strPreciosUnitarios = $this->input->post('strPreciosUnitarios'); 
		$objCotizacionRefacciones->strDescuentosUnitarios = $this->input->post('strDescuentosUnitarios'); 
		$objCotizacionRefacciones->strTasaCuotaIva = $this->input->post('strTasaCuotaIva'); 
		$objCotizacionRefacciones->strIvasUnitarios = $this->input->post('strIvasUnitarios'); 
		$objCotizacionRefacciones->strTasaCuotaIeps = $this->input->post('strTasaCuotaIeps');
		$objCotizacionRefacciones->strIepsUnitarios = $this->input->post('strIepsUnitarios'); 
		...
		if (is_numeric($objCotizacionRefacciones->intCotizacionRefaccionesID))
		{
			$bolResultado = $this->cotizaciones->modificar($objCotizacionRefacciones);
		}
		else
		{ 
			//Hacer un llamado a la función para generar el folio consecutivo del proceso
			$objCotizacionRefacciones->strFolio = $this->get_folio_consecutivo($intProcesoMenuID, 10);
			//Si no existe folio del proceso
			if($objCotizacionRefacciones->strFolio == '')
			{
			...
			{
				$bolResultado = $this->cotizaciones->guardar($objCotizacionRefacciones); 
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intCotizacionRefaccionesID');
		...
		if($otdResultado)
		{
			...
			$otdDetalles = $this->cotizaciones->buscar_detalles($intID);
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intCotizacionRefaccionesID', 'ID', 'required|integer');
	...
	public function autocomplete()
	{
		...
		if(isset($strDescripcion))
		{
			...
			$otdResultado = $this->cotizaciones->autocomplete($strDescripcion, $strTipo, $intMonedaID, 
															  $intProspectoID);
	    	...
	    	if ($otdResultado)
			{
				...
				foreach ($otdResultado as $arrCol)
				{
		        	$arrDatos[] = array('value' => $arrCol->folio,
		        						'data' => $arrCol->cotizacion_refacciones_id, 
		        						'total' => $arrCol->total,
	...
	//Método para enviar cotización al correo electrónico del prospecto
	public function enviar_correo_electronico_prospecto()
	{
        ...
        $strRuta = $this->get_reporte_registro($intCotizacionRefaccionesID, 'SI');

         //Array que se utiliza para enviar correo electrónico
		$arrDatos =  array('intReferenciaID' => $intCotizacionRefaccionesID,
						   'strTitulo' => utf8_decode('Solicitud de Cotización de Refacciones'),
						   'strRuta' => $strRuta,
	...
	public function eliminar_carpeta_temporal($intCotizacionRefaccionesID)
	{
        //Definir ubicación de la carpeta
		$strNombreCarpeta = $this->archivo['strCarpetaTemporal'].$intCotizacionRefaccionesID; 
	...
	{	            
		...
		$arrEstatus = array('ACTIVO','PEDIDO','REMISION','FACTURADO','INACTIVO');
		...
		$pdf->strLinea1 =  'LISTADO DE COTIZACIONES DE REFACCIONES '.$strTituloRangoFechas;
		...
		{	
			...
			//Recorremos el arreglo para obtener la información de las cotizaciones de compra
			foreach ($otdResultado as $arrCol)
			{ 
				...
				//Seleccionar los detalles del registro
				$otdDetalles = $this->cotizaciones->buscar_detalles($arrCol->cotizacion_refacciones_id);
				...
				*/
			    ...
			    $pdf->ClippedCell(8, 4, utf8_decode($arrCol->codigo_moneda), 0, 0, 'L', 0);
		    	//Tipo de cambio
			        ...
			        foreach ($arrDetalles as $arrDet) 
			        {
					   //Se agrega la informacion al reporte... se utiliza utf8 para acentos y tildes
					    $pdf->Row(array($arrDet['cantidad'], $arrDet['codigo'], $arrDet['descripcion'],
					    				$arrDet['precio_unitario'], $arrDet['subtotal']),  $arrAlineacionDetalles, FALSE, FALSE, NULL, 'ClippedCell');
					}
				}//Cierre de verificación de detalles
		    	$pdf->Ln(5);//Deja un salto de línea
		...
		$pdf->Output('cotizaciones_refacciones.pdf','I'); 
	}

	//Método para generar un reporte PDF con la información de un registro 
	public function get_reporte_registro($intCotizacionRefaccionesID =  NULL, $strEnviarCorreo = NULL) 
	{	            
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->cotizaciones->buscar($intCotizacionRefaccionesID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->cotizaciones->buscar_detalles($intCotizacionRefaccionesID);
		...
		$strNombreArchivo  = 'cotizacion_refacciones_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('COTIZACIÓN DE REFACCIONES'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
				//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
			...
			if($otdDetalles)
			{
				...
				{ 
					...
					$pdf->Row(array(number_format($intCantidad,2), utf8_decode($arrDet->codigo), 
									...
									number_format($intDescuentoUnitario, 2), number_format($intSubTotalUnitario, 2)), 
		...
		{	
            //Definir ubicación de la carpeta
			$strCarpetaDestino =  $this->archivo['strCarpetaTemporal'].$intCotizacionRefaccionesID.'/'; 
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE COTIZACIONES DE REFACCIONES '.$strTituloRangoFechas);
		//Si existe id del prospecto
		if($intProspectoID > 0)
		{   
			//Seleccionar los datos del prospecto que coincide con el id
        ...
        $objExcel->setActiveSheetIndex(0)
        		 ...
        		 ->setCellValue('K'.$intPosEncabezados, 'MADUREZ')
        ...
        if($strDetalles == 'SI')
        {
        	...
        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('Q'.$intPosEncabezados.':W'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
			...
			{   
				...
				if($otdDetalles)
				{
					...
					foreach ($otdDetalles as $arrDet)
					{
			        	...
			        	$arrDetalles[$intContDetCot]['cantidad'] = $intCantidad;
			        	$arrDetalles[$intContDetCot]['codigo'] = $arrDet->codigo;
			        	$arrDetalles[$intContDetCot]['descripcion'] = $arrDet->descripcion;
			        	$arrDetalles[$intContDetCot]['linea'] = $arrDet->refacciones_linea;
			        	$arrDetalles[$intContDetCot]['marca'] = $arrDet->refacciones_marca;
			        	$arrDetalles[$intContDetCot]['precio_unitario'] = $intPrecioUnitario;
			        	$arrDetalles[$intContDetCot]['subtotal'] = $intSubTotalUnitario;
				...
				*/
				...
				if($arrCol->gastos_paqueteria > 0)
	        	{
					...
					{
						//Agregar datos al array
						$arrDetalles[$intContDetCot]['cantidad'] = 1;
		        		$arrDetalles[$intContDetCot]['codigo'] = '';
		        		$arrDetalles[$intContDetCot]['descripcion'] = 'GASTOS DE PAQUETERÍA';
		        		$arrDetalles[$intContDetCot]['linea'] = '';
				        $arrDetalles[$intContDetCot]['marca'] = '';
		        		$arrDetalles[$intContDetCot]['precio_unitario']= $intGatosPaqueteriaSubtotal;
		        		$arrDetalles[$intContDetCot]['subtotal'] = $intGatosPaqueteriaSubtotal;
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					$objExcel->setActiveSheetIndex(0)
	                         ...
	                         ->setCellValue('K'.$intFila, $arrCol->madurez)
					...
					if($arrDetalles && $strDetalles == 'SI')
					{
						...
						$objExcel->setActiveSheetIndex(0)
						         ...
						         ->setCellValue('R'.$intFila, $arrDetalles[$intContDet]['codigo'])
						         ->setCellValue('S'.$intFila, $arrDetalles[$intContDet]['descripcion'])
						         ->setCellValue('T'.$intFila, $arrDetalles[$intContDet]['linea'])
						         ->setCellValue('U'.$intFila, $arrDetalles[$intContDet]['marca'])
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'cotizaciones_refacciones.xls', 'cotizaciones de refacciones', $intFila);
	}
}
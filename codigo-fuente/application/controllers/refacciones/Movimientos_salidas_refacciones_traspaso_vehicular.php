...
class Movimientos_salidas_refacciones_traspaso_vehicular extends MY_Controller {
	///Variable para identificar el tipo de movimiento
	var $intTipoMovimiento = SALIDA_REFACCIONES_TRASPASO_VEHICULAR; 
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/movimientos_salidas_refacciones_traspaso_vehicular', $arrDatos);
	}
	...
	*********************************************************************************************************************/
	...
	public function get_paginacion()
	{
		...
		$result = $this->movimientos->filtro_salida_traspaso_vehicular($this->intTipoMovimiento,
	...
	{ 
		...
		$objMovimiento->strBackOrderTraspasoInternoID = $this->input->post('strBackOrderTraspasoInternoID'); 
		...
		if (is_numeric($objMovimiento->intMovimientoRefaccionesID))
		{
			...
			$bolResultado = $this->movimientos->modificar_salida_traspaso_vehicular($objMovimiento);
		}
		else
		{ 
			...
			{		
				...
				$bolResultado = $this->movimientos->guardar_salida_traspaso_vehicular($objMovimiento); 
			}
		}
	...
	public function get_datos()
	{
	    ...
	    $otdResultado = $this->movimientos->buscar_salida_traspaso_vehicular($intID);
	...
	public function set_estatus()
	{
		...
		else
		{
			...
			if($bolResultado)
			{
				/*Hacer un llamado al método para modificar el estatus (y el back_order_traspasos_internos) de la solicitud
				  de refacciones internas por traspaso*/
	...
	{	            
		...
		$otdResultado = $this->movimientos->buscar_salida_traspaso_vehicular(NULL, $this->intTipoMovimiento, 
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE TRASPASOS A PARQUE VEHICULAR ').$strTituloRangoFechas;
		//Crea los titulos de la cabecera
		$pdf->arrCabecera = array('FOLIO', 'SOLICITUD', 'FECHA', 'SUCURSAL', 'TOTAL', 'ESTATUS');
		//Establece el ancho de las columnas de cabecera
		$pdf->arrAnchura = array(20, 20, 20, 90, 20, 20);
		...
		{	
			...
			{ 
				...
				$otdDetalles = $this->movimientos->buscar_detalles_salida_traspaso_vehicular($arrCol->movimiento_refacciones_id);
		...
		$pdf->Output('traspasos_parque_vehicular.pdf','I'); 
	}
	...
	{	            
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->movimientos->buscar_salida_traspaso_vehicular($intMovimientoRefaccionesID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->movimientos->buscar_detalles_salida_traspaso_vehicular($intMovimientoRefaccionesID);
		...
		$strNombreArchivo  = 'traspaso_parque_vehicular_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('TRASPASO A PARQUE VEHICULAR'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE TRASPASOS A PARQUE VEHICULAR '.$strTituloRangoFechas);
			...
			{   
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValue('D'.$intFila, $arrCol->sucursal)
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'traspasos_parque_vehicular.xls', 
        									'traspasos a parque vehicular', $intFila);
	}
	...
	*********************************************************************************************************************/
	...
	public function get_datos_detalles()
	{
	    ...
	    $otdResultado = $this->movimientos->buscar_detalles_salida_traspaso_vehicular($intMovimientoRefaccionesID, 
	    															                  $intReferenciaID);
		//Si existen datos, asignar los datos recuperados en el array
		if($otdResultado)
		{
	...
	*********************************************************************************************************************/
	//Método para modificar el estatus de un registro
	public function set_estatus_solicitud_traspaso($intMovimientoRefaccionesID, 
												   $intSolicitudTraspasoRefaccionesInternasID)
	{
		...
		//Variable que se utiliza para saber si se actualizó el estatus de la solicitud de refacciones internas por traspaso
		$bolResultadoSolicitud = FALSE;

		//Seleccionar el total de movimientos activos que coinciden con el id de la solicitud de refacciones internas por traspaso
		$otdMovimientos = $this->movimientos->buscar_total_movimientos_solicitud_traspaso_interno($this->intTipoMovimiento, 
																						          $intSolicitudTraspasoRefaccionesInternasID);
		//Asignar el total de movimientos activos 
		$intTotalMovimientos = $otdMovimientos->total_movimientos;
	
		//Si no existen movimientos activos de la solicitud de refacciones internas por traspaso
		if($intTotalMovimientos == 0)
		{
			//Cambiar el estatus de la solicitud de refacciones internas por traspaso
		    $strEstatusSolicitud = 'ACTIVO';

			//Hacer un llamado al método para eliminar los datos de los pedidos pendientes (back_order_traspasos_internos) de la solicitud
		    $bolResultadoBackOrder = $this->movimientos->eliminar_pedidos_solicitud_traspaso_interno($intSolicitudTraspasoRefaccionesInternasID);
		}
		else
		{
			...
			if($otdDetalles)
			{
				...
				{	
					...
					$intBackOrderTraspasoInternoID = $arrDet->back_order_traspaso_interno_id;
					$strEstatus = 'ACTIVO';

					//Calcular cantidad pendiente por surtir
					$intCantidad =  $intCantidadPendiente + $intCantidadSurtida;

					//Hacer un llamado al método para modificar los datos del pedido pendiente (back_order_traspasos_internos)
					$bolResultadoPedido = $this->movimientos->modificar_pedido_pendiente_traspaso_interno($intBackOrderTraspasoInternoID, 
																										  $intCantidad, 
																										  $strEstatus);
		...
		if($bolResultadoBackOrder == TRUE)
		{
			...
			$bolResultadoSolicitud = $this->movimientos->set_estatus_solicitud_traspaso_interno($intSolicitudTraspasoRefaccionesInternasID, 
																					    		$strEstatusSolicitud);
		}

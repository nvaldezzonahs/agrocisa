...
class Movimientos_salidas_refacciones_ajuste extends MY_Controller {
	///Variable para identificar el tipo de movimiento
	var $intTipoMovimiento = SALIDA_REFACCIONES_AJUSTE; 
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/movimientos_salidas_refacciones_ajuste', $arrDatos);
	}
	...
	*********************************************************************************************************************/
	...
	public function get_paginacion()
	{
		...
		$result = $this->movimientos->filtro_salida_ajuste($this->intTipoMovimiento,
	...
	{ 
		...
		if (is_numeric($objMovimiento->intMovimientoRefaccionesID))
		{
			...
			$bolResultado = $this->movimientos->modificar_salida_ajuste($objMovimiento);
		}
		else
		{ 
			...
			{	
				$bolResultado = $this->movimientos->guardar_salida_ajuste($objMovimiento);
	...
	public function get_datos()
	{
	    ...
	    $otdResultado = $this->movimientos->buscar_salida_ajuste($intID);
		//Si existen datos, asignar los datos recuperados en el array
		if($otdResultado)
		{
			$arrDatos['row'] = $otdResultado;
			//Seleccionar los detalles del registro
			$otdDetalles = $this->movimientos->buscar_detalles_salida_ajuste($intID);
	...
	{	            
		...
		$otdResultado = $this->movimientos->buscar_salida_ajuste(NULL, $this->intTipoMovimiento, 
		...
		$pdf->strLinea1 =  utf8_decode('LISTADO DE SALIDAS DE REFACCIONES POR AJUSTE ').$strTituloRangoFechas;
		...
		{	
			...
			{ 
				...
				//Seleccionar los detalles del registro
				$otdDetalles = $this->movimientos->buscar_detalles_salida_ajuste($arrCol->movimiento_refacciones_id);
		...
		$pdf->Output('salidas_refacciones_ajuste.pdf','I'); 
	}
	...
	{	            
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->movimientos->buscar_salida_ajuste($intMovimientoRefaccionesID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->movimientos->buscar_detalles_salida_ajuste($intMovimientoRefaccionesID);
		...
		$strNombreArchivo  = 'salida_refacciones_ajuste_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, utf8_decode('SALIDA DE REFACCIONES POR AJUSTE'), 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE SALIDAS DE REFACCIONES POR AJUSTE '.$strTituloRangoFechas);
		//Si existe id del empleado
		if($intEmpleadoID > 0)
		{   
			//Seleccionar los datos del empleado que coincide con el id
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'salidas_refacciones_ajuste.xls', 

...
class Rep_estadisticas extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('refacciones/rep_movimientos_refacciones_model', 'movimientos');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/rep_estadisticas', $arrDatos);
	}
	...
	/*Método para generar un reporte PDF con el listado de los márgenes de utilidad
	...
	public function get_reporte() 
	{
		...
		$arrSucursales = trim($this->input->post('arrSucursales'));
		...
		if($dteFechaInicial != '' && $dteFechaFinal != '')
		{
			...
			$strTituloFecha = 'ENTRE '.$this->get_fecha_formato_letra($dteFechaInicial, 'C');
			$strTituloFecha .= ' Y '.$this->get_fecha_formato_letra($dteFechaFinal, 'C');
		}
		...
		$pdf->arrCabecera = array(utf8_decode('MOVIMIENTO'), 'FOLIO', 'FECHA', 'CANTIDAD', 'COSTO',  
							 	  'IMPORTE', 'EXISTENCIA');
		...
		*/
		...
		/*for ($intCon = 0; $intCon < sizeof($arrSucursalID); $intCon++) 
		{
			...
			if($otdMovimientos)
			{
				...
				foreach ($otdMovimientos as $arrMov) 
				{
					...
					$strConcepto = $this->ARR_MOV_REFACCIONES_DESCRIPCIONES[$intTipoMovimiento];
		...
		$pdf->Output('reporte_margenes_utilidad.pdf','I');	 
	}
	...
	public function get_xls() 
	{
		...
		*/
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'reporte_margenes_utilidad.xls', 'estadisticas', $intNumMaxMovimientos);
	}
}	
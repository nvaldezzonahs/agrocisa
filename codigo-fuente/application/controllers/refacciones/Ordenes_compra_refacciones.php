...
class Ordenes_compra_refacciones extends MY_Controller {
	...
	function __construct()
	{
	    ...
	    $this->archivo['strCarpetaDestino'] = './archivos/ordenes_compra_refacciones/';
	    //Asignar ruta de la carpeta temporal
	    $this->archivo['strCarpetaTemporal'] = './orden_compra_refacciones_temporal_';
	    //Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Cargamos el modelo de ordenes de compra
		$this->load->model('refacciones/ordenes_compra_refacciones_model', 'ordenes');
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('refacciones/ordenes_compra_refacciones', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		foreach ($result['ordenes'] as $arrDet)
		{
            ...
            $intOrdenCompraRefaccionesID = $arrDet->orden_compra_refacciones_id;
            //Concatenar id del registro que hace referencia a la carpeta donde se encuentra el archivo
			$strNombreCarpeta = $this->archivo['strCarpetaDestino'].$intOrdenCompraRefaccionesID;
            //Asignar el nombre del archivo que le corresponde al registro
		    $strNombreArchivo = $this->get_verifar_archivo_registro($strNombreCarpeta, $intOrdenCompraRefaccionesID);
	...
	{ 
		...
		$objOrdenCompra->intOrdenCompraRefaccionesID = $this->input->post('intOrdenCompraRefaccionesID');
		...
		$objOrdenCompra->strRefaccionID = $this->input->post('strRefaccionID'); 
		$objOrdenCompra->strCodigos = $this->input->post('strCodigos'); 
		$objOrdenCompra->strDescripciones = $this->input->post('strDescripciones');
		$objOrdenCompra->strCodigosLineas = $this->input->post('strCodigosLineas');
		...
		if (is_numeric($objOrdenCompra->intOrdenCompraRefaccionesID))
		{
		...
		{ 
			...
			else
			{
				 ...
				 */
			    list($bolResultado, $objOrdenCompra->intOrdenCompraRefaccionesID, $objOrdenCompra->strFolioConsecutivo) = explode("_", $bolResultado); 
			}
		}
		...
		if($bolResultado !== NULL)
		{
			...
			if($bolResultado)
			{
				...
				$arrDatos = array('resultado' => TRUE,
							 	  ...
							 	  'orden_compra_refacciones_id' => $objOrdenCompra->intOrdenCompraRefaccionesID,
	...
	public function get_datos()
	{
		...
		//Variables que se utilizan para recuperar los valores de la vista 
		$intID = $this->input->post('intOrdenCompraRefaccionesID');
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intOrdenCompraRefaccionesID', 'ID', 'required|integer');
	...
	public function autocomplete()
	{
		...
		if(isset($strDescripcion))
		{
			...
			$otdResultado = $this->ordenes->autocomplete($strDescripcion, $strFormulario);
	    	...
	    	if ($otdResultado)
			{
				...
				foreach ($otdResultado as $arrCol)
				{
		        	$arrDatos[] = array('value' => $arrCol->orden_proveedor, 
		        						'data' => $arrCol->orden_compra_refacciones_id);
				}
	    	}
	...
	{
		//Variables que se utilizan para recuperar los valores de la vista 
		$intOrdenCompraRefaccionesID = $_POST["intOrdenCompraRefaccionesID_ordenes_compra_refacciones_refacciones"];
		$strBotonArchivoID = $_FILES["archivo_varios_ordenes_compra_refacciones_refacciones"];
		...
		$this->subir_archivo_reg($strBotonArchivoID, $this->archivo['strCarpetaPrincipal'], 
							     ...
							     $strNombreCarpeta, $intOrdenCompraRefaccionesID, TRUE);
	}
	...
	{
        //Variables que se utilizan para recuperar los valores de la vista
		$intOrdenCompraRefaccionesID = $this->input->post('intOrdenCompraRefaccionesID');
		$strFolio = $this->input->post('strFolio');

		//Definir ubicación de la carpeta
		$strNombreCarpeta = $this->archivo['strCarpetaDestino'].$intOrdenCompraRefaccionesID;
		//Asignar nombre de la carpeta zip
		$strNombreCarpetaZIP = 'OCR_'.$strFolio.'.zip';
	...
	public function enviar_correo_electronico_proveedor()
	{
        ...
        $strRuta = $this->get_reporte_registro($intOrdenCompraRefaccionesID, 'SI');

         //Array que se utiliza para enviar correo electrónico
		$arrDatos =  array('intReferenciaID' => $intOrdenCompraRefaccionesID,
						   'strTitulo' => utf8_decode('Solicitud de Orden de Compra Refacciones'),
						   'strRuta' => $strRuta,
	...
	public function eliminar_carpeta_temporal($intOrdenCompraRefaccionesID)
	{
		//Definir ubicación de la carpeta
		$strNombreCarpeta = $this->archivo['strCarpetaTemporal'].$intOrdenCompraRefaccionesID; 
	...
	{	            
		...
		$pdf->strLinea1 =  'LISTADO DE ORDENES DE COMPRA REFACCIONES '.$strTituloRangoFechas;
		...
		{	
			...
			{ 
				...
				//Seleccionar los detalles del registro
				$otdDetalles = $this->ordenes->buscar_detalles($arrCol->orden_compra_refacciones_id);
			        ...
			        foreach ($arrDetalles as $arrDet) 
			        {
					   //Se agrega la informacion al reporte... se utiliza utf8 para acentos y tildes
					    $pdf->Row(array($arrDet['cantidad'], $arrDet['codigo_linea'],
					    				...
					    				$arrDet['precio_unitario'], $arrDet['subtotal']), NULL, 
		...
		$pdf->Output('ordenes_compra_refacciones.pdf','I'); 
	}

	//Método para generar un reporte PDF con la información de un registro 
	public function get_reporte_registro($intOrdenCompraRefaccionesID = NULL, $strEnviarCorreo = NULL) 
	{	            
		...
		//Seleccionar los datos del registro que coincide con el id
		$otdResultado = $this->ordenes->buscar($intOrdenCompraRefaccionesID);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->ordenes->buscar_detalles($intOrdenCompraRefaccionesID);
		...
		$strNombreArchivo  = 'orden_compra_refacciones_';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		{	
			...
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_SUBTITULO_PDF);
			$pdf->SetXY(108, 42);
			$pdf->ClippedCell(92, 3, 'ORDEN DE COMPRA REFACCIONES', 0, 0, 'C', TRUE);
			$pdf->SetTextColor(0);//establece el color de texto
			//Folio
			//Asigna el tipo y tamaño de letra
			$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
		...
		{	
            //Definir ubicación de la carpeta
			$strCarpetaDestino =  $this->archivo['strCarpetaTemporal'].$intOrdenCompraRefaccionesID.'/'; 
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE ORDENES DE COMPRA REFACCIONES '.$strTituloRangoFechas);
		//Si existe id del proveedor
		if($intProveedorID > 0)
		{   //Seleccionar los datos del proveedor que coincide con el id
			$otdProveedor =  $this->proveedores->buscar($intProveedorID);
        ...
        if($strDetalles == 'SI')
        {
        	...
        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('P'.$intPosEncabezados.':U'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'ordenes_compra_refacciones.xls', 'ordenes de compra', $intFila);
	}
}
...
class Insumos extends MY_Controller {
	...
	function __construct()
	{
		parent::__construct();
		//Cargamos el modelo
		$this->load->model('mercadotecnia/insumos_model', 'insumos');
	}
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('mercadotecnia/insumos', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		$result = $this->insumos->filtro(trim($this->input->post('strBusqueda')),
			                             $config['per_page'],
			                             $config['cur_page']);
		...
		foreach ($result['insumos'] as $arrDet)
		{
		...
		$arrDatos = array('rows' => $result['insumos'],
	...
	{ 
		...
		$strDescripcionAnterior = mb_strtoupper(trim($this->input->post('strDescripcionAnterior')));
		
        //Definir las reglas de validación
		//Validar que la descripción sea única
        if (($intInsumoID == '') OR ($strDescripcionAnterior != $strDescripcion))
        {
            $this->form_validation->set_rules('strDescripcion', 'insumo', 'required|is_unique[insumos.descripcion]');
        }
        else
        {
        	$this->form_validation->set_rules('strDescripcion', 'insumo', 'required');
        }
		...
		else
		{
			...
			if (is_numeric($intInsumoID))
			{
				$bolResultado = $this->insumos->modificar($intInsumoID, $strDescripcion);
			}
			else
			{ 
				$bolResultado = $this->insumos->guardar($strDescripcion);
			}
	...
	public function get_datos()
	{
		...
		if($strTipo == 'id')
		{
			$otdResultado = $this->insumos->buscar($strBusqueda);
		}
		else 
		{
    		$otdResultado = $this->insumos->buscar(NULL, $strBusqueda);
		}
	...
	{
	    //Definir las reglas de validación
		$this->form_validation->set_rules('intInsumoID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intInsumoID');
			...
			$bolResultado = $this->insumos->set_estatus($intID, $strEstatus);
			//Si no se obtienen errores al ejecutar el proceso
			if($bolResultado)
			{
				//Enviar el mensaje de éxito al formulario
	...
	{	            
		...
		$otdResultado = $this->insumos->buscar(NULL, NULL, $strBusqueda); 
		...
		$pdf->strLinea1= 'LISTADO DE INSUMOS';
		//Agregar la primer pagina
		$pdf->AddPage();
		//Asigna el tipo y tamaño de letra para la cabecera de la tabla
		$pdf->SetFont(TIPO_LETRA_PDF, LETRA_NEGRITA, TAMANO_LETRA_TITULO_TABLA_PDF);
		//Crea los titulos de la cabecera
		$arrCabecera = array('INSUMO', 'ESTATUS');
		//Establece el ancho de las columnas de cabecera
		$arrAchura = array(170, 20);
		//Establece la alineación de las celdas de la tabla
		$arrAlineacion = array('L', 'C');
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE INSUMOS');
		//Se agregan las columnas de cabecera
        $objExcel->setActiveSheetIndex(0)
        		 ->setCellValue('A'.$intPosEncabezados, 'INSUMO')
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'insumos.xls', 'insumos', $intFila);
	}
	...
	public function autocomplete()
	{
		...
		if(isset($strDescripcion))
		{
			...
			$otdResultado = $this->insumos->autocomplete($strDescripcion);

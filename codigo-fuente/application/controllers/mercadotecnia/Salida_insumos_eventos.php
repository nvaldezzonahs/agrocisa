...
class Salida_insumos_eventos extends MY_Controller {
	...
	public function index($strParametros = NULL)
	{
		...
		$this->cargar_vista('mercadotecnia/Salida_insumos_eventos', $arrDatos);
	}
	...
	public function get_paginacion()
	{
		...
		$result = $this->movimientos->filtro_salida_evento($dteFechaInicial,
											 $dteFechaFinal,
											 $intEventoID,
											 $intTipoMovimiento,
				                             $config['per_page'],
	...
	{ 
		...
		$intSalidaInsumosEventosID = $this->input->post('intSalidaInsumosEventosID');
		...
		if (is_numeric($intSalidaInsumosEventosID))
		{
			$bolResultado = $this->movimientos->modificar_salida_evento($intSalidaInsumosEventosID, 
													 	  $dteFecha, 
														  $intEventoID,
														  $intSucursalID,
														  $intEmpleadoID,
		...
		{ 
			...
			{	
				$bolResultado = $this->movimientos->guardar_salida_evento($strFolio,
																		  $dteFecha, 
																		  $intEventoID,
																		  $intSucursalID,
																		  $intEmpleadoID,
				 ...
				 */
			    list($bolResultado, $intSalidaInsumosEventosID, $strFolioConsecutivo) = explode("_", $bolResultado); 
			}
		}
		...
		if($bolResultado !== NULL)
		{
			...
			if($bolResultado)
			{
				...
				$arrDatos = array('resultado' => TRUE,
							 	  ...
							 	  'salida_insumos_eventos_id' => $intSalidaInsumosEventosID,
	...
	public function verificar_salida()
	{
		...
		$intID = $this->input->post('intMovimientoInsumoID');
		//Seleccionar los datos del registro que coincide con el id
	    $otdResultado = $this->movimientos->validar_salida($intID);
		//Si existen datos, asignar los datos recuperados en el array
		if($otdResultado)
		{
			$arrDatos['row'] = $otdResultado;	
	...
	public function set_estatus()
	{
		...
		$this->form_validation->set_rules('intSalidaInsumosEventosID', 'ID', 'required|integer');
		...
		{
	        //Variables que se utilizan para recuperar los valores de la vista 
	        $intID = $this->input->post('intSalidaInsumosEventosID');
	...
	{	            
		...
		//Seleccionar los datos del registro que coincide con el id
		$intTipoMovimiento = 11;
		$otdResultado = $this->movimientos->buscar_salida_evento(NULL, $intTipoMovimiento, $dteFechaInicial, $dteFechaFinal, $intEventoID);
		...
		$pdf->strLinea1 =  'LISTADO DE SALIDA DE INSUMOS A EVENTO '.$strTituloRangoFechas;
		...
		$arrCabecera = array('FOLIO', 'FECHA', 'EVENTO',  'RESPONSABLE',  'SUCURSAL', 'TOTAL', 'ESTATUS');
		//Establece el ancho de las columnas de cabecera
		$arrAchura = array(16, 16, 50, 50, 20, 18, 20);
		//Establece la alineación de las celdas de la tabla
		$arrAlineacion = array('L', 'C', 'L', 'L', 'L', 'R', 'C');
		...
		$pdf->Output('salida_insumos_evento.pdf','I'); 
	}

	//Método para generar un reporte PDF con la información de un registro 
	public function get_reporte_registro($intSalidaInsumosEventosID) 
	{
		//Cargar el helper del reporte
		$this->load->helper('hlpfpdf');
		//Seleccionar los datos del registro que coincide con el id
		$intTipoMovimiento = 11;
		$otdResultado = $this->movimientos->buscar_salida_evento($intSalidaInsumosEventosID, $intTipoMovimiento);
		//Seleccionar los detalles del registro
		$otdDetalles = $this->movimientos->buscar_detalles($intSalidaInsumosEventosID);
		...
		$strNombreArchivo  = 'salida_insumos_evento';
		//Agregar la primer pagina
		$pdf->AddPage();
		...
		$pdf->SetXY(80, 15);
		$pdf->ClippedCell(120, 3, 'SALIDA DE INSUMOS A EVENTO', 0, 0, 'C', TRUE);
		...
		//RESPONSABLE
		$pdf->SetXY(80, 27);
		$pdf->ClippedCell(35, 3, 'RESPONSABLE');
		//SUCURSAL
		$pdf->SetXY(80, 31);
		$pdf->ClippedCell(35, 3, 'SUCURSAL');
		//LOCALIDAD
		$pdf->SetXY(80, 35);
		$pdf->ClippedCell(35, 3, 'LOCALIDAD');
		//MUNICIPIO
		$pdf->SetXY(80, 39);
		$pdf->ClippedCell(35, 3, 'MUNICIPIO');
		//ESTADO
		$pdf->SetXY(140, 39);
		$pdf->ClippedCell(35, 3, 'ESTADO');

		//Información de la salida de insumos a evento
		...
		{	
			...
			//RESPONSABLE
			$pdf->SetXY(101, 27);
			$pdf->ClippedCell(60, 3, utf8_decode($otdResultado->responsable));
			//SUCURSAL
			$pdf->SetXY(101, 31);
			$pdf->ClippedCell(30, 3, utf8_decode($otdResultado->sucursal));
			//LOCALIDAD
			$pdf->SetXY(101, 35);
			$pdf->ClippedCell(60, 3, utf8_decode($otdResultado->localidad));
			//MUNICIPIO
			$pdf->SetXY(101, 39);
			$pdf->ClippedCell(30, 3, utf8_decode($otdResultado->municipio));
			//ESTADO
			$pdf->SetXY(154, 39);
			$pdf->ClippedCell(30, 3, utf8_decode($otdResultado->estado));
			...
			if($otdDetalles)
			{
				...
				//Crea los titulos de la cabecera
				$arrCabecera = array('Cantidad', utf8_decode('Descripción'), 'Importe', 'Subtotal');
				//Establece el ancho de las columnas de cabecera
				$arrAchura = array(15, 120, 25, 25);
				//Establece la alineación de las celdas de la tabla
	...
	{	
		...
		$objExcel->setActiveSheetIndex(0)
			     ->setCellValue('A7', 'LISTADO DE SALIDA DE INSUMOS A EVENTO '.$strTituloRangoFechas);
		//Si existe id del evento
		if($intEventoID > 0)
		{
			//Seleccionar los datos del proveedor que coincide con el id
        ...
        $objExcel->setActiveSheetIndex(0)
        		 ...
        		 ->setCellValue('C'.$intPosEncabezados, 'EVENTO')
        		 ->setCellValue('D'.$intPosEncabezados, 'RESPONSABLE')
        		 ->setCellValue('E'.$intPosEncabezados, 'SUCURSAL')
        ...
        if($strDetalles == 'SI')
        {
        	...
        	//Preferencias de color de relleno de celda 
        	$objExcel->getActiveSheet()
    			     ->getStyle('H'.$intPosEncabezados.':K'.$intPosEncabezados)
    			     ->getFill()
    			     ->applyFromArray($arrStyleColumnas);
			...
			{   
			    ...
			    for ($intContDet = 0; $intContDet < $intNumDetalles; $intContDet++) 
			    {
					...
					$objExcel->setActiveSheetIndex(0)
		                         ...
		                         ->setCellValue('C'.$intFila, $arrCol->evento)
		                         ->setCellValue('D'.$intFila, $arrCol->responsable)
		                         ->setCellValue('E'.$intFila, $arrCol->sucursal)
        ...
        $this->get_pie_pagina_archivo_excel($objExcel, 'salida_insumos_evento.xls', 'salida de insumos a evento', $intFila);
	}


	//Función para obtener el Costo de un insumo
	public function get_costo(){

		//Array que se utiliza para enviar datos a la vista
		$arrDatos = array('row' => NULL);
		//Variables que se utilizan para recuperar los valores de la vista 
		$intInsumoID = $this->input->post('intInsumoID');
		$strAnio = date('Y');
		//echo $intInsumoID;
		//echo $strAnio;
		
		//Seleccionar los datos del registro que coincide con el id
	    $otdResultado = $this->movimientos->get_existencia_costo($intInsumoID, $strAnio);
		//Si existen datos, asignar los datos recuperados en el array
		if($otdResultado)
		{
			$arrDatos['row'] = $otdResultado;

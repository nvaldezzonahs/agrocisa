...
class Pagos extends MY_Controller {
	...
	*********************************************************************************************************************/
	...
	{ 
		...
		$objPago->intRegimenFiscalID = (($this->input->post('intRegimenFiscalID') !== '') ? 
		...
		$objPago->intExportacionID = (($this->input->post('intExportacionID') !== '') ? 
						   	   		  $this->input->post('intExportacionID') : NULL);

		//Si no existe código del objeto impuesto asignar valor nulo
		$objPago->strObjetoImpuestoSat = (($this->input->post('strObjetoImpuestoSat') !== '') ? 
		...
		if($otdResultado)
		{
			...
			if($otdDetalles)
			{
				...
				foreach ($otdDetalles as $arrDet)
				{
					...
					$arrSaldoAnticipo = $this->get_saldo_anticipo($arrDet->anticipo_id, $intID, 'pago', 'polizas_aplicacion', $arrDet->renglon);
					//Asignar el saldo por aplicar del anticipo
	       		    $arrDet->total_aplicar = $arrSaldoAnticipo['saldo'];
	...
	public function autocomplete()
	{
		...
		if(isset($strDescripcion))
		{
			...
			$otdResultado = $this->pagos->autocomplete($strDescripcion, $strFormulario, $intReferenciaID);
	...
	*********************************************************************************************************************/
    ...
    public function get_saldo_factura($intReferenciaID, $strTipoReferencia)
	{
		...
		//Variable que se utiliza para asignar la fecha actual 
		$dteFechaCorte = date("Y-m-d");
		//Seleccionar los importes de la factura (con saldo) (primer posición del arreglo)
	...
	public function get_saldos_cliente()
	{
		...
		if($otdResultado)
		{
			...
			//Dependiendo del tipo de referencia asignar módulo
			if($strReferencia == 'MAQUINARIA')
			{
				$strModulos = 'MAQUINARIA';

			}
			else if($strReferencia == 'REFACCIONES')
			{
				$strModulos = 'REFACCIONES';
			}
			else if($strReferencia == 'SERVICIO')
			{
				$strModulos = 'SERVICIO';
			}
			else
			...
			//Seleccionar los datos de las facturas (con saldo)  que coinciden con el parámetro enviado
			$otdFacturas = $this->pagos->buscar_facturas_importes('grid_view', $dteFechaCorte, $intProspectoID, NULL, 
																   NULL, NULL, NULL, NULL,
													               NULL, $strModulos, NULL, NULL, 'saldo');

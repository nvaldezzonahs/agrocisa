						...
						<tr class="movil {{estiloRegistro}}">   
							...
							<td class="td-center movil a6"> 
								...
								<button class="btn btn-default btn-xs {{mostrarAccionGenerarPoliza}}"  
										onclick="generar_poliza_polizas_abono_cuentas_cobrar({{poliza_abono_id}}, 'principal')"  title="Generar póliza">
				...
				<form id="frmPolizasAbonoCuentasCobrar" method="post" action="#" class="form-horizontal" 
					...
					<div class="row">
						...
						<div class="col-sm-3 col-md-3 col-lg-3 col-xs-12">
							<div class="form-group">
								<div class="col-md-12">
									<label for="cmbMonedaID_polizas_abono_cuentas_cobrar">Moneda</label>
								</div>
								<div id="divCmbMsjValidacion" class="col-md-12">
									<select class="form-control" id="cmbMonedaID_polizas_abono_cuentas_cobrar" 
									 		name="intMonedaID_polizas_abono_cuentas_cobrar" tabindex="1">
                     				</select>
								</div>
							</div>
						</div>
	...
	<script type="text/javascript">
		...
		*********************************************************************************************************************/
				...
				.bootstrapValidator({excluded: [':disabled'],
									 ...
									 feedbackIcons: {
									  ...
									  fields: {
										...
										intMonedaID_polizas_abono_cuentas_cobrar: {
											validators: {
												notEmpty: {message: 'Seleccione una moneda'}
											}
										},
										intTipoCambio_polizas_abono_cuentas_cobrar: {
											...
											validators: {
										    	callback: {
					                                callback: function(value, validator, $field) {
						                                    ...
						                                    if(parseInt($('#cmbMonedaID_polizas_abono_cuentas_cobrar').val()) !== intMonedaBaseIDPolizasAbonoCuentasCobrar)
						                                    {
						                                    	if(value === '')
						                                    	{
						                                    		return {
						                                    	...
						                                    	//Verificar que el tipo de cambio no sea mayor que su valor máximo
						                                      	else if(parseFloat($.reemplazar(value, ",", "")) > intTipoCambioMaximoPolizasAbonoCuentasCobrar)
						                                    	{
						                                    		return {
						                                              valid: false,
						                                              message: 'El tipo de cambio no debe ser mayor que '+intTipoCambioMaximoPolizasAbonoCuentasCobrar
						                                          	};
						                                    	}
		...
		function guardar_polizas_abono_cuentas_cobrar()
		{
			...
			var intMonedaIDPolizaAbono = parseInt($("#cmbMonedaID_polizas_abono_cuentas_cobrar").val());
			...
			{ 
				...
				intMonedaID: $('#cmbMonedaID_polizas_abono_cuentas_cobrar').val(),
		...
		function mensaje_polizas_abono_cuentas_cobrar(tipoMensaje, mensaje)
		{
			...
			$.post('cuentas_cobrar/polizas_abono/get_datos',
			       ...
			       function(data) {
			            ...
			            if(data.row)
			            {
				            ...
				            $('#cmbMonedaID_polizas_abono_cuentas_cobrar').val(data.row.moneda_id);
                            ...
                            else
                            {
							    ...
							    if(parseInt(data.row.moneda_id) !== intMonedaBaseIDPolizasAbonoCuentasCobrar)
							    {
									//Habilitar caja de texto
									$("#txtTipoCambio_polizas_abono_cuentas_cobrar").removeAttr('disabled');
							    }
							    else
							    {
				            ...
				            //Enfocar caja de texto
							$('#cmbMonedaID_polizas_abono_cuentas_cobrar').focus();
			       	    }
			       },
			       'json');
		}
                  ...
                  function(data) {
                    if(data.row){
                       ...
                       $("#txtPais_polizas_abono_cuentas_cobrar").val(data.row.pais_rep);
			            ...
			            if($('#cmbMonedaID_polizas_abono_cuentas_cobrar').val() !== '' && $('#txtTipoCambio_polizas_abono_cuentas_cobrar').val() != '')
			            {
			            	//Habilitar botón Buscar facturas
			            	$('#btnBuscarDoc_polizas_abono_cuentas_cobrar').removeAttr('disabled');
			            }
		...
		function cargar_monedas_polizas_abono_cuentas_cobrar()
		{
			...
			$.post('contabilidad/sat_monedas/get_combo_box', {},
				function(data)
				{
					$('#cmbMonedaID_polizas_abono_cuentas_cobrar').empty();
					var temp = Mustache.render($('#monedas_polizas_abono_cuentas_cobrar').html(), data);
					$('#cmbMonedaID_polizas_abono_cuentas_cobrar').html(temp);
				},
				'json');
		}

		//Función para regresar el tipo de cambio que le corresponde a la moneda seleccionada
		function get_tipo_cambio_polizas_abono_cuentas_cobrar()
		{	
			...
			if(parseInt($('#cmbMonedaID_polizas_abono_cuentas_cobrar').val()) !== intMonedaBaseIDPolizasAbonoCuentasCobrar)
         	{
				...
				//Hacer un llamado a la función para cambiar el formato de la fecha a año-mes-día
				var dteFecha = $.formatFechaMysql($('#txtFecha_polizas_abono_cuentas_cobrar').val());

				//Concatenar criterios de búsqueda para regresar el tipo de cambio
				var strCriteriosBusq = dteFecha+'|'+$('#cmbMonedaID_polizas_abono_cuentas_cobrar').val();
	            ...
	            $.post('caja/tipos_cambio/get_datos',
	                  ...
	                  function(data) {
	                    if(data.row){
	                       $("#txtTipoCambio_polizas_abono_cuentas_cobrar").val(data.row.tipo_cambio_sat);
	                    }
	                  }
	                 ,
	                'json');
		...
		function generar_poliza_polizas_abono_cuentas_cobrar(id, formulario)
		{
		...
		*********************************************************************************************************************/
		...
		function agregar_renglon_fras_relacionadas_polizas_abono_cuentas_cobrar()
		{
			...
			else
			{
				...
				else
				{
				    ...
				    mensaje_polizas_abono_cuentas_cobrar('informacion', strMensaje);
		...
		{
			//Variable que se utiliza para asignar la moneda de la póliza de abono
			var intMonedaIDNotaCredito =  parseInt($("#cmbMonedaID_polizas_abono_cuentas_cobrar").val());
			//Variable que se utiliza para asignar el tipo de cambio de la póliza de abono
			var intTipoCambioNotaCredito =  parseFloat($("#txtTipoCambio_polizas_abono_cuentas_cobrar").val());
		...
		*********************************************************************************************************************/
		...
		function lista_facturas_relacionar_fras_polizas_abono_cuentas_cobrar() 
		{
			...
			$.post('caja/pagos/get_facturas_adeudos',
					{	
						...
						intMonedaIDPago: $('#cmbMonedaID_polizas_abono_cuentas_cobrar').val(),
		...
		$(document).ready(function() 
		{
			...
			*********************************************************************************************************************/
			...
			$('#dteFecha_polizas_abono_cuentas_cobrar').on('dp.change', function (e) {
				//Hacer un llamado a la función para regresar el tipo de cambio que le corresponde a la moneda
				get_tipo_cambio_polizas_abono_cuentas_cobrar();
			});

			//Habilitar o deshabilitar tipo de cambio cuando cambie la opción del combobox
	        $('#cmbMonedaID_polizas_abono_cuentas_cobrar').change(function(e){   
	           
	           	//Variable que se utiliza para asignar el texto del combobox moneda
				var strMoneda = '';
				//Si existe id de la moneda
             	if($('#cmbMonedaID_polizas_abono_cuentas_cobrar').val() !== '')
             	{

             		//Asignar el texto del combobox moneda
				    strMoneda = $('select[name="intMonedaID_polizas_abono_cuentas_cobrar"] option:selected').text();
					//Separar cadena para obtener el código de la moneda de la moneda de la póliza de abono
					strMoneda = strMoneda.substr(0,3);

             	}
             	
				//Asignar el código de la moneda
				$('#spnMonedaPolizaAbono_fras_relacionadas_polizas_abono_cuentas_cobrar').text(strMoneda);
				strMonedaPolizasAbonoCuentasCobrar = strMoneda;
	            ...
	            //Dependiendo del id de la moneda habilitar o deshabilitar tipo de cambio
              	if(parseInt($('#cmbMonedaID_polizas_abono_cuentas_cobrar').val()) === intMonedaBaseIDPolizasAbonoCuentasCobrar)
             	{
					...
					$('#txtTipoCambio_polizas_abono_cuentas_cobrar').val(intTipoCambioMonedaBasePolizasAbonoCuentasCobrar);
					//Asignar formatCurrency (formato Moneda) a la caja de texto para cambiar el formato de la cantidad
					$('#txtTipoCambio_polizas_abono_cuentas_cobrar').formatCurrency({ roundToDecimalPlace: 4 });
	        ...
	        $('#txtTipoCambio_polizas_abono_cuentas_cobrar').focusout(function(e){

	        	//Variable que se utiliza para asignar el tipo de cambio
				var intTipoCambio = parseFloat($.reemplazar($('#txtTipoCambio_polizas_abono_cuentas_cobrar').val(), ",", ""));

				//Si el tipo de cambio es mayor que el valor máximo permitido
	        	if(intTipoCambio > intTipoCambioMaximoPolizasAbonoCuentasCobrar)
	        	{
	        		$('#txtTipoCambio_polizas_abono_cuentas_cobrar').val(intTipoCambioMaximoPolizasAbonoCuentasCobrar);
	        	}

	        	//Si no existe tipo de cambio 
	        	if($('#txtTipoCambio_polizas_abono_cuentas_cobrar').val() == '' || $('#txtProspectoID_polizas_abono_cuentas_cobrar').val() == '' )
	        	{
	        		//Deshabilitar botón  Buscar facturas
					$('#btnBuscarDoc_polizas_abono_cuentas_cobrar').attr("disabled", "disabled"); 
					//Inicializar valores de los acumulados
	         		$('#acumTotalFactura_fras_relacionadas_polizas_abono_cuentas_cobrar').html('$0.00');
	         		$('#acumAbono_fras_relacionadas_polizas_abono_cuentas_cobrar').html('$0.00');
	         		$('#acumSaldoFactura_fras_relacionadas_polizas_abono_cuentas_cobrar').html('$0.00');
			...
			*********************************************************************************************************************/
            ...
            cargar_monedas_polizas_abono_cuentas_cobrar();
